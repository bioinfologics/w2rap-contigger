...
void writeLibraryStats( std::ostream & out, const vec<PM_LibraryStats> & stats)
{
  ...
  if (stats.empty()) {
    out << "No libraries found!" << std::endl;
    return;
  }
...
void PairsManager::writeBinary( BinaryWriter& writer ) const {
  ...
  // writer.write( _libs );
  vec< std::pair<int,int> > lib_stats;
  for ( size_t i = 0; i < nLibraries(); i++ )
    lib_stats.push_back( std::make_pair( _libs[i]._sep, _libs[i]._sd ) );
...
// Write nicely formatted library information to an output stream.
void
PairsManager::printLibraryStats( std::ostream & out, const String bases_filename ) const
{
...
void
PairsManager::printPairInfo( std::ostream & out ) const
{
  // Print one line for each pair.
  std::cout << "  PAIR ID    READ 1    READ 2   LIB" << std::endl;
  std::cout << "  -------    ------    ------   ---" << std::endl;
  char line[40];
			    ...
			    const bool grow_read_count)
{
  ...
  if ( lib_ID == -1 ) {
    ...
    ForceAssertLe( _libs.size(), std::numeric_limits<unsigned char>::max() );
  }
