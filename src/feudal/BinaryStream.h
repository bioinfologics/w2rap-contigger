...
template<class Itr>
void BinaryWriter::writeItr(Itr begin, Itr const& end ) {
    while (begin != end ) {
        write(*begin);
        ++begin;
    }
...
template<class T>
void BinaryWriter::write(T const* begin, T const* end ) {
    writeArray(begin, end, typename Serializability<T>::type());
}

template<class Itr>
void BinaryReader::readItr(Itr begin, Itr const& end ) {
    while (begin != end ) {
        read(&*begin);
        ++begin;
    }
...
template<class T>
void BinaryReader::read(T* begin, T* end ) {
