...
public:
    ...
    size_t size() const
    { return accumulate(mBlocks.begin(),mBlocks.end(),1ul,
...
void convertAppendParallel( Itr beg, Itr end, VecPQVec& vpqv ) {
  ...
  for (auto batchId = 0; batchId<nBatches; ++batchId){
    ...
    size_t maxChunkSz = alloc.getMaxEnchunkableSize();
    size_t maxUncompressed = (5*maxChunkSz+1)/2;
    //std::cout << "Batchid: "<< batchId << " off1: " << off1 << " off2: " << off2 << " Remain: " << remain << " nnn: "<< nnn << " MaxChunkSz: " << maxChunkSz << " maxUncompressed: " << maxUncompressed << std::endl;
