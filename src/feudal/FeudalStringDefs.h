...
{ if ( pos == npos ) pos = size();
  ...
  { using std::min;
    ...
    const_pointer itr = end + std::min(pos,size()-n) + 1;
    while ( itr > end )
      if ( !Traits::compare(--itr,cstr,n) )
        return itr - end; }
  return npos; }
...
{ if ( pos == npos ) pos = size();
  AssertLe(pos,size());
  if ( size() )
  { using std::min; pos = std::min(pos,size()-1);
    const_pointer end = data();
    const_pointer itr = end + std::min(pos,size()-1) + 1;
    while ( itr > end )
      if ( Traits::eq(*--itr,c) )
        return itr - end; }
  return npos; }
...
{ size_t mySize = size();
  ...
  using std::min;
  int result = Traits::compare(data(), cstr, std::min(mySize,cstrSize));
...
{ size_t mySize = lenLimit(pos1,n1);
  size_t cstrSize = Traits::length(cstr);
  using std::min;
  int result = Traits::compare(data()+pos1, cstr, std::min(mySize,cstrSize));
  if ( !result ) result = ::compare(mySize,cstrSize);
  return result; }
...
{ n1 = lenLimit(pos1,n1);
  using std::min;
  int result = Traits::compare(data()+pos1, str.data(), std::min(n1, str.size()));
...
{ n1 = lenLimit(pos1,n1);
  ...
  using std::min;
  int result = Traits::compare(data()+pos1, str.data()+pos2, std::min(n1, n2));
...
{ n1 = lenLimit(pos1,n1);
  using std::min;
  int result = Traits::compare(data()+pos1, cstr, std::min(n1, n2));
  if ( !result ) result = ::compare(n1,n2);
  return result; }
...
{ int_size_assert();
  ...
  using std::min; endSearchAt = std::min(endSearchAt,size());
...
{ size_type len = Traits::length(cstr);
  using std::min; if ( pos == npos ) pos = size() - std::min(size(),len);
