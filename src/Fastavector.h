...
private:
    ...
    static bool fexist( const std::string& filename ) {
...
public:
	...
	while ( getline(in, linebuf) ) {		// not terribly efficient, but the index will be short
		...
		if ( i == std::string::npos ) FatalErr( "non-emtpy line with no tab or space in FASTA index " << filename);
		...
		if ( linebuf[i] == ' ' ) {			// if we didn't hit a tab, then there's a fasta comment
		    ...
		    if ( j == std::string::npos ) FatalErr( "non-emtpy line with no 2nd tab in FASTA index " << filename);
		...
		std::istringstream s( linebuf.substr(j+1) );
