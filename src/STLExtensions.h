...
namespace std {
template <>
struct modulus<float> : public std::binary_function<float, float, float> {
...
template <>
struct modulus<double> : public std::binary_function<double, double, double> {
...
struct order_ShallowView: public std::binary_function< T, T, bool > {
  private:
...
struct cmp_functor: public std::binary_function< T, T, bool > {
...
inline std::pair< _T1, _T2 > CanonPair( const std::pair< _T1, _T2 >& p ) {
    return p.first <= p.second ? p : make_pair( p.second, p.first );
}

template <class _T1, class _T2>
inline std::pair< _T1, _T2 > CanonPair( const _T1& v1, const _T2& v2  ) {
...
template<class T1, class T2> int64_t LowerBound1(
    const std::vector< std::pair<T1,T2> >& x, const T1& t ) {
...
template<class T1, class T2, class T3> int64_t LowerBound1(
    const std::vector< triple<T1,T2,T3> >& x, const T1& t ) {
    int64_t count = x.size( ), it, step, first = 0;
    while ( count > 0 ) {
        it = first;
        step = count/2;
...
template<class T1, class T2, class T3, class T4> int64_t LowerBound1(
    const std::vector< quad<T1,T2,T3,T4> >& x, const T1& t ) {
    int64_t count = x.size( ), it, step, first = 0;
    while ( count > 0 ) {
        it = first;
        step = count/2;
...
int64_t BinPosition1( const std::vector<std::pair<T1,T2>,A>& v, const U& x1 ) {
...
int64_t BinPosition1( const std::vector<triple<T1,T2,T3>,A>& v, const U& x1 ) {
    if ( v.size( ) == 0 ) return -1;
    T1 const& x(x1);
    size_t first = 0, last = v.size( ) - 1, next;
    while(1) {
...
template <class _T1, class _T2, class _T3>
inline std::ostream& operator<< ( std::ostream& out, const triple<_T1, _T2, _T3>& __x ) {
    out << "(" << __x.first << ", " << __x.second << ", " << __x.third << ")";
    return out;
}

template < size_t N >
bool operator< ( const std::bitset< N >& b1, const std::bitset< N >& b2 ) {
