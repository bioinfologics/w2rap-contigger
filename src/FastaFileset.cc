...
FastaFilesetTemplate<vecT,dataT,streamT>::GetByName( const String &name, dataT &datum )
{
  ...
  std::pair<typename vec<NamedDataT>::iterator,typename vec<NamedDataT>::iterator> dataRange;
...
{
  m_log << Date() << ": Scanning FASTA files." << std::endl;
  ...
  m_log << Date() << ": Preparing to read " << totalSequences << " FASTA entries." << std::endl;
...
FastaPairedFileset::Parse()
{
  ...
  m_log << Date() << ": Pairing sequences with quality scores." << std::endl;
