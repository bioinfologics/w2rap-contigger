...
void alignment::Write( std::ostream& out ) const
{    int pos1, pos2;
...
void alignment::Read( std::istream& in )
...
void alignment::Write( std::ostream& out, int id1, int id2, Bool rc )
{    Write(out);
     BinWrite( out, id1 );
     BinWrite( out, id2 );
     BinWrite( out, rc );    }

void alignment::Read( std::istream& in, int& id1, int& id2, Bool& rc )
{    Read(in);
     BinRead( in, id1 );
     BinRead( in, id2 );
     BinRead( in, rc );    }
...
{
     align a = *this;
     std::cout << "pos1=" << a.pos1() << ", Pos1=" << a.Pos1() <<
        ", pos2=" << a.pos2() << ", Pos2=" << a.Pos2() << ", errors=" <<
        this->Errors() << std::endl;
     std::cout << "gaps lengths" << std::endl;
     for ( int i = 0; i < a.Nblocks(); ++i )
         std::cout << a.Gaps(i) << " " << a.Lengths(i) << std::endl;
}

void alignment_plus::Write( std::ostream& out ) const
{    int id1 = Id1( ), id2 = Id2( );
     BinWrite( out, id1 );
     BinWrite( out, id2 );
     BinWrite( out, score );
...
void alignment_plus::Read( std::istream& in )
{    int id1, id2;
     Bool rc2;
     BinRead( in, id1 );
     BinRead( in, id2 );
...
{    
     ...
     if (zero_gap) 
     // {    std::cout << "zero gap found, repeating\n"; // XXX
          // PRINT2( pos1, pos2 ); // XXX
          // std::cout << "gaps/lengths:"; // XXX
          // for ( unsigned int i = 0; i < gaps.length; i++ ) // XXX
          //     std::cout << " " << gaps(i) << "/" << lengths(i); // XXX
          // std::cout << "\n"; // XXX
          goto check_first_gap;    
          //     } // XXX

     Set( pos1, pos2, errors, gaps, lengths );    
     // std::cout << "leaving compactify" << std::endl; // ZZZ
          }
...
std::vector<int> alignment::MutationsGap1Gap2( const basevector& rd1, 
    ...
    read_id2_(read_id2)
{
     ...
     {    std::cout << "\nalignment_plus constructor: alignment found which fails test "
          ...
          std::cout << "gaps/lengths:";
          for ( unsigned int i = 0; i < gaps.length; i++ )
               std::cout << " " << gaps(i) << "/" << lengths(i);
          std::cout << std::endl;
          ForceAssert( 0 == 1 );    }    
     */
...
std::istream& operator>>( std::istream& in, vec<alignment_plus>& vecAligns )
{    
  VecAlignmentPlusReader( &in ).ReadAll( vecAligns );
  return in;
}

std::ostream& operator<<( std::ostream& out, const vec<alignment_plus>& vecAligns )
{    
...
{    
     ...
     std::set<int> original_reads, new_overlaps;
     ...
     for ( std::set<int>::iterator i = new_overlaps.begin( ); i != new_overlaps.end( );
          i++ )
     {    int id = *i;
          int ind = all_aligns_index[id];
...
{    int pos1, pos2, errors;
     ...
     {    std::cout << "\nWarning: test " << test_no << " for proper alignment failed\n";
          std::cout << "(This should not be a problem.)" << std::endl;
          std::cout << ( ap.Rc2( ) ? "reverse" : "forward" ) << " alignment\n";
          PRINT2( id1, id2 );
          PRINT2( pos1, Pos1 );
          PRINT2( pos2, Pos2 );
          ...
          if ( !ap.Rc2( ) ) 
               std::cout << ActualErrors( EE[id1], EE[id2], ap.a ) << "\n";
          else 
          {    basevector EErcid2 = EE[id2];
               EErcid2.ReverseComplement( );
               std::cout << ActualErrors( EE[id1], EErcid2, ap.a ) << "\n";    }
          std::cout << "gaps/lengths:";
          for ( unsigned int i = 0; i < lengths.length; i++ )
               std::cout << " " << gaps(i) << "/" << lengths(i);
          std::cout << "\n";
...
{    avector<int> gaps, lengths;
     ...
     if ( pos1 - pos1_start < n )
     {    std::cout << "Warning: TrimAlignmentFront found pos1 - pos1_start < n\n";
...
{    int Pos1 = a.Pos1( ) - 1, Pos2 = a.Pos2( ) - 1;
     ...
     if ( j < 0 )
     {    std::cout << "Warning: TrimAlignmentBack trimmed entire alignment\n";
          PRINT2( Pos1, Pos1_start );
	  PRINT( n );    }
