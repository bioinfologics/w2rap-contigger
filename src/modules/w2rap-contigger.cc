...
struct cmdline_args parse_cmdline_args( int argc,  char* argv[]) {
    ...
    try {
        options.add_options()
                ...
                ("min_freq","minimum frequency for k-mers on first DBG (default: 4)",cxxopts::value(parsed_args.minFreq))
                ("min_qual","quality to trim read ends on first DBG (default: 0 - don't trim)",cxxopts::value(parsed_args.minQual))
                ("clip_size","max size in kmers of tips to clip at small_K stage (default: 59)",cxxopts::value(parsed_args.maxTipKmers))
...
//                std::shared_ptr<KmerList> kmercounts=std::make_shared<KmerList>();
            ...
            case 3: {
                ...
                buildReadQGraph(args.out_dir, FILL_JOIN, FILL_JOIN, args.minFreq, .75, args.maxTipKmers, 0, &hbv, &paths,
                                args.small_K);
            ...
            case 4: {
                ...
                if (!file_exist(args.out_dir + "/" + args.prefix + "." + step_inputg_prefix[ostep] + ".paths") or args.from_step < 4) {
                    WriteReadPathVec(paths, (args.out_dir + "/" + args.prefix + "." + step_outputg_prefix[ostep] + ".paths").c_str());
                }

                if ( !file_exist(args.out_dir + "/" + args.prefix + "." + step_inputg_prefix[ostep] + ".hbv") or args.from_step < 4) {
