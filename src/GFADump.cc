...
void GFADumpAbyss(std::string filename, const HyperBasevector &hb, const vec<int> &inv,
              ...
              const std::vector<int> & marked_edges)
{
  ...
  if (find_lines) {
      Ofstream(gfa_out, filename + "_lines_abyss.gfa");
      Ofstream(fasta_out, filename + "_lines_abyss.fasta");
                  ...
                  if (path.empty()) {//empty path (i.e., gap!)
                      ...
                      for (auto edge: path) {
                          if (canonical_included[edge] == -1) {
                              ...
                              gfa_out << "S\t" << ce << "\t*"
                              << "\tLN:i:" << eo.isize()
                              << "\tKC:i:" << invPaths.at(ce).size()
                              << "\tCL:Z:" << colour_names[current_colour % colour_names.size()]
                              << "\tUR:Z:" << prefix << "_lines_abyss.fasta"
                              << std::endl;
                              ...
                              // write seq to FASTA
                              fasta_out << ">" << ce << " " << eo.isize() << " " << invPaths.at(ce).size() << std::endl << eo << std::endl;
                          ...
                          //}
                          if (prev_in_path != -1) {
                              gfa_out << "L\t" << prev_in_path << (prev_in_path_fw ? "\t+\t" : "\t-\t") <<
                      ...
                      for (auto pe:prev_segment_end_edges) {
                          gfa_out << "L\t" << pe.first << (pe.second ? "\t+\t" : "\t-\t") << ce <<
  ...
  // Do the raw dump
  Ofstream(gfa_raw_out,filename+"_raw_abyss.gfa");
  Ofstream(fasta_raw_out,filename+"_raw_abyss.fasta");
...
ReadPathVec &paths, const int MAX_CELL_PATHS, const int MAX_DEPTH, bool find_lines, const std::vector<int> & marked_edges){
  ...
  // Do the raw dump
  Ofstream(gfa_raw_out,filename+".gfa");
  Ofstream(fasta_raw_out,filename+".fasta");
  std::cout<<"Dumping edges"<<std::endl;
