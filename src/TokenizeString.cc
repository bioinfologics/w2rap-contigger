	      ...
	      vec<String> &tokens )
{
  ...
  // Parse string.
  vec< std::pair<int, int> > token_interval;
  bool in_token = false;
  int token_start = 0;
  for (int ii=0; ii<(int)a_string.size( ); ii++) {
    ...
    if ( matches_separator ) {
      if ( in_token ) {
	token_interval.push_back( std::pair<int, int>( token_start, ii ) );
	in_token = false;
      }
    }
    else {
  ...
  if ( in_token )
    token_interval.push_back( std::pair<int, int>( token_start, (int)a_string.size() ) );
...
void RegexMatch(String regex, String &str, vec<String> &matches)
{
    ...
    std::vector<char> a_comma(1); a_comma[0] = ',';
