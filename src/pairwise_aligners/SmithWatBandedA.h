...
template<class X> float SmithWatBandedA2( const basevector& S, 
     ...
     std::ostream* log = 0, int mis=2, int ins=3, int del=3 );

inline float SmithWatBandedA( const basevector& S, const basevector& T, 
     int offset, int bandwidth, align& a, int& errors, std::ostream* log = 0, 
     int mis=2, int gap=3  )
{    
...
public:
    ...
    float run( const basevector& S, const basevector& T
             ...
             , std::ostream* log = 0, int mis=2, int gap=3  ){
...
inline float SmithWatBandedA( const basevector& S, const basevector& T, 
     int offset, int bandwidth, alignment& a, std::ostream *log = 0 )
