...
public:
  ...
  friend std::istream& operator>>(std::istream & is, BMG_node & nodes) 
  {
    ...
    //static int nn = 0;
    //if (nn++ < 10) std::cout << size << std::endl;
...
public:
  ...
  inline void push_back(const std::pair< int, mutmer_read_idY<I> > & merge_node)
  {   
