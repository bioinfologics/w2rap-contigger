...
float SmithWatBandedA2( const basevector& S, const basevector& T, int offset,
     int bandwidth, align& a, int& errors, std::ostream *log, int MIS, int INS, int DEL)
{
	 ...
	 out << "Warning: SmithWatBandedA passed nonsense arguments:" << std::endl;
     ...
     {    
          ...
          {    if ( a_count > 0 ) std::cout << " a^" << a_count << " "; // XXX
               a_count = 0; // XXX
               std::cout << letter;    } // XXX
          */
     ...
     {    
          ...
          /*
          std::cout << "Warning: SmithWatBandedA produced a garbage alignment, "
               << "presumably because of a bug.\n";
          std::cout << "Replacing the garbage alignment by an alignment which has "
               << "one-base overlap.\n";
...
template float SmithWatBandedA2<unsigned char>( const basevector&, 
     const basevector&, int, int, align&, int&, std::ostream*, int, int, int );
