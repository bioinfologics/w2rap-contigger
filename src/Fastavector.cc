...
void fastavector::Print( std::ostream& out, const String& id ) const
{
    out << '>' << id;
	     ...
	     )
{
	...
	int new_start = std::max( 0, start - pad );
	int ilen = fai[chr].len;
	int new_end = std::min( ilen, end + pad );
	...
	while ( getline(in, buf) && buf.size() > 0 && buf[0] != '>' && so_far < new_end ) {
		...
		auto istart = buf.begin() + std::max(0, new_start - so_far );
		auto iend = std::min( buf.end(), istart + new_end - so_far );
	 ...
	 )
{
	...
	if ( bufsize > 0 && so_far + bufsize > start ) {	// are we (almost) there yet (and not blank)?
	    auto istart = buf.begin() + std::max(0, start - so_far );
	    auto iend = buf.end() + std::min(0,  end - so_far - bufsize );
	    fv.append( istart, iend );
	}
	so_far += bufsize;
    }
...
template void digraphE<fastavector>::PrettyDOT( std::ostream& out, 
