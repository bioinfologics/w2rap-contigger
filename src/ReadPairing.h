...
class read_pairing {
  ...
  public:
    ...
    friend std::ostream& operator<<( std::ostream& o, const read_pairing& p );
    friend std::istream& operator>>( std::istream& i, read_pairing& p );
    friend std::istream& operator>>( std::istream& i, vec<read_pairing>& v );

};
TRIVIALLY_SERIALIZABLE(read_pairing);

void ReadBinaryPairs ( std::istream &in, vec<read_pairing> &vecPairs );
