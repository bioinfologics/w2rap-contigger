...
FastaFilestreamPreview::FastaFilestreamPreview(std::istream& filestream)
  : max_sequence_size_(0), start_offset_(0)
{
  ...
  *bufptr = 0;
  ...
  std::streamoff offset_of_buffer = 0;
  ...
  std::streampos size_of_curr_sequence = 0;
  std::streampos offset_of_last_sequence = start_offset_;
  ...
  do
  {
    ...
    if ( *bufptr == '>' )
    {
      if ( bufptr > buffer &&
           *(bufptr-1) == '\n' )
      {
        std::streampos offset_of_curr_sequence = offset_of_buffer + static_cast<int>( bufptr - buffer );
...
const std::streampos FastaFilestreamPreview::getMaxSequenceSize()
{
  return max_sequence_size_;
}

vec<std::streampos>& FastaFilestreamPreview::getSequenceSizes()
{
  return sequence_sizes_;
}

const std::streampos FastaFilestreamPreview::getStartOffset()
{
