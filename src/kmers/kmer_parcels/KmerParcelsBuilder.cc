...
void PrintValueAndPercentage(std::ostream & out, const String & s, const size_t val, const double total)
{
  out << s 
      << std::setw(12) << val 
      << " " << std::setw(5) << std::fixed << std::setprecision(1) << (100.0 * val) / total << "%" 
      << std::endl;
}
                                  ...
                                  const size_t n_parcels)
{
  if (n_parcels <= 8) {
    ...
    std::cout << "**** n_parcels = " << n_parcels << " > 32768!" << std::endl;
    std::cout << "     Expand ComputeParcelIDFromBaseVec(...) in kmers/kmer_parcels/KmerParcelsBuilder.cc"
      " to accomodate more parcels." << std::endl; 
  }
                                      ...
                                      const size_t n_threads)
{
  ...
  //std::cout << "n_sets = " << n_parcel_sets <<std::endl;
  //std::cout << "n_parcels = " << n_parcels << std::endl;
...
{
  //std::cout <<  "parcel_vec_vec<K>(" << _n_parcels << ", " << _n_threads << ")" << std::endl;
...
void  KmerParcelsBuilder::ComputeRead1IDs()
{
  ...
  std::cout << "n_kmers_total = " << _n_kmers_total << std::endl;
  std::cout << "n_threads = " << _n_threads << std::endl;
  */
  ...
  /*
  std::cout << "n_blocks = " << _read1_IDs.size() << std::endl;
  for (size_t block_ID = 0; block_ID < _n_threads; block_ID++) {
    std::cout << "read1_IDs[" << block_ID << "] = " << _read1_IDs[block_ID] 
...
void KmerParcelsBuilder::Build(const size_t n_parcels)
{
  ...
  if (_n_parcels == 0) {
    ...
    // ribeiro: sorting a vec< std::pair<BaseVec, KmerLoc> >
