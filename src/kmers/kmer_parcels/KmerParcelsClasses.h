...
public:
  ...
  void BinaryStreamRead(std::istream & is) 
  { 
  ...
  void BinaryStreamWrite(std::ostream & os) const 
  { 
    ForceAssertGt(_n_bytes, 0u);
...
public:
  ...
  // output
  void Print(std::ostream& out, bool alt = false ) const
  {
    if ( alt ) {
      out << "[" << _read_ID
	  << " " << ( IsRC( ) ? "+" : "-" )
      ...
      out << std::setw(12) << _read_ID << " " 
	  << std::setw(12) << GetUnsignedPos() << " " 
	  << (IsRC() ? "(RC)" : "(FW)") << std::endl;
    }
  }
...
public:
  ...
  inline void PrintKmer(std::ostream& out = std::cout) 
  { 
    out << "[";
    for (size_t i = 0; i != _n_bytes; i++)
      out << std::setw(16) << std::hex << static_cast<uint16_t>(_kmer[i]) << " ";
    out << "]" << std::dec; 
  }
...
public:
  ...
  void PrintBinnedPos(std::ostream& out, const size_t read_len = 101) const
  {
    ...
    std::cout << " bin          FW          RC" << std::endl;
    for (size_t i = 0; i != n_bins; i++) 
      std::cout << std::setw(4) << i << "  " 
           << std::setw(10) << pos_fw[i] << "  " 
           << std::setw(10) << pos_rc[i] << std::endl; 
  }


  void PrintKmerLocs(std::ostream& out, const size_t max_freq = 0)
  {
    if (max_freq) {
      ...
      std::cout << ((max_freq < size()) ? "..." : "") << std::endl;
    }
  }
  
  void PrintKmerStats(std::ostream& out, const size_t read_len = 101) const
  {
  ...
  // output 
  void Print(std::ostream& out, const size_t read_len = 101) const
  {
    BaseVec kmer_bv;
    kmer_bv.assignBaseBits(GetK(), _kmer.CharPointer());
  ...
  void Print(std::ostream& out, const QualNibbleVec & qv, const size_t read_len = 101) const
  {
    BaseVec kmer_bv;
    kmer_bv.assignBaseBits(GetK(), _kmer.CharPointer());
