...
void PrintKmerBaseVec(const BaseVec & bv, 
                      const size_t color = 3, std::ostream& out = std::cout);

void PrintKmerBaseVec(const BaseVec & bv, const QualNibbleVec & qv, 
                      const size_t color, std::ostream& out);
...
public:
    ...
    : KmerParcelsStore(K)
  {
    ...
    if (IsRegularFile(_n_parcels_fn)) {    
      ...
      std::ifstream ifs(_n_parcels_fn.c_str());
      ifs >> _n_parcels;
      ifs.close();
      
      if (IsRegularFile(_n_reads_fn)) {    
        std::ifstream ifs(_n_reads_fn.c_str());
        ifs >> _n_reads;
        ifs.close();
      }
    }
  ...
  {
    ForceAssertLt(parcel_ID, _n_parcels);
    //std::cout << "KmerParcelDiskStore.GetKmerParcelReader(): parcel_ID = " << parcel_ID << std::endl;
  ...
  {
    if (_keep_on_disk) {
      std::ofstream ofs(_n_parcels_fn.c_str());
      ofs << _n_parcels << std::endl;
      ofs.close();

      if (_n_reads != 0) {
        std::ofstream ofs(_n_reads_fn.c_str());
        ofs << _n_reads << std::endl;
        ofs.close();
      }
    }
    _log.close();
  ...
  void Log(String const & mess) { _log << mess << std::endl; }
...
public:
  ...
  {
    //std::cout << "KmerParcelReader constructor: parcelID = " << parcel_ID << std::endl; 
    //std::cout << "KmerParcelReader constructor: reader->parcelID = " << GetParcelID()  << std::endl; 
  }

  // the 'delete' matches the 'new' in parcels.GetKmerParcelReader
  ~KmerParcelReader() { 
    //std::cout << "KmerParcelReader destructor" << std::endl;
    delete _reader; } 
