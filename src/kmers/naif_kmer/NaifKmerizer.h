...
public:
  ...
  void print() const 
  {
    ...
    std::cout << TagNK() << "  time total       = " 
         << std::setw(6) << tot       << "s (100%)" << std::endl;
    std::cout << TagNK() << "  time parsing     = " 
         << std::setw(6) << parse     << "s (" << std::setw(3) << 100 * parse / tot << "%)" << std::endl;
    std::cout << TagNK() << "  time collecting  = " 
         << std::setw(6) << collect   << "s (" << std::setw(3) << 100 * collect / tot << "%)" << std::endl;
    std::cout << TagNK() << "  time sorting     = " 
         << std::setw(6) << sort      << "s (" << std::setw(3) << 100 * sort / tot << "%)" << std::endl;
    std::cout << TagNK() << "  time summarizing = " 
         << std::setw(6) << summarize << "s (" << std::setw(3) << 100 * summarize / tot << "%)" << std::endl;
    std::cout << TagNK() << "  time merging     = " 
         << std::setw(6) << merge     << "s (" << std::setw(3) << 100 * merge / tot << "%)" << std::endl;
    std::cout << TagNK() << "  time waiting     = " 
         << std::setw(6) << wait      << "s (" << std::setw(3) << 100 * wait / tot << "%)" << std::endl;
    std::cout << TagNK() << "  time idling      = " 
         << std::setw(6) << idle      << "s (" << std::setw(3) << 100 * idle / tot << "%)" << std::endl;
  }
};
...
private:
  ...
  vec<rec_t> & _build_parcel(const size_t i_sub_parcel)
  {
    ...
    if (_verbosity >= 3) 
      std::cout << TagNK() << "i_sub_parcel= " << i_sub_parcel 
           << ": reserved space for " << n_recs << " records (n_blks= " << n_blks << ")." << std::endl;
...
public:
  ...
  void operator() (const size_t i_thread) 
  {
    ...
    for (size_t i_pass = 0; i_pass < _n_passes; i_pass++) {
      ...
      if (i_thread == 0) {
        if (_verbosity == 1) 
          std::cout << TagNK() << str_pass << " parse" << std::flush;
        if (_verbosity >= 2) 
          std::cout << TagNK() << str_pass << " Parsing reads for kmers." << std::endl;
      }
      ...
      if (_verbosity >= 3) 
	std::cout << TagNK() << str_pass << str_thread 
	     << " bvs: [" << i0_bv << " : " << i1_bv << "] " 
	     << i1_bv - i0_bv << " to parse." << std::endl;
      ...
      if (_verbosity >= 3) 
	std::cout << TagNK() << str_pass << str_thread << " Done parsing." << std::endl;
      ...
      if (_verbosity >= 3) 
	std::cout << TagNK() << str_pass << str_thread 
	     << " Collect parcel " << i_parcel << "." << std::endl;
      else if (i_thread == 0) {
        if (_verbosity >= 2) std::cout << TagNK() << str_pass << " Collecting parcels." << std::endl;
        if (_verbosity == 1) std::cout << " collect" << std::flush;
      }
      ...
      if (_verbosity >= 3) 
	std::cout << TagNK() << str_pass << str_thread 
	     << " Done collecting parcel of size " << parcel.size() << ". Sorting now." << std::endl;
      else if (i_thread == 0) {
        if (_verbosity >= 2) std::cout << TagNK() << str_pass << " Sorting parcels." << std::endl;
        if (_verbosity == 1) std::cout << " sort" << std::flush;
      }
      ...
      if (_verbosity >= 3) 
	std::cout << TagNK() << str_pass << str_thread 
	     << " Done sorting. Summarizing " << n_recs << " records now." << std::endl;
      else if (i_thread == 0) {
        if (_verbosity >= 2) std::cout << TagNK() << str_pass << " Summarizing consecutive kmers." << std::endl;
        if (_verbosity == 1) std::cout << " summarize" << std::flush;
      }
	...
	i0 = i1;
      ...
      if (_verbosity >= 3) 
	std::cout << TagNK() << str_pass << str_thread 
	     << " Done summarizing. Merging now." << std::endl;
      else if (i_thread == 0) {
        if (_verbosity >= 2) std::cout << TagNK() << str_pass << " Merging results." << std::endl;
        if (_verbosity == 1) std::cout << " merge" << std::flush;
      }
      ...
      if (i_thread == 0) {
        if (_verbosity >= 2) std::cout << TagNK() << str_pass << " Done." << std::endl;
        if (_verbosity == 1) std::cout << std::endl;
      }
    }
                  ...
                  const size_t   mem_mean_ceil_user = 0)
{
  ...
  if (verbosity > 0) std::cout << TagNK() << "Starting kmerization." << std::endl;
  ...
  if (n_k_total == 0) {
    std::cout << TagNK() << "No kmers to process. Something is wrong with the data. Aborting." << std::endl;
    CRD::exit(1);
  }
  ...
  if (verbosity > 0) {
    std::cout << TagNK() << "  n_threads     = " << std::setw(18) << n_threads << std::endl;
    std::cout << TagNK() << "  K             = " << std::setw(18) << K << std::endl;
    std::cout << TagNK() << "  sizeof_krec   = " << std::setw(18) << sizeof(KmerRec_t) << std::endl;
    std::cout << TagNK() << "  n_bv          = " << std::setw(18) << n_bv << std::endl;
    std::cout << TagNK() << "  n_kmers       = " << std::setw(18) << n_k_total << std::endl;
    std::cout << TagNK() << "  mem_total     = " << std::setw(18) << mem_total << std::endl;
    std::cout << TagNK() << "  mem_used      = " << std::setw(18) << mem_used << std::endl;
    std::cout << TagNK() << "  mem_avail     = " << std::setw(18) << mem_avail << std::endl;
    std::cout << TagNK() << "  mem_mean      = " << std::setw(18) << mem_mean << std::endl;
    std::cout << TagNK() << "  mem_mean_user = " << std::setw(18) << mem_mean_ceil_user << std::endl;
    std::cout << TagNK() << "  mem_needed    = " << std::setw(18) << mem_needed << std::endl;
    std::cout << TagNK() << "  mem_to_use    = " << std::setw(18) << mem_to_use << std::endl;
    std::cout << TagNK() << "  n_parcels     = " << std::setw(18) << n_parcels << std::endl;
    std::cout << TagNK() << "  n_passes      = " << std::setw(18) << n_passes << std::endl;
  }
  ...
  if (verbosity > 0) {
    ...
    timer.print();
    std::cout << TagNK() << "Done with kmerization. Took " << TimeSince(time_start) << "." << std::endl;
  }
}
