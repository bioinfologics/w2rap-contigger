    ...
    if (tag == tag_A) { // store only first kmer for the read set A
      if (kmer_cur.not_done()) {  // checks if there are any kmers in read
        ...
        if (parcels.in_one_parcel(kmer)) {
          ...
          parcels.add(kmer);
          if (0 && ibv == 34056714) {
            std::cout << "debug: fw " << hieroglyphs(kmer_cur.fw()) << std::endl;
            std::cout << "debug: bw " << hieroglyphs(kmer_cur.rc()) << std::endl;
            std::cout << "debug: palidrome " << kmer.is_palindrome << std::endl;
          }
        }
      }
    }
