...
public:
  ...
  {
    std::cout << "  QSsum: ";
    for (size_t base = 0; base != 4; base++) std::cout << " " << std::setw(5) << QSsum[base];
    std::cout << "  nQ: ";
    for (size_t base = 0; base != 4; base++) std::cout << " " << std::setw(2) << nQ[base];
    std::cout << "  QBcount: ";
    for (size_t base = 0; base != 4; base++) std::cout << " " << std::setw(2) << QBcount[base];
    std::cout << "  Win: " << highest_base;
    std::cout << "  recom: ";
    for (size_t base = 0; base != 4; base++) std::cout << unsigned(recommended[base]);
    std::cout << "  conf: ";
    for (size_t base = 0; base != 4; base++) std::cout << unsigned(confirmed[base]);
    std::cout << std::endl;
  }
...
public:
  ...
  {
    if (0) {
      std::cout << "test_confs_counts = " << test_confs_counts << std::endl;
      std::cout << "test_recoms_counts = " << test_recoms_counts << std::endl;
    }
  }
  ...
  explicit ErrorFinder(const ErrorFinder & that)
    : _n_threads(that._n_threads),
      ...
      _qs(that._qs)
  {
    //std::cout << "clone: qs.size = " << _qs.size() << std::endl; 
    //std::cout << "clone: n_blocks = " << _n_blocks << std::endl; 
    //std::cout << "clone: blocks.size() = " << _p_blocks->size() << std::endl; 
  }
                                 ...
                                 const size_t parcel_size) // parcel_size is for debug
  {
    ...
    while (blocks_todo.size() > 0) {
      ...
      if (debug)
        std::cout << Date() << ":[" << std::setw(10) << parcel_size << "] need lock for           " 
             << " " << todo_str(_p_blocks->size(), blocks_todo)
             <<  " blocks: " << _p_blocks->str() << std::endl; 
      ...
      if (debug)
        std::cout << Date() << ":[" << std::setw(10) << parcel_size << "] locked block " << std::setw(2) << iblk << " from    " 
             << " " << todo_str(_p_blocks->size(), blocks_todo)
             <<  " blocks: " << _p_blocks->str() << std::endl; 
      ...
      if (debug)
        std::cout << Date() << ":[" << std::setw(10) << parcel_size << "] unlocking block " << std::setw(2) << iblk << " from " 
             << " " << todo_str(_p_blocks->size(), blocks_todo)
             <<  " blocks: " << _p_blocks->str() << std::endl; 
      ...
      if (debug)
        std::cout << Date() << ":[" << std::setw(10) << parcel_size << "] unlocked block  " << std::setw(2) << iblk << " from " 
             << " " << todo_str(_p_blocks->size(), blocks_todo)
             <<  " blocks: " << _p_blocks->str() << std::endl; 
                         ...
                         const size_t i1k)
  {
    ...
    if (print_stack) {
      vec< std::pair<unsigned, size_t> > i_base_i_krec;
      for (size_t ik = i0k; ik != i1k; ik++) {
        ...
        i_base_i_krec.push_back(std::make_pair(ibc, ik));
      }
      ...
      for (size_t iik = 0; iik != nk; iik++) {
        ...
        const BaseVec  & bv   = _bases[ir];
        const unsigned   nb   = bv.size();
        oss << std::setw(12) << ir << (krec.rc ? " rc " : " fw ")
      ...
      for (unsigned base = 0; base < 4; base++)
        oss << "base " << hieroglyph(base) 
            ...
            << " q_sum= " << std::setw(8) << col.q_sum[base]
            << " n_q_hi= " << std::setw(4) << col.n_q_high[base]
            << std::endl;
      oss << std::endl;
      */
    ...
    /*
    for (size_t i_col = 0; i_col != _n_cols; i_col++) {
      std::cout << parcel[ikt].i_read << "[ " << parcel[ikt].i_base << " " << ((parcel[ikt].rc) ? "RC ]:" : "FW ]:");
      _qs[i_col].print();
    }
    */
    return print_stack;
                 ...
                 const size_t i1k)
  {
    ...
    if (false && parcel[i0k].palindrome) {
      std::cout << "FOUND PALINDROME!!!!" << std::endl;
    }
