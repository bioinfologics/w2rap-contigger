...
class KmerDictEntry {
  public:
    ...
    bool isPalindrome() const {
        return mForm == CanonicalForm::PALINDROME;
    }
    ...
    void assign( size_t kmerId, size_t edgeId, bool rc ) {
        mKmerID.setVal(kmerId);
        mEdgeID.setVal(edgeId);
        if ( rc ) {
            mForm = complement(mForm);
            mContext = mContext.rc();
        }
    }

    void toPrev( CanonicalForm form ) {
        mKmerID.setVal(mKmerID.val()-1);
        mForm = form;
    }
    ...
    class Printer {
      public:
        ...
        friend std::ostream& operator<<( std::ostream& os, Printer const& pr ) {
  ...
  public:
    ...
    Entry const& lookupSuccessor( IncrementalHasher& hasher,
        ...
        if ( K&1 ) form = itr[K/2+1]<2 ? CanonicalForm::FWD : CanonicalForm::REV;
            ...
            unsigned char first = itr[1];
            unsigned char last = baseN ^ 3;
...
class GraphEdge {
  public:
    ...
    size_t getInitialKmerId() const {
        return mInitialKmerID.val();
    }
    size_t getFinalKmerId() const {
        return mFinalKmerID.val();
    }
