...
class BigKMer {
  public:
    ...
    void addContext( KMerContext context ) {
        mContext |= context;
    }
    ...
    bool isFwd() const {
        return getForm()==CanonicalForm::FWD;
    }
    bool isRev() const {
        return getForm()==CanonicalForm::REV;
    }
    bool isPalindrome() const {
        return getForm()==CanonicalForm::PALINDROME;
    }
    ...
    friend std::ostream& operator<<( std::ostream& os, BigKMer const& kmer ) {
        PrintBasesIter(os,kmer.begin(),kmer.end(),K+1);
        return os;
    }
