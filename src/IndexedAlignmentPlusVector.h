...
{
 public:
  void  WriteDataHeader( std::ostream *pDataOut );
  void  WriteIndexHeader( std::ostream *pIndexOut );

  bool  VerifyDataHeader( std::istream *pDataIn );
  bool  VerifyIndexHeader( std::istream *pIndexIn );

  void  WriteNumAlignments( const String &strDataFilename, const int numAligns );
  void  WriteNumAlignments( std::ostream *pDataOut, const int numAligns );
  void  ReadNumAlignments( std::istream *pDataIn, int &numAligns );
...
class ReadBuffer
{
 public:
  ...
  void  Attach( std::istream *pIstream ) { mpIstream = pIstream; }
  ...
  void  JumpTo( const std::streampos filePos );
...
class VecAlignmentPlusReader
{
 public:
  ...
  VecAlignmentPlusReader( std::istream *pVectorIfstream,
                          const int bufferSize = 100000 );
 ...
 private:
  ...
  std::istream *mpDataIn, *mpIndexIn;
...
class VecAlignmentPlusWriter
{
 public:
  ...
  VecAlignmentPlusWriter( std::ostream *pVectorOstream,
                          const int bufferSize = 100000 );
 ...
 private:
  ...
  std::ostream *mpDataOut, *mpIndexOut;
