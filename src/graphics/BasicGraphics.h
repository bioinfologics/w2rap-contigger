...
inline void Render( const vec<graphics_primitive>& g, std::ostream& out ) {
    for ( int i = 0; i < g.isize( ); i++ )
        out << g[i].Render( ) << "\n";
}
...
inline void Display( vec<graphics_primitive> g, double w, double h, std::ostream& out ) {
...
inline void VerticalDisplay( vec< vec<graphics_primitive> > g, const vec<double>& h,
                             double w, std::ostream& out, double scale = 1.0, double xtrans = 200.0,
