...
const int          parsed_args::INVALID_INT = std::numeric_limits<int>::min() + 1;
const unsigned int parsed_args::INVALID_UINT = (unsigned int) -1 ;
const longlong     parsed_args::INVALID_LONGLONG = std::numeric_limits<longlong>::min() + 1;
...
void parsed_args::PrintTheCommandPretty( std::ostream& out, const String& prefix ) {
    String bar( "-----------------------------------------------------------"
                "---------------------" );
...
parsed_args::parsed_args( int argc, char *argv[], const Bool no_header )
    ...
    if ( argc == 1 || ( argc > 1 && (strcmp( argv[1], "-h" ) == 0  || strcmp( argv[1], "--help" ) == 0 ) ) ) {
        ...
        PrintTheCommandPretty( std::cout );
        PrintTheTee();
    }
}
...
void parsed_args::FetchArgsFromFile( const String& filename ) {
    if ( IsRegularFile(filename) ) {
        ...
        while(1) {
            ...
            std::istrstream iline( line.c_str( ) );
            while(iline) {
                iline >> a;
                if ( a.empty( ) ) break;
...
bool parsed_args::ParseString( const String& string ) {
    ...
    if ( !Member( name_, name ) ) {
        ...
        std::cout << "You've specified two values for " << name << ".\n"
...
void parsed_args::ConversionFailure(int i) {
    std::cout << "I couldn't convert the command line value \"" << def_[i] <<
...
void parsed_args::ValidationFailure(int i, const String& valid ) {
    std::cout << "Invalid command line value \"" << def_[i]
...
void parsed_args::ValidationFailureDefault(const String& name,
        const String& deflt,
        const String& valid ) {
    std::cout << "Invalid default command line value \"" << deflt
              << "\" for argument \"" << name << "\".\n"
              << "Valid value set/range is " << valid << "\n";
...
void parsed_args::NoValue(String n) {
    std::cout << "You must specify a command-line value for " << n << ".\n";
    System( command_ + " -h " + n );
    std::cout << "Aborting.\n\n";
    CleanUpAndExit(1);
}
...
String parsed_args::CheckForArgAndAbbreviation( String n, String abbr ) {
    ...
    if ( gotName && gotAbbr ) {
        std::cout << "You must not specify both an argument-" << n
...
void parsed_args::MakeTheTee(const String filenames) {
    if (filenames != "") {
        ...
        outputRedirectionPipe = new procbuf( pipe_command.c_str( ), std::ios::app );
        std::ostream *pipeout = new std::ostream( outputRedirectionPipe );
        ...
        if ( used_[i] == False ) {
            if ( name_[i] == "GDB" ) {
                if ( def_[i] == "True" ) use_gdb_for_tracebacks = True;
                    ...
                    std::cout << "illegal value for parameter GDB" << std::endl;
                    CleanUpAndExit(1);
                }
                continue;
            }
            if ( name_[i] == "NO_HEADER" ) {
                if ( def_[i] != "True" && def_[i] != "False" ) {
                    std::cout << "illegal value for parameter NO_HEADER" << std::endl;
                    CleanUpAndExit(1);
                }
                continue;
            }
            if ( name_[i] == "NH" ) {
                if ( def_[i] != "True" && def_[i] != "False" ) {
                    std::cout << "illegal value for parameter NH" << std::endl;
                    CleanUpAndExit(1);
                }
                continue;
            }
            if ( name_[i] == "MEM_MONITOR" || name_[i] == "MM" ) {
                if ( def_[i] != "True" && def_[i] != "False" ) {
                    std::cout << "illegal value for parameter " << def_[i] << std::endl;
                    CleanUpAndExit(1);
                }
                continue;
            }
            ...
            std::cout << "You should not have specified a value for " <<
                      name_[i] << ".\n";
            junk_found = True;
        }
    if (junk_found)
        if ( abort_if_found ) {
            ...
            std::cout << "Ignoring extra arguments.\n" << std::endl;
        }
}

/// The special command-line argument TEE="file1 file2..."
/// Redirects std::cout to the specified files.  This method tells the caller the
...
void parsed_args::SetOutputRedirection(const String filenames) {
    if (outputRedirectedTo != "") {
        std::cout << "Unable to redirect output to: " << filenames << std::endl;
        std::cout << "Output already redirected via TEE parameter." << std::endl;
    } else {
...
void parsed_args::PrintVersion() {
    std::cout << command_ << " r" << SVN_REVISION << std::endl;
    return;
}
...
void parsed_args::PrintArgHelp() {
    ...
    if (parsed_args::pretty_help) std::cout << usage_format;
    std::cout << "\nUsage: " << command_ << " arg1=value1 arg2=value2 ..." << std::endl;
    if (parsed_args::pretty_help) std::cout << END_ESCAPE;
    if (doc_) {
        WhitespaceFold(String(doc_), std::cout);
        std::cout << "\n";
    }
    vec<parsed_arg_help>::iterator firstOpt =
        stable_partition(arg_help_.begin(), arg_help_.end(),
                         std::mem_fun_ref(&parsed_arg_help::isRequired));
    if (firstOpt != arg_help_.begin()) {
        if (parsed_args::pretty_help) std::cout << header_format;
        std::cout << "\nRequired arguments:\n\n";
        if (parsed_args::pretty_help) std::cout << END_ESCAPE;
        copy( arg_help_.begin(), firstOpt,
              std::ostream_iterator<parsed_arg_help>(std::cout, "\n") );
    }
    ...
    if (print_special) {
        ...
        std::cout << parsed_arg_help( "GDB", "Bool", "", "False", "", "Whether to use GDB for tracebacks." )
                  << "\n"
        ...
        std::cout << std::endl << "To see additional special arguments, type: " << command_ << " --help special" << std::endl;
    }
    std::cout << std::endl;
}

std::ostream& operator<< (std::ostream& out, const parsed_arg_help& an_arg) {
