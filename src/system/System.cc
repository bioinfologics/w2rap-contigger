                        ...
                        int* pStatus, int* pErrNo )
{
  ...
  if (pid == -1) {
    ErrNo err;
    std::cout << std::endl;
    std::cout << "Fatal error at " << Date( ) << ": can't fork process." << std::endl;
...
std::vector<String> TokenizeCommand(const String &sentence)
{
    ...
    std::istream_iterator<String> itr = std::istream_iterator<String>(iss);
    std::istream_iterator<String> itr_end;
    char quote_flag = 0;
...
{    
    ...
    if (pid == -1)
    {
        ...
        ErrNo err;
        std::cout << "Can't fork process.  Exiting..." << std::endl;
    ...
    {    
        ...
        std::vector<String> args = TokenizeCommand(command);
        if (System("which " + args[0] + " > /dev/null") != 0)
        {    
            std::cout << "WARNING: Could not find the command: \"" << args[0] << "\" in your $PATH" << std::endl;
            std::cout << "         Please verify \"" << args[0] << "\" is available in your $PATH." << std::endl;
            std::cout << "         I will still attempt to run the command, but this will likely fail." << std::endl;
        }
        ...
        execv("/bin/sh", argv);
        ErrNo err;
        std::cout << "ERROR: execv() call to \"" << args[0] << "\" failed!" << std::endl;
        std::cout << "Error is" << err << std::endl;
        CRD::exit(127);
    }
    return pid;
...
{    String command;
     ...
     {    if ( system( command.c_str( ) ) == 0 ) return;
          std::cout << "copy failed, retrying in 10 seconds" << std::endl;
          sleep(10);    }
     for ( int n = 0; n < 10; n++ )
     {    if ( system( command.c_str( ) ) == 0 ) return;
          std::cout << "copy failed, retrying in 5 minutes" << std::endl;
          sleep(300);    }
     for ( int n = 0; n < 20; n++ )
     {    if ( system( command.c_str( ) ) == 0 ) return;
          std::cout << "copy failed, retrying in 10 minutes" << std::endl;
          sleep(600);    }
     if ( !append )
...
{    int ntries = 0;
     ...
     {    int answer = stat( path, buffer );
          ...
          {    ++ntries;
               ...
               if ( ntries <= 5 )
               {    std::cout << "stat of " << path << " failed with "
                         << error_message << "retrying in 10 seconds" << std::endl;
                    sleep(10);    }
               else if ( ntries <= 15 )
               {    std::cout << "stat of " << path << " failed with "
                         << error_message << "retrying in 5 minutes" << std::endl;
                    sleep(300);    }
               else if ( ntries <= 25 )
               {    std::cout << "stat of " << path << " failed with "
                         << error_message << "retrying in 10 minutes" << std::endl;
                    sleep(600);    }    }
          else return answer;    }    }
...
{    std::vector<String> allfiles;
     ...
     std::ifstream temp( tempfile.c_str( ) );
     while (temp)
     {    String f;
          temp >> f;
...
std::vector<String> AllFilesWithPrefix( const String &prefix, const String &dirname )
{
    std::vector<String> allfiles;
    ...
    if ( !dir_ptr )
    {
      ...
      {
        case ENOENT:
          std::cout << "Error: " << dirname << " does not exist." << std::endl;
          break;
        case ENOTDIR:
        ...
        {
            ErrNo err;
            std::cout << "Error opening director" << err << std::endl;
            break;
        }
      }
...
{    o.open( f.c_str( ), std::ios::app | std::ios::out );
...
void PlainFold( const String &any_word, std::ostream& out, int start, int stop )
{
  int page_width = 80;
  if ( -1 == stop )
...
void WhitespaceFold( const String &text, std::ostream& out, int width, String indent )
{
  vec<String> lines;
  Tokenize(text,lines,"\n");
...
void CpIfNewer( String fn1, String fn2, Bool ignoreErrors ) {
  ...
  if ( !IsRegularFile( fn1 ) ) {
      std::cout << " Warning: could not find " << fn1
	   << " so not updating " << fn2 << std::endl;
...
{    String pscom = "ps -p " + ToString(pid) + " -o command";
     ...
     {    std::cout << "Bummer.  I can't run ps to determine the command name." << std::endl;
          std::cout << "failed: " << pscom << std::endl;
          struct stat buf;
          if ( Stat( tempfile.c_str( ), &buf ) != 0 )
          { ErrNo err;
            std::cout << "Stat of " << tempfile << " failed" << err << std::endl;
            CRD::exit(-1);    }
          if ( !S_ISREG(buf.st_mode) )
          {    std::cout << tempfile << "does not exist" << std::endl;
               CRD::exit(-1);     }
          {    std::ifstream in;
               in.open( tempfile.c_str( ) );
               if ( !in )
               {    std::cout << tempfile << " exists but I can't open it." << std::endl;
                    remove( tempfile.c_str( ) );
                    CRD::exit(-1);    }
               std::cout << "Here's what happened:" << std::endl;
               String line;
               while(1)
               {    getline( in, line );
                    if ( !in ) break;
...
{    std::vector<String> allfiles;
     {    temp_file tempfile( "/tmp/tmp_AllFilesInSource_XXXXXXX" );
          ...
          if ( allfiles.size( ) == 0 )
          {    std::cout << "Your SOURCE expands to zero files.  Abort.\n";
               CRD::exit(1);    }    }
     return allfiles;    }
...
{    std::ostringstream out;
     if ( s < 60.0 ) out << std::setprecision(3) << s << " seconds";
     else if ( s < 3600.0 || top == "minutes" )
          out << std::setprecision(3) << s/60.0 << " minutes";
     else if ( s < 86400.0 || top == "hours" )
          out << std::setprecision(3) << s/3600.0 << " hours";
     else out << std::setprecision(3) << s/86400.0 << " days";
     return out.str( );    }
...
{    std::ostringstream out;
     out << std::setiosflags(std::ios::fixed) << std::setprecision(2)
          << MemUsageGB( ) << std::resetiosflags(std::ios::fixed) << " GB";
     return out.str( );    }

String PeakMemUsageGBString( )
{    std::ostringstream out;
     out << std::setiosflags(std::ios::fixed) << std::setprecision(2)
          << PeakMemUsageGB( ) << std::resetiosflags(std::ios::fixed) << " GB";
     return out.str( );    }
...
{    double p = physicalMemory( ) / double( 1024 * 1024 * 1024 );
     std::ostringstream out;
     out << std::setiosflags(std::ios::fixed) << std::setprecision(2)
          << p << std::resetiosflags(std::ios::fixed) << " GB";
     return out.str( );    }
...
{    if ( x > -1.0 && x < 1.0 ) out << std::setprecision(d) << x;
     else
     {    out << std::setiosflags(std::ios::fixed) << std::setprecision(d)
               << x << std::resetiosflags(std::ios::fixed);    }    }

String Getenv( const String& var, const String& default_val )
{    char* valp = getenv( var.c_str( ) );
     if ( valp == 0 )
     {    if ( default_val == "" )
          {    std::cout << "Getenv: environment variable " << var << " is undefined.\n";
               std::cout << "Abort.\n";
               TracebackThisProcess( );    }
...
//[GONZA] fix, ifstram as bool not legal in > c++11, changed to return true or false
