...
inline void* ReturnAddress(int i)
{
          ...
          cout << "ReturnAddress only works if you compile with g++." << std::endl;
          exit(1);
          return( std::cout << std::endl ) ; // returns (void*)
...
{    
     ...
     hex_stream << hex << returnAddress << std::endl;
     String sp_addr;
     hex_stream >> sp_addr;
...
{    
     // if ( !minimal) out << "]\n" << std::endl; // closure for calls to PrintFrame
     ...
     {    strstream hex_stream;
          #if __ia64 || __x86_64
               hex_stream << hex << (void *)(((char *)sp_addresses[i])-1) << std::endl;
          #else
               hex_stream << hex << sp_addresses[i] << std::endl;
          #endif
          String sp_addr;
          hex_stream >> sp_addr;
     ...
     {    out << "Call to " << backtracer << " failed." << std::endl;
          out << "Perhaps an interrupt was received, or perhaps "
               << backtracer << " could not be found." << std::endl << std::endl;
          _exit(1);    }
...
void dump_stack( String command, ostream& out, Bool exit_when_done = True,
     Bool minimal = False )
{    if ( !minimal ) out << "\nDump of stack:" << std::endl << std::endl;
     vector<void*> sp_addresses;

#ifdef __solaris
     cout << "Sorry, stack dump does not work on Solaris " << std::endl;

     exit(1);
#else //__solaris
     #ifndef __GNUC__
          cout << "Sorry, stack dump only works if code was "
               << "compiled with g++." << std::endl;
          exit(1);
     #endif
...
void TracebackThisProcess( ostream& out, Bool exit_when_done, Bool minimal )
{
     ...
     out << "(If process stops, you may have to foreground (fg) it.)" << std::endl;
     Echo( "attach " + ToString(pid), "bt_file" );
     Echo( "bt",  "bt_file" );
     Echo( "quit",  "bt_file" );
     if ( System( "gdb -batch -x bt_file -q " + exe ) != 0 )
          out << "Bummer.  My call to gdb failed." << std::endl;
     Remove( "bt_file" );
...
{    cout << "\nDang dang dang, we've got a problem." << std::endl;
     cout << "Attempt to allocate memory failed, memory usage before call = "
          << MemUsageGBString( ) << "." << std::endl;
     cout << "-----------------------------------------------------------------"
          << "---------------" << std::endl;
     cout << "Top memory processes on this server now:" << std::endl;
#ifdef __alpha
     ...
     cout << "-----------------------------------------------------------------"
          << "---------------" << std::endl;
     cout << "Stack trace (sometimes informative):" << std::endl;
     TracebackThisProcess( cout, False, True );
     cout << "-----------------------------------------------------------------"
          << "---------------" << std::endl;
     cout << "Giving up.  Here are some possible solutions:" << std::endl
          ...
          << "(if available)." << std::endl << std::endl;
     _exit(1);    }
...
void SetLimits()
{
    ...
    if ( rlim.rlim_cur < stackBytes || rlim.rlim_cur == RLIM_INFINITY )
    {
        ...
        cout << "Performing re-exec to adjust stack size." << std::endl;
...
{  
  ...
  switch ( signal_number ) {
    case SIGSEGV: std::cout << SIGSEGV_msg << std::endl; break;
    case SIGFPE:  cout << SIGFPE_msg << std::endl;  break;
    case SIGINT:  cout << SIGINT_msg << std::endl;  break;
    case SIGBUS:  cout << SIGBUS_msg << std::endl;  break;
    case SIGILL:  cout << SIGILL_msg << std::endl;  break;
    case SIGABRT: std::cout << SIGABRT_msg << std::endl; break;
    case SIGTERM: std::cout << SIGTERM_msg << std::endl; break;
    case SIGCHLD:
        ...
        {
            cout << "Child process " << info->si_pid
                << " terminated abnormally. Stopping." << std::endl;
        }
        else
        {
            cout << "Harmless SIGCHLD - we shouldn't be here." << std::endl;
        }
        break;        
    default:      
      cout << "Unrecognized signal (" << signal_number << ") received.  Stopping." << std::endl;
  }
}
    ...
    Bool no_ctrlc )
{
  ...
  {    cout << "\nGenerating a backtrace..." << std::endl;
...
void ArachneInterruptHandler(ArachneSignalHandler* pSigFunc)
{
  ...
  { 
      cout << "Unable to catch unaligned accesses." << std::endl;
      exit(1);
  }
  #endif
  ...
  {
    cout << "Error catching SIGSEGV signal." << std::endl;
    exit(1);
  }
  if ( sigaction( SIGFPE, &act, &oact ) < 0 )
  {
    cout << "Error catching SIGFPE signal." << std::endl;
    exit(1);
  }
  if ( sigaction( SIGBUS, &act, &oact ) < 0 )
  {
    cout << "Error catching SIGBUS signal." << std::endl;
    exit(1);
  }
  if ( sigaction( SIGILL, &act, &oact ) < 0 )
  {
    cout << "Error catching SIGILL signal." << std::endl;
    exit(1);
  }
  if ( sigaction( SIGABRT, &act, &oact ) < 0 )
  {
    cout << "Error catching SIGABRT signal." << std::endl;
    exit(1);
  }
  if ( sigaction( SIGUSR1, &act, &oact ) < 0 )
  {
    cout << "Error catching SIGUSR1 signal." << std::endl;
    exit(1);
  }
  if ( sigaction( SIGUSR2, &act, &oact ) < 0 )
  {
    cout << "Error catching SIGUSR2 signal." << std::endl;
    exit(1);
  }
  if ( sigaction( SIGTERM, &act, &oact ) < 0 )
  {
    cout << "Error catching SIGTERM signal." << std::endl;
    exit(1);
  }
  if ( sigaction( SIGCHLD, &act, &oact ) < 0 )
  {
    cout << "Error catching SIGCHLD signal." << std::endl;
    exit(1);
  }
  act.sa_flags = SA_RESETHAND ^ SA_NOMASK;
  if ( sigaction( SIGINT, &act, &oact ) < 0 )
  {
    cout << "Error catching SIGINT signal." << std::endl;
    exit(1);
  }
}
