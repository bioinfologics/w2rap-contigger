...
std::vector<String> TokenizeCommand(const String &sentence);
...
void CpAppend( String file1, std::ostream& file2 );
...
std::vector<String> AllFiles( String dirname );

std::vector<String> AllFilesByTime( String dirname );

std::vector<String> AllFilesWithPrefix( const String &prefix, const String &directory );

// Return the list of files that "echo" would produce.

std::vector<String> AllFilesInSource( String source );
...
/// Open FILENAME  to STREAMNAME with MODE. Truncate if !(MODE & std::ios::app).

#define OfstreamMode(STREAMNAME, FILENAME, MODE)               \
     std::ofstream STREAMNAME;                                      \
     OpenOfstream( STREAMNAME, #STREAMNAME, FILENAME, MODE );

/// PipeOstream: create an std::ostream STREAMNAME for output which is to be gzipped
...
#define PipeOstream(STREAMNAME, FILENAME)                                           \
     ...
     procbuf STREAMNAME ## _pipe( STREAMNAME ## _name_of_pipe.c_str( ), std::ios::out ); \
     std::ostream STREAMNAME( &STREAMNAME ## _pipe );

/// PipeIstream: create an std::istream STREAMNAME for input from the
...
#define PipeIstream(STREAMNAME, FILENAME)                                           \
     ...
     procbuf STREAMNAME ## _pipe( STREAMNAME ## _name_of_pipe.c_str( ), std::ios::in ); \
     std::istream STREAMNAME( &STREAMNAME ## _pipe );
...
#define PRINT3(X, Y, Z) std::cout << PRCORE(X) << ", " << PRCORE(Y) << ", " \
     << PRCORE(Z) << std::endl;
#define PRINT4(X, Y, Z, W) std::cout << PRCORE(X) << ", " << PRCORE(Y) << ", " \
     << PRCORE(Z) << ", " << PRCORE(W) << std::endl;
#define PRINT5(X, Y, Z, W, T) std::cout << PRCORE(X) << ", " << PRCORE(Y) << ", " \
     << PRCORE(Z) << ", " << PRCORE(W) << ", " << PRCORE(T) << std::endl;
#define PRINT6(X, Y, Z, W, T, U) std::cout << PRCORE(X) << ", " << PRCORE(Y) << ", " \
     << PRCORE(Z) << ", " << PRCORE(W) << ", " << PRCORE(T) << ", "             \
     << PRCORE(U) << std::endl;
#define PRINT7(X, Y, Z, W, T, U, V) std::cout << PRCORE(X) << ", " << PRCORE(Y) << ", " \
     << PRCORE(Z) << ", " << PRCORE(W) << ", " << PRCORE(T) << ", "             \
     << PRCORE(U) << ", " << PRCORE(V) << std::endl;
#define PRINT8(X, Y, Z, W, T, U, V, A) std::cout << PRCORE(X) << ", " << PRCORE(Y)  \
     << ", " << PRCORE(Z) << ", " << PRCORE(W) << ", " << PRCORE(T) << ", "  \
     << PRCORE(U) << ", " << PRCORE(V) << ", " << PRCORE(A) << std::endl;
#define PRINT9(X, Y, Z, W, T, U, V, A, B) std::cout << PRCORE(X) << ", " << PRCORE(Y)  \
     << ", " << PRCORE(Z) << ", " << PRCORE(W) << ", " << PRCORE(T) << ", "     \
     << PRCORE(U) << ", " << PRCORE(V) << ", " << PRCORE(A) << ", "               \
     << PRCORE(B) << std::endl;

#define PRINT_TO(O, X) O << PRCORE(X) << std::endl;
#define PRINT2_TO(O, X, Y) O << PRCORE(X) << ", " << PRCORE(Y) << std::endl;
...
#define DPRINT(X) std::cout << Date( ) << ": " << PRCORE(X) << std::endl;
#define DPRINT2(X, Y) std::cout << Date( ) << ": " << PRCORE(X) << ", " << PRCORE(Y) \
     << std::endl;
#define DPRINT3(X, Y, Z) std::cout << Date( ) << ": " << PRCORE(X) << ", " << PRCORE(Y) \
     << ", " << PRCORE(Z) << std::endl;
#define DPRINT4(X, Y, Z, W) std::cout << Date( ) << ": " << PRCORE(X) << ", " \
     << PRCORE(Y) << ", " << PRCORE(Z) << ", " << PRCORE(W) << std::endl;
#define DPRINT5(X, Y, Z, W, T) std::cout << Date( ) << ": " << PRCORE(X) << ", " \
     << PRCORE(Y) << ", " << PRCORE(Z) << ", " << PRCORE(W) << ", " << PRCORE(T) \
     << std::endl;
#define DPRINT6(X, Y, Z, W, T, U) std::cout << Date( ) << ": " << PRCORE(X) << ", " \
     << PRCORE(Y) << ", " << PRCORE(Z) << ", " << PRCORE(W) << ", " << PRCORE(T) \
     << ", " << PRCORE(U)  \
     << std::endl;
#define DPRINT7(X, Y, Z, W, T, U, V) std::cout << Date( ) << ": " << PRCORE(X) << ", " \
     << PRCORE(Y) << ", " << PRCORE(Z) << ", " << PRCORE(W) << ", " << PRCORE(T) \
     << ", " << PRCORE(U) << ", " << PRCORE(V)				\
     << std::endl;
#define DPRINT_TO(O, X) O << Date( ) << ": " << PRCORE(X) << std::endl;
#define DPRINT2_TO(O, X, Y) O << Date( ) << ": " << PRCORE(X) << ", " << PRCORE(Y) \
     << std::endl;
#define DPRINT3_TO(O, X, Y, Z) O << Date( ) << ": " << PRCORE(X) << ", " \
     << PRCORE(Y) << ", " << PRCORE(Z) << std::endl;
...
void Dot( std::ostream& log, unsigned int pass );
...
///MULTIWRITE( std::vector<std::ostream*>, stuff to be written )
///If a std::vector element is zero, nothing is written to the corresponding stream.
...
//
// Note that an std::istream operator for TYPE must exist!
//
// ==============================================================================
...
#define READ( FILE, TYPE, DATA )                                       \
     ...
     {    String f(FILE);                                              \
          if ( IsRegularFile(f) )                                      \
          {    std::ifstream temporary_read_stream( f.c_str( ) );           \
          ...
          {    String pipe_command = "gzip -dc " + f + ".gz";          \
               procbuf inp( pipe_command.c_str( ), std::ios::in );          \
               std::istream temporary_read_stream( &inp );                  \
               if ( !temporary_read_stream )                           \
...
{    Remove(FILE);                                                \
     ...
     procbuf outp( pipe_command.c_str( ), std::ios::out );             \
     std::ostream temporary_write_stream( &outp );                     \
     temporary_write_stream << DATA;                              \
     outp.close( );    }
...
#define PERCENT_RATIO( PRECISION, NUMERATOR, DENOMINATOR )                    \
   std::setprecision((NUMERATOR==DENOMINATOR && DENOMINATOR != 0) ? 3 : PRECISION) \
   << ( (0 == DENOMINATOR)                                                    \
...
#define PERCENT_RATIOB( WIDTH, PRECISION, NUMERATOR, DENOMINATOR )            \
   setiosflags(std::ios::fixed) << std::setprecision(PRECISION) << setw(WIDTH)          \
   << ( (0 == DENOMINATOR) ? std::numeric_limits<double>::infinity()               \
          : 100 * SafeQuotient(NUMERATOR, DENOMINATOR))                       \
...
void PlainFold( const String &any_word, std::ostream& out, int start=0, int stop=-1 );
...
void WhitespaceFold( const String &text, std::ostream& out, int width=70, String indent="" );
...
// out, every 100 calls, so long as "USE_TIMERS" is defined.
     ...
     #define START_TIMER( timer_name, freq )                              \
          ...
          {    std::cout << std::endl << "===== " << std::setprecision(3) << timer_name  \
...
// exit(0), because it will terminate without std::flushing buffers.
...
void RightPrecisionOut( std::ostream& out, const double x, int d );

/// Prints value <val> into output stream <out> using <sep> to separate
/// every third position (e.g. 9123456789 -> 9,123,456,789)

void PrintWithSep(std::ostream & out, unsigned int val, char sep=',');
...
int Glob( const String& x, std::vector<String>& xs );
