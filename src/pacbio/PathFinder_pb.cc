...
#include "PathFinder_pb.h"


std::string PathFinder_pb::edge_pstr(uint64_t e){
    return "e"+std::to_string(e)+"("+std::to_string(mHBV.EdgeObject(e).size())+"bp "+std::to_string(paths_per_kbp(e))+"ppk)";
};

void PathFinder_pb::init_prev_next_vectors(){
...
std::array<uint64_t,3> PathFinder_pb::transition_votes(uint64_t left_e,uint64_t right_e){
...
std::array<uint64_t,3> PathFinder_pb::path_votes(std::vector<uint64_t> path){
...
std::string PathFinder_pb::path_str(std::vector<uint64_t> path) {
    std::string s="[";
    for (auto p:path){
...
std::array<uint64_t,3> PathFinder_pb::multi_path_votes(std::vector<std::vector<uint64_t>> paths){
    //Returns a vote vector: { FOR, PARTIAL (reads end halfway, but validate at least one transition), AGAINST }
    //does this on forward and reverse paths, just in case
    std::vector<ReadPath> vfor,vpartial,vagainst;
    //TODO: needs to be done in both directions? (votes for only in one direction?)
...
void PathFinder_pb::classify_forks(){
...
void PathFinder_pb::unroll_loops(uint64_t min_side_sizes) {
...
/*
void PathFinder_pb::untangle_complex_in_out_choices() {
    //find a complex path
    init_prev_next_vectors();
    for (int e = 0; e < mHBV.EdgeObjectCount(); ++e) {
...
void PathFinder_pb::untangle_pins() {
...
void PathFinder_pb::untangle_complex_in_out_choices(uint64_t large_frontier_size, bool verbose_separation) {
    //find a complex path
...
std::vector<std::vector<uint64_t>> PathFinder_pb::AllPathsFromTo(std::vector<uint64_t> in_edges, std::vector<uint64_t> out_edges, uint64_t max_length) {
...
std::array<std::vector<uint64_t>,2> PathFinder_pb::get_all_long_frontiers(uint64_t e, uint64_t large_frontier_size){
...
std::vector<std::vector<uint64_t>> PathFinder_pb::is_unrollable_loop(uint64_t loop_e, uint64_t min_size_sizes){
...
uint64_t PathFinder_pb::paths_per_kbp(uint64_t e){
    return 1000 * mEdgeToPathIds[e].size()/mHBV.EdgeObject(e).size();
};

std::map<uint64_t,std::vector<uint64_t>> PathFinder_pb::separate_path(std::vector<uint64_t> p, bool verbose_separation){
...
void PathFinder_pb::migrate_readpaths(std::map<uint64_t,std::vector<uint64_t>> edgemap){
...
bool PathFinder_pb::join_edges_in_path(std::vector<uint64_t> p){
