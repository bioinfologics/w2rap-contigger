                          ...
                          const size_t       mem_mean_ceil) 
{
  ...
  for (size_t ibv = 0; ibv < n_reads; ibv++) {
    ...
    if (pcp.i_reads_print.count(ibv) > 0) {
      std::cout << std::setw(12) << ibv << std::endl;
      std::cout << hieroglyphs(bv) << std::endl;
      for (size_t ib = 0; ib < nb; ib++) 
        std::cout << ((bv[ib] != bv_new[ib]) ? hieroglyph(bv_new[ib]) : ToString(" "));
      std::cout << std::endl;     
    }
    ...
    if (pcp.i_reads_print.count(ibv) > 0) 
      std::cout << hieroglyphs(bv) << std::endl << std::endl;
  }
...
class CounterMap : public std::map<size_t, size_t>
{
public:
  size_t total() const 
  {
  ...
  void to_text_file(const String & filename) const
  {
    ...
    for (CounterMap::const_iterator it = (*this).begin(); 
      ...
      ofs << " " << it->first
          << " " << it->second
          << " " << std::setprecision(10) << static_cast<double>(it->second) / tot
          << " " << cum
          << " " << std::setprecision(10) << static_cast<double>(cum) / tot
          << std::endl;
    }
    ofs.close();
  }
