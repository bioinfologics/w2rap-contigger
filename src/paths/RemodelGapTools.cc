     ...
     vec<int>& true_gap, vec<Bool>& true_gap_computed )
{
     ...
     if (TIME_STAMPS) std::cout << Date( ) << ": computing gkmers_plus" << std::endl;
     const int L1 = 100;
     vec< triple<kmer<L1>,int,int> > gkmers_plus;
     MakeKmerLookup( genome, gkmers_plus );
     if (TIME_STAMPS) std::cout << Date( ) << ": forming gkmers" << std::endl;
     vec< kmer<L1> > gkmers( gkmers_plus.size( ) );
     for ( size_t i = 0; i < gkmers.size( ); i++ )
          gkmers[i] = gkmers_plus[i].first;
     if (TIME_STAMPS) std::cout << Date( ) << ": finding true gaps" << std::endl;
     int gap_id = -1;
     for ( int s = 0; s < scaffolds.isize( ); s++ )
     ...
     std::cout << Date( ) << ": found true size for " << Sum(true_gap_computed)
          << " gaps of " << true_gap_computed.size( ) << std::endl;    }

void PrintSideBySide( const String& z1, const String& z2, const int N )
{    std::istringstream zin1(z1), zin2(z2);
     String line1, line2;
     while(1)
     {    getline( zin1, line1 );
          getline( zin2, line2 );
          ...
          {    for ( int i = 0; i < N; i++ )
                    std::cout << " ";
               std::cout << line2 << "\n";    }
          else if ( !zin1.fail( ) && zin2.fail( ) ) std::cout << line1 << "\n";
          else break;    }    }
     ...
     vec<Bool>& placed_rc, vec< std::pair<int,int> >& placement, const Bool TIME_STAMPS )
{
     ...
     if (TIME_STAMPS) std::cout << Date( ) << ": forming read alignments" << std::endl;
     #pragma omp parallel for
     ...
     const Bool TIME_STAMPS )
{
     ...
     {    if ( K2 == 20 )
          ...
          {    std::cout << "AlignReads not implemented for K2 = " << K2 << std::endl;
               exit(1);    }    }
     if (TIME_STAMPS) std::cout << Date( ) << ": indexing alignments" << std::endl;
...
void DefineDistributions(
     ...
     vec<Bool>& placed_fw, vec<Bool>& placed_rc, vec< std::pair<int,int> >& placement,
     ...
     vec< vec<double> >& X )
{
     ...
     std::cout << Date( ) << ": "; PRINT(min_to_use);
     ...
     std::cout << Date( ) << ": maximum contig size = "
          << max_tig << std::endl;
     ...
     if (TIME_STAMPS) std::cout << Date( ) << ": computing library distributions" << std::endl;
     ...
     {    for ( int l = 0; l < nlibs; l++ )
          ...
          {    if ( tigs[m].isize( ) < min_to_use ) continue;
               ...
               {    longlong pid = pids[pi];
                    ...
                    if ( !placed_fw[id1] ) std::swap( id1, id2 );
          ...
          if ( pass == 2 ) {
              std::cout << "Number of links after pass 2 : " << std::endl;
              for ( int i = 0; i < nlibs; i++ ) {
                  String flag = ( lfail[i] ? "True" : "False" );
                  std::cout << "Lib " << i << " " << DISTS[i].size() << " lfail= " << flag <<  std::endl;
                  if ( DISTS[i].empty() && ! lfail[i] ) {
                      std::cout << "    Disable lib " << i << " since no links are found in pass 2 " << std::endl;
                      lfail[i] = True;
                  } 
              }
          }
     ...
     {    if ( !lfail[l] )
          ...
          std::cout << Date( ) << ": library " << l << ", ";
          if ( lfail[l] ) std::cout << "can't find upper bound" << std::endl;
          else
          {    std::cout << lib_mean[l] << " +/- " << lib_dev[l]
     ...
     const int fails, const vec<int>& tigs_to_process )
{
     ...
     {    std::cout << "\ngaps:\n" << devs.size( ) << " + " << fails << " fails"
               << "\n+ " << unassayed << " unassayed\n";
          std::cout << "No gaps computed, report ends here." << std::endl;
          return;    }
     std::ostringstream outw1, outw2;
     outw1 << "\ngaps:\n" << devs.size( ) << " + " << fails << " fails"
           << "\n+ " << unassayed << " unassayed\n";
     ...
     {    int count = 0, acount = 0, aacount = 0;
          ...
          out1 << std::setiosflags(std::ios::fixed) << std::setprecision(1)
               << 100.0 * double(count) / double( offbys.size( ) );
          out2 << std::setiosflags(std::ios::fixed) << std::setprecision(1)
               << 100.0 * double(acount) / double( aoffbys.size( ) );
          out3 << std::setiosflags(std::ios::fixed) << std::setprecision(1)
     ...
     int& nlibs, const int VERBOSITY, const Bool TIME_STAMPS )
{
     ...
     {    for ( int l = 0; l < nlibs_this; l++ )
          {    std::cout << Date( ) << ": library " << nlibs + l << " is of type "
     ...
     const int VERBOSITY, vec<int>& accepted_overlaps, int& max_overlap )
{
     ...
     if ( VERBOSITY >= 2 ) 
     {    std::cout << "accepted overlaps:\n";
          for ( int j = 0; j < accepted_overlaps.isize( ); j++ )
               std::cout << accepted_overlaps[j] << "\n";    }    }
