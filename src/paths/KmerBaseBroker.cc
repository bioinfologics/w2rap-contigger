...
const basevector& KmerBaseBrokerTemplate<TAG>::Bases(kmer_id_t k) const {
  ...
  if( ! kmer_bases.Initialized() ) {
    ...
    if( match_index == -1 ) {
      std::cout << "Error: Bases(" << k << "): no such kmer in database!" << std::endl;
      return kmer_bases;
    }
    ...
    if( verbose ) 
      std::cout << "kmer " << k << " from "
	   << (rc ? "rc of read " : "read ") << read_id
	   << "(len=" << bases[read_id].size() << ")"
	   << ", bases " << offset << "-" << offset+K-1 << std::endl;
    
    if( offset+K-1 > (int)bases[read_id].size() ) {
      std::cout << "Error: not enough bases to match path!\n";
      PRINT(read_id);
      PRINT(paths[read_id]);
      std::cout << "bases[read_id] = ";
      bases[read_id].Print(std::cout);
      std::cout << std::endl;
      return kmer_bases;
    }
  ...
  for(kmer_id_t k = rpi.Start(); k <= rpi.Stop(); ) {
    ...
    if( match_index == -1 ) {
      std::cout << "\nERROR: KmerPathInterval " << rpi
	   << " contains kmer " << k
	   << ", not from any path!" << std::endl;
      std::cout << "*** Make sure you use the correct run directory.\n" <<
	...
	" that were not in the reads.pathsdb.kN in the original run directory." << std::endl;
    ...
    kmer_id_t kstop = std::min( rpi.Stop(), match.Stop() );
	...
	std::cout << "\nERROR: bad overlap of kmers " << k-1 << " and " << k << std::endl;
      std::cout << "*** Make sure you use the correct run directory.\n" <<
	" For assemblies it is under ASSEMBLIES/assemblyName/run.\n" <<
	" The reads.pathsdb.kN kmer database there has additional kmers\n" <<
	" (added to represent merges of reads overlapping by <K)\n" <<
...
int KmerBaseBrokerTemplate<TAG>::MinOffset(kmer_id_t k1, kmer_id_t k2, int d_min) const {
  ...
  std::pair< typename std::map<cache_key,int>::iterator,bool > lookup =
    min_offset_cache.insert( std::make_pair(cache_key(k1, k2, d_min),K) );
...
int KmerBaseBrokerTemplate<TAG>::MaxOffset(kmer_id_t k1, kmer_id_t k2, int d_max) const {
  ...
  std::pair< typename std::map<cache_key,int>::iterator,bool > lookup =
    max_offset_cache.insert( std::make_pair(cache_key(k1, k2, d_max),-1) );
  ...
  for( int seg = 0; seg < path.NSegments(); seg++ ) {
	...
	if( current_seq_str.substr( current_seq_str.size()-K+1, K-1) !=
	  ...
	  std::cout << "\nERROR: bad overlap of adjacent intervals " 
	       << path.Segment(seg-1) << path.Segment(seg) << std::endl;
...
bool KmerBaseBrokerTemplate<TAG>::KmersBetween( kmer_id_t k1, 
  ...
  std::pair< typename std::map<cache_key,KmerPath>::iterator,bool > lookup =
    between_cache.insert( std::make_pair(cache_key(k1, k2, gapsize),KmerPath()) );
