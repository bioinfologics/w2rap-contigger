...
void NotOne( const int K, const vecbasevector& unibases, const vec<int>& to_rc,
    ...
    for ( int u = 0; u < (int) unibases.size( ); u++ ) {
        ...
        if ( RAISE_VERBOSE && not_one[u] ) {
            std::cout << u << " not of copy number one?\n";
            std::cout << to_rc[u] << " not of copy number one?\n";
        }
    }
}

template<int K2> void ToBig( const vecbasevector& unibases1,
                             const vecbasevector& unibases2, vec< vec< std::pair<int,int> > >& to_big ) {
    to_big.resize( unibases1.size( ) );
    std::cout << Date( ) << ": getting unibase2 kmers" << std::endl;
    vec<int64_t> starts;
    starts.push_back(0);
    ...
    std::cout << Date( ) << ": copying" << std::endl;
    vec< kmer<K2> > kmers( kmers_plus.size( ) );
    for ( size_t i = 0; i < kmers.size( ); i++ )
        kmers[i] = kmers_plus[i].first;
    std::cout << Date( ) << ": mapping from 1 to 2" << std::endl;
    ...
    for ( size_t i = 0; i < unibases1.size( ); i++ ) {
        ...
        if ( locsx.nonempty( ) ) {
            ...
            // {    std::cout << "(" << locsx[j].first << ") " << locsx[j].second
            //           << "." << locsx[j].third << "\n";    }
            ...
            if ( locsx[0].third < 0 ) {
                #pragma omp critical
                {
                    std::cout << "WARNING: negative placement of " << i
                    << std::endl;
                }
            }
    ...
    for ( int u = 0; u < (int) unibases.size( ); u++ ) {
        ...
        for ( int i1 = 0; i1 < G.From(u).isize( ); i1++ )
            for ( int i2 = 0; i2 < G.From(u).isize( ); i2++ ) {
                ...
                if (DELETE_VERBOSE) {
                    std::cout << "method A1, deleting link from " << u << " to "
                              << v2 << "\n";
                    std::cout << "method A1, deleting link from " << rv2 << " to "
                              << ru << "\n";
                }
    ...
    for ( int u = 0; u < (int) unibases.size( ); u++ ) {
        for ( int j = 0; j < G.From(u).isize( ); j++ ) {
            ...
            if ( not_one[u] || not_one[v] ) {
                if (DELETE_VERBOSE) {
                    std::cout << "method B, deleting link from " << u
                              << " to " << v << "\n";
                }
                to_delete.push_back(
            ...
            for ( int k = 0; k < G.From(v).isize( ); k++ ) {
                if ( G.From(v)[k] == u ) {
                    ...
                    if ( nlinks2 >= max_link_ratio * nlinks1 ) {
                        if (DELETE_VERBOSE) {
                            std::cout << "method C, deleting links from "
                                      << u << " to " << v << "\n";
                        }
                        to_delete.push_back(
                            G.EdgeObjectIndexByIndexFrom( u, j ) );
    ...
    for ( int u = 0; u < (int) unibases.size( ); u++ ) {
        ...
        for ( int j = 0; j < G.From(u).isize( ); j++ ) {
            ...
            if ( nlinks_max >= total_ignore_ratio * nlinks ) {
                if (DELETE_VERBOSE) {
                    std::cout << "method D, deleting links from " << u << " to "
                              << G.From(u)[j] << "\n";
                }
                to_delete.push_back(
                    G.EdgeObjectIndexByIndexFrom( u, j ) );
...
void RemoveWeakLinks2( const int K2, const vecbasevector& unibases2,
                       ...
                       const vec< vec< std::pair<int,int> > >& nexts2x, const int max_link_ratio,
        ...
        for ( int j = 0; j < G2I.From(u1).isize( ); j++ ) {
            ...
            if ( l.nlinks >= min_links && cn1 >= min_cn_mult * cn2
                ...
                {    std::cout << "\n";
                     ...
                     std::cout << u1 << " not of copy number one\n";    }
                */
                not_one2[u1] = True;
            }
        }
    ...
    for ( int u1 = 0; u1 < nuni2; u1++ ) {
        ...
        for ( int j = 0; j < G2I.From(u1).isize( ); j++ ) {
            ...
            /*
            if (DELETE_VERBOSE)
            {    std::cout << "method X, deleting link from " << u1
                      << " to " << G2I.From(u1)[j] << "\n";    }
            */
            int u2 = G2I.From(u1)[j];
...
void RaiseLinks( const int K2, const vecbasevector& unibases1,
                 ...
                 const vec< vec< std::pair<int,int> > >& to_big, const digraphE<linklet>& G,
                 digraphE<linklet>& G2 ) {
...
void Advance(
    ...
    for ( int m = 0; m < linksx.isize( ); m++ ) {
        ...
        out << "-- see " << v << "[" << unibases2[v].isize( ) - K2 + 1 << ",cn="
            << std::setiosflags(std::ios::fixed) << std::setprecision(2) << raw2[v]
    ...
    for ( int m1 = 0; m1 < linksx.isize( ); m1++ )
        for ( int m2 = m1 + 1; m2 < linksx.isize( ); m2++ ) {
            ...
            if ( Member( G.From(v2), v1 ) && !Member( G.From(v1), v2 ) ) {
                std::swap( linksx[m1], linksx[m2] );
                std::swap( linksy[m1], linksy[m2] );
