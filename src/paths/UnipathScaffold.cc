     ...
     vec<Bool>& circled ) 
{
     ...
     if (SCAFFOLDING_VERBOSE) std::cout << "\nINITIAL SCAFFOLDS\n";
     ...
     {    int u = len_uni[ui].second;
          ...
          {    if ( G.To(z).empty( ) ) break;
               vec< std::pair<int,int> > links_uni;
          ...
          {    vec< triple<int,int,int> > links_uni;
               ...
               {    int u2 = G.From(v)[j];
                    if (SCAFFOLDING_VERBOSE) std::cout << "- see " << u2 << "\n";
                    ...
                    if ( used[u2] && u2 != u ) 
                    {    if (SCAFFOLDING_VERBOSE)
                              std::cout << "- used and not at beginning, rejecting\n";
                         continue;    }
                    ...
                    {    if (SCAFFOLDING_VERBOSE)
                         {    std::cout << "- have link to its reverse complement too, "
                                   << "rejecting\n";    }
                         continue;    }
			...
			vec<Bool>& circled ) 
{
  ...
  std::cout << "Initially: "; PRINT( G.Edges().size() );
  ...
  std::cout << Date() << ": initial number of non-trivial ends: sources= " << sources.size() 
       << " sinks= " << sinks.size() << std::endl;
  vec<int> toRemove;
  ...
  std::cout << Date() << ": final number of non-trivial ends: sources= " << sources.size() 
       << " sinks= " << sinks.size() << std::endl;
  
  // create dots of affected components
  
  std::cout << Date() << ": writing dot files" << std::endl;
  String dot_dir = "dots";
  Mkdir777( dot_dir );
  {
    vec<double> tmp( G.N(), 1.0 );
    String fname1 = dot_dir + "/graph_o.dot";
    std::ofstream gout( fname1.c_str() );
  ...
  std::cout << Date() << ": building initial scaffolds" << std::endl;
  if (SCAFFOLDING_VERBOSE) std::cout << "\nINITIAL SCAFFOLDS\n";
  for ( int ui = 0; ui < len_uni.isize( ); ui++ ){    
  //for ( int ui = len_uni.isize( ) -1; ui >= 0;  ui-- ){    
    ...
    if ( ! FoundSelf ){
      std::cout << "WARNING: starting unipath " << u0 << " not found in corresponding scaffold!!!!!!!\n";
    }
