...
void parse_genotype_fields( VCFChromosome::VCFLine& currentLine ) {
    ...
    for ( size_t i = 0; i < nSamples; ++i ) {		// for each sample
        ...
        currentLine.GENOTYPE_FIELDS.push_back(std::map<string,string>());
    ...
    for ( size_t i = 0; i < nSamples; ++i ) {
        ...
        std::istringstream s(currentLine.GENOTYPE_FIELDS[i]["GT"]);
        ...
        if ( s.peek() != '.' ) {
            s >> idx;
            ForceAssertLe(idx, std::numeric_limits<char>::max());
        } else {
            s.ignore();
            idx = -1;
        }
        ...
        if ( ( ph = s.get() ) > 0 && !s.fail()  ) {
            ...
            if (ph == '|')
                ...
                std::cout << "about to die: currentLine sample i="  << i << " GT=[" << currentLine.GENOTYPE_FIELDS[i]["GT"] << "] genotype=[" << currentLine.GENOTYPE[i] << "]"<< std::endl;
...
void
VCFChromosome::VCFLine::Write( const std::string&sChrom, std::ostream& os ) const {
    os << sChrom << '\t'
       << POS1 << '\t'
       << ID << '\t'
       << REF.ToString() << '\t';
...
void
VCFChromosome::VCFLine::Print( std::ostream& os ) const {
    os << "POS1 " << POS1 << std::endl;
    os << "ID   " << ID << std::endl;
    os << "REF  " << REF.ToString() << std::endl;
    os << "ALT (" << ALT.size() << ")" << std::endl;
    for ( size_t i = 0; i < ALT.size(); ++i )
        if ( ALT[i].isBaseVec() )
            std::cout << "\tALT " << i << " " << ALT[i].bRep.ToString() << std::endl;
        else
            std::cout << "\tALT " << i << " " << ALT[i].sRep << std::endl;
    os << "FILTER " << FILTER << std::endl;
    os << "INFO   " << INFO << std::endl;
    os << "GENOTYPE_FORMAT " << GENOTYPE_FORMAT << std::endl;
    os << "GENOTYPE_FIELDS.size() " << GENOTYPE_FIELDS.size() << std::endl;
    for ( size_t i = 0; i < GENOTYPE_FIELDS.size(); ++i ) {

        for ( auto j = GENOTYPE_FIELDS[i].begin(); j != GENOTYPE_FIELDS[i].end(); ++j )
            std::cout << "\tGENOTYPE_FIELDS " << i << " [" << (j->first) << "] "  << (j->second) << std::endl;

        std::cout << "\tGT " << i << " " << GT[i] << std::endl;
        for ( size_t j = 0; j < GT_IDX[i].size(); ++j )
            std::cout << "\t\tGT_IDX " << i << " = " << static_cast<int>(GT_IDX[i][j]) << std::endl;
        std::cout << "\t\tGT_PHASED " << ( GT_PHASED[i] ? "true" : "false") << std::endl;
...

{
    ...
    if ( sDump != "" ) OpenOfstream(outfile, sDump, sDump, std::ios::trunc);
    ...
    */
    ...
    // std::cout << "bSelectRange=" << (bSelectRange ? "true":"false") << ", lBegin1=" << lBegin1 << ", lEnd1=" << lEnd1 << std::endl;
    std::string sTmp;
    size_t nDataLine=0;
    ...
    if ( sChromosome != "" && !bFoundSelection ) {
        ...
        s << "You specified chromosome " << sChromosome <<
          ...
          << std::endl <<
          "your VCF uses chr1, chr2, chr3..., then you can set VCF_CHR_PREFIX=chr" << std::endl;
        FatalErr( s.str() );
    }
