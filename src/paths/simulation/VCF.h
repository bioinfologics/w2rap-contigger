...
private:
    ...
    friend std::ostream& operator<<(std::ostream&,const VCFWriter&);

};
std::ostream& operator<<(std::ostream&,const VCFWriter&);
...
public:
  ...
  struct VCFLine {
    ...
    vector<std::map<string, string> > GENOTYPE_FIELDS;	// e.g. GT->0|0, AD->139,2, DP->141, GQ->15, PL->0,15,196, TP->15
    vector<string> 		 GT;
...
public:
  ...
  VCF( const std::string& filename
     ...
     , size_t lBegin1=1
     , size_t lEnd1=std::numeric_limits<size_t>::max()
     , const string& sDump = "");
...
private:
  ...
  static void dumpLine( const string& filename, std::ofstream& stDump, const string& line ) {
      if ( stDump.is_open() ) {
	  stDump << line << std::endl;
