                                 ...
                                 const bool            VERBOSE)
{
  ...
  if (must_sort) {
    id_sid.resize(n_bvs);
    {
      vec< std::pair<int, size_t> > lengths(n_bvs);
      for (size_t id = 0; id < n_bvs; id++)
        lengths[id] = std::make_pair(bvv[id].size(), id);
      ReverseSort(lengths);
  ...
  for (size_t id2 = 0; id2 < n_bvs; id2++) {
    ...
    if (VERBOSE) {
      if ((n_bvs >= 1e6 && id2 % (n_bvs / 20) == 0) ||
          n_bvs < 20) {
        std::cout << "id2 = " << id2 << " (" << (100.0 * id2) / n_bvs << " %)  ih01 = " << ih0 << " " << ih1 << std::endl;
      }
    }     
    ...
    for (size_t ik = 0; ik < nk2; ik++) {
      if (knums[ik] < 0) PRINT3_TO(std::cout, id2, ik, nb2);
