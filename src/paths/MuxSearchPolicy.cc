...
MSP_FateOfClosure::PushAndGiveOpinion( const Mux& final_mux, 
  ...
  if( IsPerfectMatch( CurrentPath().End(), m_closure.End() ) ) {
    ...
    if( m_closure == CurrentPath() ) {
      cout << "Found the closure!" << std::endl;
      return GOOD;
    }
	...
	if( m_verbose ) {
	  cout << "  So far so good: mux " << final_mux << "->" << *mux 
	       << " extending " << CurrentPath() << std::endl;
	}
	return GOOD;
      }
    }
    ...
    cout << "\nNo muxes match closure!" << std::endl;
  }
...
MSP_ReachCloserOrAbort::PushAndGiveOpinion( const Mux& final_mux, 
					    const MuxSearchState& search,
					    const vec<Bool>& is_good ) {
//  cout << "PAGO called on " << final_mux << std::endl;
...
//  cout << "GetOffsetsTo returns " << can_get_here.size() << " offsets" << std::endl;
...
//  cout << "None of them are helpful" << std::endl;
...
// 	 << " in " << io.GetAmount() << " kmers]" << std::endl;
//     return true;
//   }
//   else {
//     cout << "Endpoint [read " << goal.path_id 
