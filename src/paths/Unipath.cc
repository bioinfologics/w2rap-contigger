...
struct uinterval {
    ...
    friend std::ostream& operator<<(std::ostream& out, const uinterval u) {
...
int BuildUnipathIntervals( const vec<TAGGED_RPINT>& pathsdb,
    ...
    while( db_start != pathsdb.end() ) {  // actually the loop always exits via a break
        ...
        if(verbose)
            std::cout << "Starting with kmer " << current_kmer << "...  " << std::endl;
        ...
        if(verbose)
            std::cout << "    unipath interval is now [" << current_kmer
                      << "-" << ending_kmer << "]" << std::endl;
        ...
        for( db_iter = db_start;
            ...
            max_stop = std::max( max_stop, my_stop );

            if(verbose)
                std::cout << "  Checking tagged_rpint [" << db_iter->Start()
        ...
        if(verbose)
            std::cout << "  Found unipath interval " << uni_intervals.back().kpi
                      << ", prev_seg_kmer=" << prev_seg_kmer
                      << ", next_seg_kmer=" << next_seg_kmer << std::endl;
...
void BuildUnipathsFromIntervals( vec<uinterval>& uni_intervals,
    ...
    for( start_iter = uni_intervals.begin(); start_iter != uni_intervals.end();
        ...
        if(verbose)
            std::cout << "Unipath " << unipaths.size()
                      << ": starting from unipath interval " << *start_iter << std::endl;
        ...
        for( random_iter = start_iter;
            ...
            if(verbose)
                std::cout << "kpis_forward gets " << *random_iter << "\n";
        ...
        for( random_iter = PrevUI( start_iter, uni_intervals );
            ...
            if(verbose)
                std::cout << "kpis_backward gets " << *random_iter << "\n";
        ...
        if ( is_loop ) {
            ...
            if(verbose)
                std::cout << "Final path: " << this_uni << std::endl;
        }
...
void Unipath( const vecKmerPath& paths, const vecKmerPath& paths_rc,
    ...
    if ( paths.empty() ) {
        ...
        if (log_progress)
            std::cout << Date( ) << ": starting unipath construction" << std::endl;
        ...
        if (log_progress)
            std::cout << "\n" << Date( )
                      << ": Step 1 (building next_read_kmer, prev_read_kmer)" << std::endl;
        ...
        if (log_progress)
            std::cout << "\n" << Date( )
                      << ": Step 2 (building unipath intervals)" << std::endl;
        ...
        if (log_progress)
            std::cout << "\n" << Date( )
                      << ": Step 3 (stringing together unipaths)" << std::endl;
        ...
        if (log_progress) std::cout << Date( ) << ": post-processing" << std::endl;
...
void DecomposePath( const KmerPath& x, Bool brief ) {
    ...
    while(1) {
        ...
        if ( seg > 0 || kmer > 0 ) std::cout << ".";
        std::cout << BaseAlpha(id);
        int nkmers = unipaths[id].KmerCount( ) - lead;
        if ( lead > 0 ) std::cout << "[" << nkmers << "]";
        lead = 0;
    ...
    if ( !brief ) {
        for ( size_t i = 0; i < unipaths.size( ); i++ ) {
            std::cout << BaseAlpha(i) << ": (" << unipaths[i].KmerCount( )
                      << " kmers)\n";
            std::cout << unipaths[i] << "\n";
        }
    } else {
