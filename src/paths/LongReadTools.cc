...
void GetLocalAligns( const basevector& r, const vecbasevector& U,
                     const vec< vec< std::pair<int,int> > >& Ulocs, const int L, const int flank,
                     const int max_errs, vec< triple<int,int,int> >& aligns, std::ostream& out,
...
void GetGlobalAligns( const basevector& r, const vecbasevector& U,
    ...
    for ( int i = 0; i < aligns.isize( ); i++ ) {
        ...
        SmithWatBandedA2<unsigned int>( U[u], r, offset, bandwidth,
                                        aligns_a[i], errors, (std::ostream*) 0, sub, ins, del );
    }
}

void MakeAlignmentGraph( const vec< std::pair< int, vec< std::pair<int,int> > > >& alignsx,
                         const vecbasevector& U, const int verbosity, std::ostream& rout, digraphE<int>& G ) {
    int N = alignsx.size( );
    ...
    Sort(flat);
    vec< vec< std::pair<int,int> > > offsets(N);
...
void AlignToRef( const vecbasevector& B, const String& run_dir,
                 const String& data_dir, std::ostream& rout, const String& module ) {
...
void ConvertPathIntoBases( const int L, const digraphE<int>& G, const vec<int>& p,
                           vec< std::pair< int, vec< std::pair<int,int> > > >& alignsx, const vecbasevector& U,
                           triple< basevector, int, int >& B ) {
    vec<int> to_left, to_right;
    G.ToLeft(to_left), G.ToRight(to_right);
    const std::pair< int, vec< std::pair<int,int> > >& a1 = alignsx[ to_left[ p.front( ) ] ];
    const std::pair< int, vec< std::pair<int,int> > >& a2 = alignsx[ to_right[ p.back( ) ] ];
...
Bool ConvertPathIntoBasesValid( const int L, const digraphE<int>& G,
                                const vec<int>& p, vec< std::pair< int, vec< std::pair<int,int> > > >& alignsx,
...
void ConvertPathsIntoBases( const int L, const basevector& target,
                            ...
                            const vec<int>& vpaths, vec< std::pair< int, vec< std::pair<int,int> > > >& alignsx,
                            const vecbasevector& U, vec<basevector>& bpaths, const int verbosity,
                            std::ostream& rout, const String& run_dir, const String& data_dir,
    ...
    for ( int i = 0; i < vpaths.isize( ); i++ ) {
        triple< basevector, int, int > B;
        const std::pair< int, vec< std::pair<int,int> > >& a = alignsx[ vpaths[i] ];
        int u = a.first;
...
vec<placementx> FindGenomicPlacements( basevector b, const int L,
                                       const vecbasevector& genome, const vec< vec< std::pair<int,int> > >& Glocs ) {
...
void indexes_gaps_for_parallel(const vec<vec<GapPatcher> >& patchers_gap,
    ...
    if (verbose) {
        for (unsigned ig = 0; ig != n_gaps; ig++) {
            std::cout << "ig_sorted= " << std::setw(3) << ig
                      << " ig= " << (*p_is_gaps)[ig]
                      << " time= " << std::setw(10) << time[ig]
                      << std::endl;
        }
    }
}
...
void patcher_short_print(const int i_gap,
    ...
    std::cout  << "i_gap= " << i_gap
               << "  upos1= " << std::setw(6) << upos1
               << "  bv1.size()= " << std::setw(8) << nb1
               << "  upos2= " << std::setw(6) << upos2
               << "  bv2.size()= " << std::setw(8) << nb2
               << "  r.size()= " << std::setw(8) << nbp
               << "  gap= " << std::setw(6) << gap
               << std::endl;
    ...
    std::cout << "i_gap= " << i_gap << " ... ";
    for (uint i = 0; i < nb1_left; i++) std::cout << as_base(bv1[upos1 - nb1_left + i]);
    std::cout << " ";
    for (uint i = 0; i < nb1_right; i++) std::cout << as_base(bv1[upos1 + i]);
    std::cout << " ... gap = " << std::setw(4) << gap << " ... ";
    for (uint i = 0; i < nb2_left; i++) std::cout << as_base(bv2[upos2 - nb2_left + i]);
    std::cout << " ";
    for (uint i = 0; i < nb2_right; i++) std::cout << as_base(bv2[upos2 + i]);
    std::cout << std::endl;
    ...
    std::cout << "i_gap= " << i_gap << "     ";
    for (uint i = 0; i < nb1_left; i++) std::cout << " ";
    std::cout << " ";
    for (uint i = 0; i < nbp_right; i++) std::cout << as_base(p_opt.r[i]);
    for (uint i = nbp_right; i < nb1_right; i++) std::cout << "_";
    std::cout << " .................. ";
    for (uint i = nbp_left; i < nb2_left; i++) std::cout << "_";
    for (uint i = 0; i < nbp_left; i++) std::cout << as_base(p_opt.r[nbp - nbp_left + i]);
    std::cout << " ";
    for (uint i = 0; i < nb1_right; i++) std::cout << " ";
    std::cout << std::endl;
}
...
void CleanPatch(
    ...
    const vec< vec< std::pair<int,int> > >& Ulocs,
    ...
    if ( (int) left.size( ) < K || (int) right.size( ) < K ) {
        if ( verbosity >= 1 ) outx << "flanks too short" << std::endl;
        return;
    }
    ...
    if ( F.empty( ) ) {
        if ( verbosity >= 1 )
            outx << "failed to find flanking kmer, giving up" << std::endl;
        return;
    }
    ...
    for ( int fi = 0; fi < F.isize( ); fi++ ) {
        ...
        if ( verbosity >= 1 ) {
            outx << Date( ) << ": flank " << fi+1
                 << ", making alignment graph" << std::endl;
        }
        ...
        if ( verbosity >= 1 ) outx << Date( ) << ": creating edgepaths" << std::endl;
        Bool OK = G.EdgePaths(START, STOP, epaths, -1, max_paths, max_iterations);
        if ( !OK ) {
            if ( verbosity >= 1 )
                outx << "edge path computation exploded" << std::endl;
            break;
        }
        if ( verbosity >= 1 )
            outx << Date( ) << ": found " << epaths.size( ) << " paths" << std::endl;
        Bool exploded = False;
        if ( epaths.empty( ) ) {
            if ( verbosity >= 1 ) {
                outx << "no complete paths, looking for "
                     << "incomplete ones" << std::endl;
            }
            for ( int v = 0; v < G.N( ); v++ )
    ...
    if ( !exploded && best_bpaths.nonempty( ) ) {
        ...
        if (same_size) {
            ...
            if ( X.AmbCount( ) == 0 ) {
                if ( verbosity >= 1 ) outx << "saving patch" << std::endl;
...
template void CleanPatch<96>(
    const int L, const vecbasevector& U, const vec< vec< std::pair<int,int> > >& Ulocs,
    const vec< triple<kmer<96>,int,int> >& kmers_plus, const fastavector& LEFT,
    const fastavector& RIGHT, assembly_edit& e, std::ostringstream& outx,
...
template< int K > void GetFlankData( const fastavector& left,
                                     ...
                                     const vec< triple<kmer<K>,int,int> >& kmers_plus,
                                     vec<flank_datum>& F, const int verbosity, std::ostream& rout,
...
void AlignToTarget(
    ...
    /* logging */
    const int verbosity, std::ostream& rout
) {
    vec< triple<int,int,int> > aligns;
    vec<double> mismatch_rates;
    alignment al;
    int best_loc;
    std::set< triple<int,int,int> > Mused;
    ...
    if ( verbosity >= 1 ) {
        for ( int j = 0; j < aligns.isize( ); j++ ) {
            rout << "unipath " << aligns[j].third << " aligned "
                 ...
                 << target.size( ) << ", error rate = " << std::setprecision(3)
                 << 100.0 * mismatch_rates[j] << "% " << std::endl;
        }
    }
    ...
    vec<int> reps, o;
    E.OrbitReps(reps);
    vec< std::pair< int, vec< std::pair<int,int> > > > alignsy;
    ...
    std::pair< int, vec< std::pair<int,int> > > STARTA, STOPA;
    ...
    for ( int i = 0; i < alignsx.isize( ); i++ ) {
        const vec< std::pair<int,int> >& M = alignsx[i].second;
        Bool more = False;
        for ( int j = 0; j < M.isize( ); j++ ) {
...
void PickBestPath( const vec<basevector>& bpaths, const basevector& target,
                   basevector& bpath, const int verbosity, std::ostream& rout, const String& run_dir,
                   const String& data_dir ) {
    alignment al;
    int best_loc;
