...
std::vector<std::vector<uint64_t>> PathFinder::find_all_solution_paths(std::vector<std::vector<uint64_t>> pair_solutions){
  ...
  for (auto pn=0; pn < pair_solutions.size(); ++pn){
    auto p_in = pair_solutions[pn][0];
    auto p_out = pair_solutions[pn][1];
    ...
    int cont = 0;
    auto status = find_all_pair_conecting_paths(p_in, path, cont, p_out, 0);
...
bool PathFinder::find_all_pair_conecting_paths(uint64_t edge_name , std::vector<uint64_t> path, int cont, uint64_t end_edge, int maxloop = 0) {
