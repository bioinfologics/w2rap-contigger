...
class FriendAlignFinderNaif : public FriendAlignerImpl {
  ...
  private:
    ...
    static void alignsToFriends( String const& input, String const& output,
        ...
        fr.read( &aligns[0], sz );
        fr.close();
        std::cout << Date() << ": done reading." << std::endl;
        unlink(input.c_str());
        std::cout << Date() << ": populating reverse Alignments" << std::endl;
        ...
        std::cout << Date() << ": about to parallel sort " << aligns.size() << " aligns" << std::endl;
        ParallelSort(aligns);
        std::cout << Date() << ": before unique-erase " << aligns.size() << " aligns" << std::endl;
        aligns.erase(std::unique(aligns.begin(),aligns.end()),aligns.end());
        std::cout << Date() << ": after unique-erase " << aligns.size() << " aligns" << std::endl;
