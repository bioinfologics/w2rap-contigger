...
class EdgeLoc {
  public:
    ...
    size_t hash() const {
        return (mEdgeID.val() << 32) | mEdgeOffset;
    }
...
class PathPart {
  public:
    ...
    EdgeID const& getEdgeID() const {
        return mEdgeLoc.getEdgeID();
    }
    bool isRC() const {
        return mEdgeLoc.isRC();
    }
    unsigned getEdgeOffset() const {
        return mEdgeLoc.getEdgeOffset();
    }
    ...
    void incrLength(unsigned len) {
        mLen += len;
    }

    unsigned getEndOffset() const {
        return getEdgeOffset()+mLen;
    }
...
class Join {
  public:
    ...
    EdgeLoc const& getEdgeLoc1() const {
        return mEdgeLoc1;
    }
    EdgeLoc const& getEdgeLoc2() const {
        return mEdgeLoc2;
    }
    ...
    struct Hasher {
        size_t operator()( Join const& join ) {
            return join.hash();
        }
    };
