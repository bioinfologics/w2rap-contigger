...
void CreateHBPlus(const HyperBasevector& hb, const vec<int>& inv,
        HyperBasevector& hbp, vec<std::pair<int,Bool>>& hbp_to_hb) 
{
     vec< vec<int> > compx;
     hb.Components(compx);
     vec<HyperBasevector> HB;
     vec< std::pair<int,Bool> > to_hb;
     ...
     {    for ( int j = 0; j < hbp.From(v).isize( ); j++ )
          {    int e = hbp.EdgeObjectIndexByIndexFrom( v, j );
               ...
               hbp_to_hb[e] = std::make_pair( ez, fw );    }    }
}
     ...
     {    while(1)
          {    Bool changed = False;
               ...
               {    int i1 = right[v][j1], i2 = left[v][j2];
                    ...
                    PrintVisualAlignment( True, std::cout, hbp.EdgeObject(e1),
                         G[gg], a1 );
                    std::cout << "\n";
                    PRINT2( a2.pos2( ), a2.Pos2( ) );
                    PrintVisualAlignment( True, std::cout, hbp.EdgeObject(e2),
                         G[gg], a2 );
                    */
...
void EdgePlacements::TwiddleSmart()
{
    ...
    vec<vec<std::pair<int,int>>> vlocs(hbp.N());  
    for ( int i = 0; i < vedata.isize( ); i++ ) {    
        int  e = vedata[i].third.first;
    ...
    for (int v = 0; v < hbp.N(); v++) {
        vec<std::pair<int,int>>& locs = vlocs[v];
        Sort(locs);
        vec<std::pair<int,int>> locs_merged;
        for (size_t i = 0; i < locs.size(); i++) {
            int start = locs[i].first;
            int stop = locs[i].second;
            size_t j = i+1;
            while (j < locs.size() && locs[j].first <= stop) {
                stop = std::max(stop, locs[j].second);
                j++;
            }
                                  ...
                                  , vec<std::tuple<int64_t,int64_t,int,int64_t,int64_t,int64_t,int64_t>>& coors_edge)
{
    ...
    for(bool bLoop=true; bLoop ;){
        ...
        for(auto&entry: coors_edge){
            const auto& b2 = std::get<5>(entry);
            auto& e2 = std::get<6>(entry);
            if(e2<b2){
                e2 = b2+ std::get<1>(entry) - std::get<0>(entry);
                bLoop=true;
            }
        }
        // adjacent edges can have overlaping "rough" positions on reference
        for(size_t cc=1;cc<coors_edge.size();++cc){
            auto& b2 = std::get<5>(coors_edge[cc]);
            const auto& e2 = std::get<6>(coors_edge[cc-1]);
            if( b2 < e2){
                b2=e2;
                bLoop=true;
            }
...
void GraphZ::BuildGraph(const int verbosity, std::ostream& out)
{
     const vec< quad< triple<int,int,int>, triple<int,int,int>, 
          std::pair<int,int>, std::pair<int,int> > >& vedata = edge_placements.vedata;
     ...
     {    out << "\nvertices:\n";
          for ( int j = 0; j < verts.isize( ); j++ )
          {    out << "[" << j << "] " << verts[j].third << " : " 
                    << verts[j].first << "." << verts[j].second << std::endl;    }
          out << "\nedges:\n";
          for ( int j = 0; j < edges.isize( ); j++ )
          {    int g = verts[ edges[j].first ].first;
               ...
               out << "[" << j << "] " << edges[j].first << " --> "
                    ...
                    << hbp_to_hb[ edges[j].third.first ].first << std::endl;    }    }
     int NN = verts.size( );
...
void GraphZ::AddConnectedGapEdges(const int min_dist, const int max_dist, const int verbosity,
        std::ostream& out ,const bool bPreserveDisconnectedComponents)
{
...
void GraphZ::FindShortestPath( const int min_dist, const int max_dist,
        ...
        vec< std::pair<int,int> >& spaths_gg_pen, std::ostream& out, int verbosity) 
{
    if ( verbosity >= 1 ) out << Date() <<":FindShortestPath()" << std::endl;
    int K = hbp.K();
    spaths.clear();
    spaths_egd.clear();
    spaths_gg_pen.clear();
    if ( verbosity >= 1 ) out << Date() <<":b4 BuildGraph()" << std::endl;
    BuildGraph(verbosity, out);
    if ( verbosity >= 1 ) out << Date() <<":after BuildGraph()" << std::endl;
    if(verbosity>=3){
    ...
    vec<int> sources, sinks;
    Z.Sources(sources), Z.Sinks(sinks);
    if ( verbosity >= 1 ) out << Date() <<":b4 AddConnectedGapEdges()" << std::endl;
    AddConnectedGapEdges(min_dist, max_dist, verbosity, out );
    if(verbosity>=3){
        std::ofstream ofs("z_AddConnectedGapEdges.dot");
        MakeZDot(ofs);
    }
    if ( verbosity >= 1 ) out << Date() <<":after AddConnectedGapEdges()" << std::endl;

    if ( verbosity >= 1 ) out << Date() <<":b4 AddGapEdges()" << std::endl;
    AddGapEdges(min_dist, max_dist, verbosity, out );
    if(verbosity>=3){
        std::ofstream ofs("z_AddGapEdges.dot");
        MakeZDot(ofs);
    }
    if ( verbosity >= 1 ) out << Date() <<":after AddGapEdges()" << std::endl;
    if ( verbosity >= 1 ) out << Date() <<":b4 AddSpecialVerts()" << std::endl;
    AddSpecialVerts(K, sources, sinks);
    if(verbosity>=3){
        std::ofstream ofs("z_AddSpecialVerts.dot");
        MakeZDot(ofs);
    }
    if ( verbosity >= 1 ) out << Date() <<":after AddSpecialVerts()" << std::endl;
    ...
    if ( verbosity >= 1 ) out << Date() <<":FindShortestPath() done" << std::endl;
}
...
void GraphZ::FindShortestPathBetween( const int this_source, const int this_sink, 
     ...
     vec< triple<int,int,int> >& spaths_egd, vec< std::pair<int,int> >& spaths_gg_pen,
     const int verbosity, std::ostream& out ) const
{
     vec<int> spath;
     ...
     {    out << "\nThere are " << spaths.size( ) << " paths:\n";
          ...
          {    const vec<int>& p = spaths[i];
               ...
               out << "[" << i+1 << "] " << gg << "." << verts[ p.front( ) ].second
                    ...
                    << ",gap_bases=" << spaths_egd[i].third << "]" << std::endl;    }    }
     if ( verbosity >= 3 )
     {    out << "\nDetailed path info:\n";
          for ( int i = 0; i < spaths.isize( ); i++ )
          {    const vec<int>& p = spaths[i];
...
void GraphZ::MakeZDot(std::ostream& out){
    vec<String> vl(Z.N());
