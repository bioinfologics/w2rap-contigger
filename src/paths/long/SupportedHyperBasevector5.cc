...
{
     std::cout << Date( ) << ": building kmers_plus, mem in use = "
          << std::setiosflags(std::ios::fixed) << std::setprecision(1) 
          << MemUsage( ) / (1024.0*1024.0) << std::resetiosflags(std::ios::fixed) 
          << " GB" << std::endl;
     ...
     {    vec<int64_t> starts;
          ...
          std::cout << Date( ) << ": sorting kmers_plus, mem in use = " 
               << std::setiosflags(std::ios::fixed) << std::setprecision(1) 
               << MemUsage( ) / (1024.0*1024.0)
               << std::resetiosflags(std::ios::fixed) << " GB" << std::endl;
           sortInPlaceParallel( kmers_plus.begin( ), kmers_plus.end( ) );    }
     std::cout << Date( ) << ": done creating kmers_plus, mem in use = " 
          << std::setiosflags(std::ios::fixed) << std::setprecision(1) << MemUsage( ) / (1024.0*1024.0)
          << std::resetiosflags(std::ios::fixed) << " GB" << std::endl;
      ...
      {   vec<int> to_delete_loc;
          ...
          kmer<K> x1,x2;
          vec< std::pair<int,int> > locs1, locs2;
     ...
     const vecqualvector& quals, const long_heuristics& heur, const long_logging& logc )
{
     ...
     if (logc.STATUS_LOGGING)
          std::cout << Date( ) << ": begin wingle branch approach" << std::endl;
     if ( logc.verb[ "WINGLE_BRANCH" ] >= 1 ) std::cout << "\n";
     vec<int> to_delete;
     ...
     if (logc.STATUS_LOGGING)
          std::cout << Date( ) << ": end wingle branch approach" << std::endl;

     // Find the bubbles.

     double clock1 = WallClockTime( );
     std::cout << Date( ) << ": divining bubbles" << std::endl;
     ...
     // the hashmap coincidentally can be accessed as a list of std::pair<a,b>,
     ...
     if ( verbosity >= 1 ) std::cout << Date( ) << ": building data structures" << std::endl;
     ...
     //      ( e1, e2, (w,q) )
     // arising from a read alignment, where (e1,e2) is a std::pair of consecutive
     ...
     vec< triple< int, int, std::pair<fix64_6,int> > > adj;
     ...
     vec< vec< std::pair<int,read_place> > > bplaces( bubbles.size( ) );
     vec< vec<read_place> > PLACES( bases.size( ) );
     if ( verbosity >= 1 ) std::cout << Date( ) << ": going through the reads" << std::endl;
     #pragma omp parallel
     {
          vec<read_place> places;

          vec< triple< int, int, std::pair<fix64_6,int> > > loc_adj;
          vec< vec< std::pair<int,read_place> > > loc_bplaces( bplaces.size() );
          #pragma omp for schedule(dynamic,1) nowait
          ...
          {    places.clear(); //vec<read_place> places;
               ...
               {    
                    #pragma omp critical
                    {    std::cout << Date( ) << ": processing read " << id 
                              << std::endl;    }    }
...
//               {
               for ( int i = 0; i < np; i++ )
               {
                    ...
                    /*
                    #pragma omp critical
                    {    std::cout << "read " << id << ", placement " << i+1 << " of "
                              << places.size( ) << ": " << places[i] << "; meets bubbles "
                              << printSeq(bids) << std::endl;    }
                    */
               }
     ...
     {    std::cout << "\nedge support:\n";
          for ( int e = 0; e < EdgeObjectCount( ); e++ ){
               ...
               std::cout << e << " --> " << tmp << "\n";
//               std::cout << e << " --> " << support[e] << "\n";
          }
          std::cout << "\n";    }

     // Test branches.

     if ( verbosity >= 1 ) std::cout << Date( ) << ": testing branches" << std::endl;
     ParallelSort(adj);
     ...
     {    if ( To(v).size( ) != 1 || From(v).size( ) != 2 ) continue;
          ...
          int f1 = EdgeObjectIndexByIndexFrom( v, 0 );
          int f2 = EdgeObjectIndexByIndexFrom( v, 1 );
          if ( support_raw[f2] > support_raw[f1] ) std::swap(f1, f2);
     ...
     {    if ( To(v).size( ) != 2 || From(v).size( ) != 1 ) continue;
          ...
          if ( support_raw[e2] > support_raw[e1] ) std::swap(e1, e2);
     ...
     {    for ( int e = 0; e < EdgeObjectCount( ); e++ )
          {    if ( support_raw[e] == 0 )
               {    
                    ...
                    if ( verbosity >= 1 ) 
                         std::cout << "deleting edge " << e << ", no support" << std::endl;
     ...
     {    double f1 = votes_bub_raw[uVotesBubWidth*i+0]/dFixedScale, r1 = votes_bub_raw[uVotesBubWidth*i+1]/dFixedScale; //double f1 = votes_bub[i][0].ToDouble( ), r1 = votes_bub[i][1].ToDouble( );
          ...
          if ( verbosity >= 1 )
          {    std::cout << "[" << i << "] " << bubbles[i][0] << " --> {" 
                    << bubbles[i][1] << "," << bubbles[i][2] << "} --> " 
                    << bubbles[i][3] << "; vote: " << std::setiosflags(std::ios::fixed) 
                    << std::setprecision(1) << f1 << "+" << r1 << " vs " << f2 << "+" 
                    << r2 << std::resetiosflags(std::ios::fixed);
               if ( q >= 0 ) std::cout << ", surprise(" << e2 << ") = " << q;
               std::cout << std::endl;    }    }
     if ( verbosity >= 1 ) std::cout << "\n";
     ...
     if ( verbosity >= 1 ) std::cout << Date( ) << ": bubble testing, approach 2" << std::endl;
     ...
     {    int e1 = bubbles[i][1], e2 = bubbles[i][2];
          ...
          {    read_place z1 = bplaces[i][j].second, z2;
               ...
               if ( k1 < 0 ) std::swap( z1, z2 );
               if ( verbosity >= 1 )
               {    std::cout << "\nid = " << id << ", place1 = " << z1 << "\n";
                    std::cout << "id = " << id << ", place2 = " << z2 << "\n";    }
          ...
          {    Sort(q1), Sort(q2);
               ...
               {    std::cout << std::setiosflags(std::ios::fixed) << std::setprecision(1);
                    ...
                    if ( m1 >= 0 )
                    {    std::cout << ", mean qsum diff = " << std::setprecision(1)
                              << m1 / 1000.0 << "\n";    }
                    std::cout << "edge " << e2 << " is supported by " << n2 << " reads";
                    if ( m2 >= 0 )
                    {    std::cout << ", mean qsum diff = " << std::setprecision(1)
                              << m2 / 1000.0 << "\n";    }    }
               if ( q1.empty( ) )
               {    if ( verbosity >= 1 ) std::cout << "deleting edge " << e1 << "\n";
                    to_delete.push_back(e1);
                    continue;    }
               if ( q2.empty( ) )
               {    if ( verbosity >= 1 ) std::cout << "deleting edge " << e2 << "\n";
                    to_delete.push_back(e2);
                    continue;    }
               double dist = (m1-m2) / sqrt( m1*m1/n1 + m2*m2/n2 );
               if ( verbosity >= 1 )
               {    std::cout << "dist = " << std::setprecision(2) << Abs(dist) << "\n";
     ...
     {    int n = KmerId( bases[id], L, 0 );
          ...
          {    
               ...
               #pragma omp critical
               {    for ( int j = 0; j < places.isize( ); j++ )
                    {    std::cout << "read " << id << " placed: " << places[j] 
     ...
     vec< vec< std::pair<int,int> > > homes_index( EdgeObjectCount( ) );
     for ( int64_t id = 0; id < nBases; id++ )
...
{    double clock = WallClockTime( );
     if (logc.STATUS_LOGGING) std::cout << Date( ) << ": making loops" << std::endl;
     ...
     {    if ( !From(v).solo( ) || To(v).size( ) != 2 ) continue;
          ...
          if ( logc.verb[ "MAKE_LOOPS" ] >= 1 )
               std::cout << "making loop from " << e1 << ", " << e2 << std::endl;
          DeleteEdges(dels);
     ...
     RemoveDeadEdgeObjects( );
     if (logc.STATUS_LOGGING)
     {    std::cout << Date( ) << ": after making loops there are "
               << EdgeObjectCount( ) << " edges" << std::endl;    }
     TestValid(logc);    }
     ...
     const vecqualvector& quals, const long_heuristics& heur, const long_logging& logc )
{
     ...
     double clock1 = WallClockTime( );
     std::cout << Date( ) << ": divining single mutation bubbles" << std::endl;
     int verbosity = logc.verb[ "DIVINE_BUBBLES" ];
     vec< vec<int> > bubbles;
...
//     vec<int64_t> support_raw(nEdgeObjectCount, 0); 
     ...
     if ( verbosity >= 1 ) std::cout << Date( ) << ": going through " << bases.size() << " reads" << std::endl;
     #pragma omp parallel
     {
     ...
     {    double f1 = votes_bub_raw[uVotesBubWidth*i+0]/dFixedScale, r1 = votes_bub_raw[uVotesBubWidth*i+1]/dFixedScale; //double f1 = votes_bub[i][0].ToDouble( ), r1 = votes_bub[i][1].ToDouble( );
          double f2 = votes_bub_raw[uVotesBubWidth*i+2]/dFixedScale, r2 = votes_bub_raw[uVotesBubWidth*i+3]/dFixedScale; //double f2 = votes_bub[i][2].ToDouble( ), r2 = votes_bub[i][3].ToDouble( );
          if (verbosity>=1) std::cout<< "raw " << std::setiosflags(std::ios::fixed) << std::setprecision(1) << f1 << " " <<r1 << " vs " << f2 << " " << r2 <<std::endl <<std::resetiosflags(std::ios::fixed);
          int e1 = 1, e2 = 2;
          if ( f2 + r2 > f1 + r1 || ( f2 + r2 == f1 + r1 && f2 > f1 ) )
          {    std::swap( e1, e2 );
               std::swap( f1, f2 );
          ...
          {    std::cout << "[" << i << "] " << bubbles[i][0] << " --> {" 
                    ...
                    << r2 << std::resetiosflags(std::ios::fixed);
               if ( q >= 0 ) std::cout << ", surprise(" << e2 << ") = " << q;
               if (iDeletedEdge>=0) std::cout << " deleted edge " << iDeletedEdge << std::endl;
               std::cout << std::endl;    }    }
     if ( verbosity >= 1 ) std::cout << "\n";
