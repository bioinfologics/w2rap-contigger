...
class IndirectKmer {
  public:
    ...
    bvec::const_iterator getBases() const {
        return mpBV->begin(mOffset);
    }
    size_t getId( bvec const* pBV ) const {
        return mpBV-pBV;
    }
    ...
    class BizarreComparator {
      public:
        ...
        std::set<size_t> const& getIds() const {
            return mIds;
        }
...
class Dotter {
  ...
  private:
    ...
    struct EdgeStatus {
        ...
        void setAdjacent( bool isAdjacent = true ) {
            mIsAdjacent = isAdjacent;
        }
...
void GapToyEvaluate( const String& SAMPLE, const String& species,
    ...
    for ( int g = 0; g < (int) G.size( ); g++ ) {
        ...
        if (verbose) {
            std::cout << Date( ) << ":";
            if ( SAMPLE == "NA12878" ) std::cout << "F = " << fosmids[g];
            else std::cout << "G = " << g+1;
            std::cout << ", using " << c.size( ) << " vertices ("
                      << PERCENT_RATIO( 3, c.isize( ), hb.N( ) ) << ")" << std::endl;
        }
    ...
    // std::cout << "perfect N50=" << n50 << std::endl;
    // PerfStatLogger::log("N50_perfect",n50,"error-free N50");

    gaps.append(endgaps);
    ReverseSort(gaps), ReverseSort(indels);
    std::cout << "gaps = " << ToStringN(gaps,10) << ",\n" << "indels = "
              << ToStringN(indels,10) << ",\nsubs = " << subs << std::endl;
    if ( res.Defined( ) ) {
        std::cout << "Comparison to r" << res.rev << ": ";
        if ( gaps.isize( ) + endgaps.isize( ) == res.gaps
                && indels.isize( ) == res.indels && subs == res.subs ) {
            std::cout << "SAME.\n";
        } else {
            std::cout << "CHANGED, previous values were:\n";
            std::cout << "gaps = " << res.gaps << ", " << "indels = "
                      << res.indels << ", subs = " << res.subs << std::endl;
        }
    }
}
