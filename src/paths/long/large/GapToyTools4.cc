...
void ExtendPath( ReadPath& p, const int64_t i, const HyperBasevector& hb,
    ...
    for ( int j = 0; j < exts.isize( ); j++ ) {
        if ( j > max_exts ) {
            fail = True;
            if (verbose) std::cout << "too many extensions" << std::endl;
            break;
        }
    ...
    if (verbose) std::cout << "<0> " << r.ToString( ) << std::endl;
    ...
    for ( int j = 0; j < exts.isize( ); j++ ) {
        ...
        if (verbose) {
            std::cout << "<" << j+1 << "> " << b.ToString( )
                      << ", qsum = " << qsum[j] << std::endl;
        }
    }
    SortSync( qsum, exts );
    if ( mode == 1 ) {
...
void ExtendPath2( ReadPath& p, const int64_t i, const HyperBasevector& hb,
    ...
    int adds = 0;
    if (verbose)
        std::cout << "\n[" << i << "] (" << start << ") " << printSeq(p) << std::endl;
    if ( start < 0 ) {
        if ( mode == 0 ) return;

        // Try extending backwards.

        if (verbose) std::cout << "-- left extends " << -start << " bases" << std::endl;
        int e = p.front( );
        int v = to_left[e];
        ...
        if ( start < 0 ) {
            if ( verbose && adds > 0 )
                std::cout << "extended to " << printSeq(p) << std::endl;
            return;
        }
    }
    ...
    int v = to_right[e];
    if ( hb.From(v).empty( ) ) return;
    if (verbose) std::cout << "-- right extends " << ext << " bases" << std::endl;
    ...
    if ( verbose && adds > 0 ) std::cout << "extended to " << printSeq(p) << std::endl;
}
    ...
    for(int vv=0; vv<hb.N(); ++vv) {
        ...
        if(    hb.ToSize(vv) == 1
            ...
            if(edge_alt_[edge_0] < 0 || edge_alt_[edge_1]<0) {
                if( edge_alt_[edge_0]>=0 || edge_alt_[edge_1]>=0) {
                    std::cout << "inconsistent bubble, probably due to inversion properties" << std::endl;
                    continue;
                }
...
std::ostream& operator<<(std::ostream&os, bubble_logger const& in) {
    for(const auto& entry: in.getData()) {
        os << "\n" << entry << "\n";
    }
    return os;
};
std::ostream& operator<<(std::ostream& os, bubble_logger::bubble_data_t const&in) {
    ...
    auto sum_branch_branch=[](vec<s_t>const&i) {
        return std::accumulate(i.begin(),i.end(),0 ,[](int i, s_t const&s) {
            return i+s.branch_branch;
        });
    };

    auto print_branch_branch=[](std::ostream& o, vec<s_t> i) {
        if(i.size()==0) return;
        std::sort(i.begin(),i.end(),[](s_t const&L,s_t const&R) {
            return L.branch_branch > R.branch_branch;
        } );
        o << i.front().branch_branch;
        for(size_t ii=1; ii<i.size(); ++ii) {
            o << "," << i[ii].branch_branch;
        }
    };
...
void LogBubbles( bubble_logger& logger, HyperBasevector& hb , const vec<int>& inv2
    ...
    */
    ...
    for(int64_t rr=0; rr<bases.size(); ++rr,++qvItr) {
...
void BuildGenomeMap( const HyperBasevector& hb, const vec<int>& inv,
    ...
    vec< vec< std::pair<int,int> > > hitsx(hits);
    for ( int e = 0; e < hits.isize( ); e++ ) {
    ...
    for ( int i = 0; i < lines.isize( ); i++ ) {
        ...
        std::pair<int,int> start( -1, -1 ), stop( -1, -1 );
        for ( int j = 0; j < L.isize( ); j += 2 ) {
            int e = L[j][0][0];
            int re = inv[e];
    ...
    for ( int i = 0; i < locs.isize( ); i++ ) {
        ...
        if ( l < 0 ) {
            vec<String> row;
            //[GONZA] ToStrignAddCommas removed, casted to string to fit the function
            row.push_back( genome_names[chr], std::to_string(start), std::to_string(stop), "**REF GAP**" );
            rows.push_back(row);
            continue;
        }
        ...
        vec<String> row;
        //[GONZA] ToStrignAddCommas removed, casted to string to fit the function
        row.push_back( genome_names[chr], std::to_string(start), std::to_string(stop), "line[" + ToString(l) + "]", ToString(e1) + ".." + ToString(e2), "len=" + ToString(lens[l]) );
        int ns = covs.size( );
        Bool defined = False;
        for ( int ss = 0; ss < ns; ss++ )
...
String PrintHits( const int e, vec<vec<std::pair<int,int>>>& hits,
    ...
    for ( int pass = 1; pass <= 2; pass++ ) {
        ...
        for ( int j = 0; j < hits[f].isize( ); j++ ) {
            ...
            //[GONZA] ToStrignAddCommas removed, casted to string to fit the function
            String xstart = std::to_string(start);
            String xstop = std::to_string(stop);
