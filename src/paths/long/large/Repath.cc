...
vec<vec<int> > get_places(const HyperBasevector &hb,
    ...
    OutputLog(2) << pathed << " / " << paths.size() << " reads pathed, " << multipathed << " spanning junctions"
                 << std::endl;
...
//          std::cout<<" )"<<std::endl;
        ...
        if (old_paths[m].size() == 0) {
        ...
        else if (old_paths[m].size() == 1 and old_paths[m + 1].size() == 1 and
                 old_paths[m][0] == old_hbvinv[old_paths[m + 1][0]]) {
            ++same;
            ...
            if (ovlp > -1) {
                ...
                if (toRight[old_paths[m].back()] == toLeft[old_hbvinv[old_paths[m + 1].back()]]) {
                    ...
                    auto r1p = old_paths[m];
                    for (auto r2e = old_paths[m + 1].rbegin(); r2e < old_paths[m + 1].rend(); ++r2e)
                        r1p.push_back(old_hbvinv[*r2e]);
                    newplaces.push_back(r1p);
                    ++direct;
                } else {
    ...
    OutputLog(3) << "paths -> places: single=" << single << " same=" << same << " overlap=" << overlap << " direct="
                 << direct << " disjoint=" << disjoint << std::endl;
...
#pragma omp parallel for
    for (int64_t m = 0; m < (int64_t) old_paths.size(); m += batch) {
        ...
        for (int64_t i = m; i < n; i++) {
            ...
            if (nkmers + ((int) old_K - 1) >=
                new_K) {      // If path is 'interesting' add rc to y and add that to places
                for (int j = x.size() - 1; j >= 0; j--)
