...
class PullAparter {
  public:
    ...
    size_t getRemovedReadPaths() const {
        return mRemovedReadPaths;
    }
    ...
    bool isSeparable(int edge, vec<vec<int>>* sep_paths, bool nukeBadReadPaths = false) {
        ...
        if ( pathMask == 0b1001 ) {
            ...
            if ( nukeBadReadPaths ) {
                nukeReadPaths( scoreReads[1] );
                nukeReadPaths(scoreReads[2]);
            }
        } else if ( pathMask == 0b0110 ) {
            if ( sep_paths ) sep_paths->push_back( paths[1], paths[2] );
            if ( nukeBadReadPaths ) {
                nukeReadPaths( scoreReads[0] );
                nukeReadPaths(scoreReads[3]);
            }
        } else {
            if ( mDebug ) {
    ...
    void MigrateReadPaths( vec<int> const& path1, vec<int> const& path2,
        ...
        for ( auto itr = invPaths.begin(); itr != invPaths.end(); ++itr ) {
            ...
            if ( ldebug ) {
                std::cout << "...";
                PRINT2(path1_support, path2_support);
            }
