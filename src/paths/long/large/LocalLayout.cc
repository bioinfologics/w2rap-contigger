...
longlong KmerQScoreSum(int K, kmer_id_t kid, vecbasevector const& bases,
                       ...
                       vecKmerPath const& paths_rc, vec<big_tagged_rpint> const& pathsdb, std::ostream& out ) {
    longlong qsumsum = 0;
    ...
    out << "  [TOTAL " << qsumsum << " AVG " << static_cast<double>(qsumsum) / path_ids.size() << "]" << std::endl;
    ...
    for ( int vid = 0; vid < h.N(); ++vid ) {
        ...
        for ( int fid = 0; fid < h.FromSize(vid); ++fid ) {
            ...
            if ( test1 || test2 ) {
                out3 << "  REMOVE EDGE " << edge_id << " test1=" << (test1?"T":"F")
                     << " test2=" << (test2?"T":"F") << std::endl;
                delete_me_from[edge_id] = True;
                delete_from.insert(std::make_pair(edge_id,vid));
            }
        }
        out3 << std::endl;

        out3 << "TO " << vid << ": " << std::endl;
        scores.clear();
        ...
        for ( int fid = 0; fid < h.ToSize(vid); ++ fid ) {
            ...
            if ( test1 || test2 ) {
                ...
                delete_to.insert( std::make_pair(edge_id,vid) );
            }
        }
        out3 << std::endl;
    }
