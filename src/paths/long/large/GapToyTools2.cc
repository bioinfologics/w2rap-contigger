...
{    
     ...
     {    if ( !hb.Sink( to_right[e] ) ) continue;
          ...
          std::cout << "extensions of edge " << e << std::endl;
          ...
          {    int id = layout_id[e][j];
               ...
               {    std::cout << "rc ";
                    ...
                    {    int pos = bases[id].isize( ) - 1
                         ...
                         // if ( quals[id][pos] <= 2 ) break;
                         std::cout << as_base( 3 - bases[id][pos] );
          ...
          {    int j;
               ...
               std::cout << exts[i].first << ":";
               for ( int l = 0; l < 4; l++ )
               {    if ( qsum[l] == 0 ) break;
                    std::cout << " " << as_base( ids[l] ) << "["
                         << qsum[l] << "]";    }
               std::cout << "\n";
               i = j - 1;    }    }    }
...
{    
     ...
     std::cout << Date( ) << ": found " << unplaced.size( )
          << " unplaced reads" << std::endl;
     vecbasevector all;
     ...
     const int M = 32;
     vec< vec< std::pair<int,int> > > places( unplaced.size( ) );
     vec<String> prefixes;
     ...
     {    vec< triple<kmer<M>,int,int> > kmers_plus;
          ...
          {    int64_t& s = bstart[i];
               ...
               {    s--;    }    }
          // std::cout << Date( ) << ": start parallel for loop" << std::endl;
               ...
               {    for ( int64_t k = i; k < j; k++ )
                    {    if ( kmers_plus[k].second >= nedges )
                         {    int e = kmers_plus[ g[0] ].second;
                              ...
                              std::pair<int,int> p = std::make_pair( e, epos - rpos );
...
{    std::cout << Date( ) << ": setup" << std::endl;
     ...
     std::cout << "total kmers = " << ToStringAddCommas(total_kmers) << std::endl; // XXX
     const int K0 = 60;
     vec< triple<kmer<K0>,int,int> > kmers_plus;
     MakeKmerLookup0( edges, kmers_plus );
     std::cout << Date( ) << ": made kmer lookup table" << std::endl; // XXXXXXXXXXXXXXXX
     ...
     std::cout << Date( ) << ": main loop" << std::endl;
     #pragma omp parallel for
