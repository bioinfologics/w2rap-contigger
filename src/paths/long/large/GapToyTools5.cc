...
void AlignToGenome( const HyperBasevector& hb, const vec<int>& inv,
    ...
    for ( int pi = 0; pi < 4; pi++ ) {
        ...
        // std::cout << TimeSince(clock2) << " used building kmers" << std::endl;
        double clock2b = WallClockTime( );
        ParallelSort(kmers_plus);
        // std::cout << TimeSince(clock2b) << " used sorting kmers" << std::endl;
        ...
        // std::cout << TimeSince(clock3a) << " used setting up traversal" << std::endl;
            ...
            for ( int64_t i = bstart[bi]; i < bstart[bi+1]; i++ ) {
                ...
                if ( k > i && j - k == 1 && k - i <= max_gmult ) {
                    for ( int64_t m = i; m < k; m++ ) {
                        ...
                        if ( !Member( hits[e], std::make_pair( g, offset ) ) ) {
                            Bool mismatch = False;
        ...
        // std::cout << TimeSince(clock3) << " used traversing kmers" << std::endl;
    }
...
void AlignToGenomePerf( const HyperBasevector& hb, const vecbasevector& genome,
    ...
    for ( int e = 0; e < hb.E( ); e++ )
        for ( int j = 0; j < hits[e].isize( ); j++ ) {
            perfs.push( std::make_pair( hits[e][j].second, hits[e][j].third ),
                        std::make_pair( e, hits[e][j].first ), hits[e][j].fourth );
        }
    ...
    {    return std::make_pair( perfs[i1].first, -(perfs[i1].third) )
              < std::make_pair( perfs[i2].first, -(perfs[i2].third) );    } );
    */

    vec< std::pair< std::pair<int,int>, int > > keys( perfs.size( ) );
    for ( int i = 0; i < keys.isize( ); i++ )
        keys[i] = std::make_pair( perfs[i].first, -perfs[i].third );
    ...
    {    perf_place& p = places[i];
         ...
         {    int f = hb.IFrom( v, j );
              ...
              {    if ( genome[g][ gstop + l ] != hb.EdgeObject(f)[ K-1 + l ] )
                   {    mismatch = True;
                        cout << "mismatch" << std::endl; // XXXXXXXXXXXXXXXXXXXXXXXXXXXX
                        break;    }    }
...
void ExtendPairs60( HyperBasevector& hb, vec<int>& inv, ReadPathVec& paths ) {
    ...
    Sort(con);
    vec< std::pair<int,int> > mix1;
    vec<int> mix2;
    for ( int i = 0; i < con.isize( ); i++ ) {
        int j = con.NextDiff(i);
        int e1 = con[i].first, e2 = con[i].second;
        if ( !( std::make_pair( e1, e2 ) <= std::make_pair( inv[e2], inv[e1] ) ) ) continue;
        ...
        int o = overs[0];
        /*
        std::cout << "[" << j-i << "] " << con[i].first << ".." << con[i].second;
        std::cout << " over=" << o << " ";
        for ( int j = 0; j < o; j++ )
             std::cout << as_base( hb.EdgeObject(e2)[j] );
        std::cout << std::endl;
        */
    ...
    for ( int64_t i = 0; i < (int64_t) paths.size( ); i += 2 ) {
        ...
        for ( int i = ( (int) p2.size( ) ) - 1; i >= 0; i-- )
            x2.push_back( inv[ p2[i] ] );
        int p = BinPosition( mix1, std::make_pair( x1.back( ), x2.front( ) ) );
        if ( p < 0 ) continue;
    ...
    std::cout << TimeSince(clock) << " used extending pairs" << std::endl;
}
...
void PlacePartners( const HyperBasevector& hb, const vec<int>& inv,
    ...
    double clock = WallClockTime( );
    std::cout << Date( ) << ": placing partners" << std::endl;
    vec<int> to_left, to_right;
    hb.ToLeft(to_left), hb.ToRight(to_right);
    ...
    std::cout << Date( ) << ": defining homes" << std::endl;
    ...
    std::cout << Date( ) << ": forming all" << std::endl;
    vec<int> hall;
    for ( int i = 0; i < ids.isize( ); i++ )
        hall.append( homes[i] );
    UniqueSort(hall);
    ...
    vec< vec< std::pair<int,int> > > hits( ids.size( ) );
    std::cout << Date( ) << ": forming lookup table" << std::endl;
    std::cout << TimeSince(clock) << " used initially" << std::endl;
    vec<String> prefixes = { "AA", "AC", "AG", "AT", "CA", "CC", "CG", "CT", "GA",
                             "GC", "GG", "GT", "TA", "TC", "TG", "TT"
                           };
    std::cout << "memory usage = " << ToStringAddCommas( MemUsageBytes( ) ) << std::endl;
    // prefixes = { "A", "C", "G", "T" };
    for ( int pass = 0; pass < prefixes.isize( ); pass++ ) {
        double lclock = WallClockTime( );
        vec< triple<kmer<K>,int,int> > kmers_plus;
        MakeKmerLookup0Pre( all, prefixes[pass], kmers_plus );
        std::cout << TimeSince(lclock) << " used forming lookup table" << std::endl;

        // Find homes for partners.

        std::cout << Date( ) << ": finding homes for partners" << std::endl;
        ...
        std::cout << TimeSince(fclock) << " used finding homes" << std::endl;
    }
    double sclock = WallClockTime( );
    for ( int64_t i = 0; i < ids.jsize( ); i++ )
        UniqueSort( hits[i] );
    std::cout << TimeSince(sclock) << " used sorting" << std::endl;
            ...
            for ( int j = 0; j < hits[i].isize( ); j++ ) {
                ...
                if ( start < 0 && hb.To(v).nonempty( ) ) {
                    ...
                    for ( int l = 0; l < hb.To(v).isize( ); l++ ) {
                        int e = hb.EdgeObjectIndexByIndexTo( v, l );
                        int len = hb.EdgeLengthKmers(e);
                        if ( l == 0 ) hits[i][j] = std::make_pair( e, start + len );
                        else {
                            hits[i].push( e,
                                          start + len );
                        }
    ...
    for ( int64_t i = 0; i < ids.jsize( ); i++ ) {
        ...
        hits[i][0] = std::make_pair( ep, start + hb.EdgeLengthKmers(ep) );
    }
    ...
    for ( int64_t i = 0; i < ids.jsize( ); i++ ) {
        ...
        hits[i].resize(1);
        hits[i][0] = std::make_pair( e, start1 + hb.EdgeLengthKmers(e) );
    }
    ...
    for ( int64_t i = 0; i < ids.jsize( ); i++ ) {
        ...
        if ( start >= 0 && start < hb.EdgeLengthBases(e) ) {
            ...
            placed++;
            // std::cout << "\nplaces for " << i << " = " << ids[i] << ":\n";
            // std::cout << p.getOffset( ) << ":" << printSeq(p) << "\n";
        }
    }
    ...
    std::cout << TimeSince(tclock) << " used in tail" << std::endl;
    std::cout << TimeSince(clock) << " used in total placing partners" << std::endl;
}
...
size_t findInterestingReadIds( HyperBasevector const& hbv,
    ...
    if ( true ) {
        ...
        vec<int> D;
        int hbk = hbv.K();
        DistancesToEndFunc( hbv, [hbk](bvec const& bv) {
            return bv.size()-hbk+1;
        },
        MAX_DIST, True, D );
...
class KmerLocs : public Kmer {
  public:
    ...
    unsigned getELocs() const {
        return mELocs;
    }
    void setELocs( unsigned eLocs ) {
        mELocs = eLocs;
    }

    size_t getTotalLocs() const {
        return mNLocs+mELocs;
    }
  ...
  private:
    void addLocs( KmerLocs const& locs, int eOffset ) {
        ...
        while ( beg != end && lItr != lEnd ) {
            --lEnd;
            Loc loc(lEnd->getReadId(),lEnd->getOffset()-eOffset);
            if ( !(loc == *--end) ) {
                ++lEnd;
                break;
            }
...
void PartnersToEndsOld( const HyperBasevector& hb, ReadPathVec& paths,
    ...
    int hbk = hb.K();
    DistancesToEndFunc( hb, [hbk]( bvec const& bv ) {
        return bv.size()-hbk+1;
    },
    max_dist, True, D );
