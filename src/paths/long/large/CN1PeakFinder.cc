...
void CN1PeakFinder::PrefilterHighCNPeaks (const vec<double>& coverage, const vec<int64_t>& mass) {
    ...
    if (debug)
        std::cout << "Prefilter assumed CN1 peak coverage: " << max_coverage << std::endl;
    ...
    if (debug)
        std::cout << "Prefilter removed last " << peak_count - max_candidate << " peaks" << std::endl;
...
double CN1PeakFinder::FindPeak (const vec<double>& coverage, const vec<int64_t>& mass) {
    ...
    if (mass.size() == 0) {
        if (debug) std::cout << "No valid data, cannot find peak" << std::endl;
        return 0;
    }
    ...
    if (peak_count == 1 ) {
        ...
        for (size_t i = 0; i < peak_count; i++) {
            ...
            // Compute the score
            score = std::count_if(used.begin(), used.end(),  [] (int val) {
                return val != 0;
            });
            if (debug)
                std::cout << "SCORE: " << i << " " << score << " " << ToStringBool(diploid) << std::endl;
