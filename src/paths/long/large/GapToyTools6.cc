...
void PrintSysInfo( ) {
    ...
    int ok = uname( &un );
    if ( ok != 0 ) {
        std::cout << "- Warning: attempt to determine system name through uname "
                  << "failed." << std::endl;
        std::cout << "- This is weird but not a problem." << std::endl;
    } else {
        std::cout << "- OS: " << un.sysname << " :: " << un.release
                  << " :: " << un.version << std::endl;
        std::cout << "- node name: " << un.nodename << std::endl;
        std::cout << "- hardware type: " << un.machine << std::endl;
    }
    if ( IsRegularFile( "/proc/cpuinfo" ) ) {
        ...
        for ( int i = 0; i < lines.isize( ); i++ ) {
            ...
            if ( L.Contains("model name", 0) ) std::cout << "- cpu " << L << std::endl;
            if ( L.Contains("cpu MHz", 0) || L.Contains("cache size", 0) )
                std::cout << "- " << L << std::endl;
        }
    }
    std::cout << "- physical memory: " <<  PhysicalMemGBString( ) << std::endl << std::endl;
}

void MemoryCheck( const Bool MEMORY_CHECK, const String& work_dir ) {
    if (MEMORY_CHECK) {
        std::cout << "MEMORY CHECK (typically takes several minutes; could cause "
        ...
        vec<char*> ptr(100);
        if ( pid == -1 ) {
            std::cout << "- Warning: tried to fork process to test available memory, "
                      << "but the fork failed." << std::endl;
        } else if ( pid == 0 ) {
            ...
            for ( d = 1; d <= 100; d++ ) {
                ...
                if ( ptr[d-1] == NULL ) {
                    d--;
                    std::cout << "- Apparently able to allocate " << d << "% of "
                              << "nominally available memory (and not more)."
                              << std::endl;
                    std::cout << "- Can access " << (d*M) / (1024*1024*1024)
                              << " GB." << std::endl;
                    if ( d < 90 ) {
                        std::cout << "- WARNING: you may have less available memory "
                                  << "than you think!" << std::endl;
                    } else {
                        std::cout << "- But you're pretty close, so things should "
                                  << "be OK." << std::endl;
                    }
            ...
            if ( d > 100 ) {
                std::cout << "- Apparently able to allocate 100% of nominally "
                          << "available memory." << std::endl;
                std::cout << "- Can access at least " << P / (1024*1024*1024)
                          << " GB." << std::endl;
            }
            _exit(0);
        } else {
        ...
        if ( IsRegularFile( work_dir + "/available_mem" ) ) {
            ...
            memin >> mema;
            SetMaxMemory(mema);
            std::cout << "- Lowering max memory to " << mema
                      << " bytes." << std::endl;
        }
        std::cout << std::endl;
    } else {
        std::cout << "Omitting memory check.  If you run into problems with memory,\n"
...
void DefineRegions( const String& X, vec<int>& fosmids, vec<String>& regions,
                    std::map<String,GapToyResults>& res, const int PAD, Bool& all, const String& SAMPLE,
    ...
    if ( X == "tiny" ) {
        ...
        std::cout << "assembling fosmid id = " << fosno << std::endl;
        ...
        fosmids = AllFosmids( );
        regions = AllFosmidRegions(PAD);
    } // all fosmids with padding
        ...
        std::cout << "Unknown X argument." << std::endl;
        Scram(1);
    }
    if ( F != "default" ) {
        vec<int> ftmp;
        ...
        if ( !Subset(ftmp, fosmids) ) {
            std::cout << "WARNING: your eval fosmids may not intersect with the "
