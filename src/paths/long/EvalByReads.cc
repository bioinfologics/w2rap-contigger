...
{    
     ...
     std::priority_queue<read_place, std::vector<read_place>, CompareReadPlaceByQsum> candidates;
     ...
     const int min_qual, const double prox, const int max_diff )
{
     ...
     const int infinity = 1000000000;
     int best_qsum = infinity;
     typedef std::priority_queue<read_place, std::vector<read_place>, CompareReadPlaceByQsum> candidates_t;
          ...
          {    const read_place& p = places[l];
               ...
               {    basevector r( ref.size( ) );
                    for ( int i = 0; i < ref.isize( ); i++ )
                         r.Set( i, ref[i] );
                    #pragma omp critical
                    {    std::cout << "read " << id << " aligns to " 
                              << ( pass == 0 ? "A" : "R" )
                              << " with " << errs << " errors" 
                              << " and qual sum = " << qual_sum[pass] 
                              << ", start = " << p.E(0) << ":" << p.P( ) << std::endl;
                         std::cout << "\nalignment of read " << id << std::endl;
                         align a;
                         a.SetNblocks(1);
                         a.SetGap( 0, 0 );
                         ...
                         if ( p.Fw( ) )
                              PrintVisualAlignment( True, std::cout, b, r, a, q );
                         else
