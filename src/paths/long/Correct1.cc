...
{    double clock = WallClockTime( );
     ...
     std::cout << "Time use in FriendAligner " << TimeSince( time ) << std::endl;
     ...
     std::cout << Date( ) << ": starting main correction loop" << std::endl;
               ...
               {    for ( int pass = 1; pass <= 2; pass++ )
                    {    if ( pass == 2 ) b.resize( trim_to[id1] );
                         ...
                         {    look_align& la = aligns[j];
                              la.query_id = id1;
                              int g = la.target_id;
                              la.PrintReadableBrief( std::cout, "read_" + ToString(id1),
                                   ToString(g) );
