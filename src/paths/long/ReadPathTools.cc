...
void DisplayReadPath(std::ostream &out, const vecbvec& graph_edges,
	...
	std::cout << "ERROR - Invalid path offset: " << offset << std::endl;
	return;
    }
    ...
    if (show_mismatch) out << mismatches << std::endl;
    if (show_seq)        out << seq << std::endl;
    out << paddingStr << edge_path << std::endl;
    if (show_overlap)  out << overlaps << std::endl;
    if (show_edges)   for (String& line : edges)  out << line << std::endl;
    if (show_ruler)  out << MakeRuler(read_length) << std::endl;
    if (edge_lookup && have_hbv) {
	if (left_overlap && (hbv.ToSize(to_left[edge_list[1]]) > 1) )
	    std::cout << "Warning: path extends ambgiously to the left >-" << std::endl;
	if (right_overlap && (hbv.FromSize(to_right[edge_list[edge_list.size() - 2]]) > 1) )
	    std::cout << "Warning: path extends ambgiously to the right -<" << std::endl;
    }
    if ( bad_path )
	std::cout << "Warning: a path was encountered where the read finished before the path" << std::endl;
}
