...
class FriendContext {
  public:
    ...
    friend std::ostream& operator<<( std::ostream& os, FriendContext const& fc ) {
        if ( fc.mRC ) os << '~';
  ...
  private:
    ...
    class HashEnt1 : public KMer<K> {
      public:
        ...
        bool isRC() const {
            return mForm==CanonicalForm::REV;
        }
                ...
                for ( auto qvItr=qv.begin(); qvItr!=qvEnd; ++qvItr, ++off ) {
                    if ( *qvItr < mMinQual ) goodQuals = 0;
                        ...
                        if ( !first ) mHS.apply(lastEnt,lastEnt);
                            ...
                            first = false;
                            ent.setContext(KMerContext::NNContext());
                        }
                        lastEnt = ent;
                    }
                }
      ...
      public:
        ...
        void operator()( size_t batchId ) {
            ...
            for ( auto bItr = mReads.begin(rId); bItr != bEnd; ++bItr, ++qItr, ++rId ) {
                ...
                circIdx.push_back(circleIds);
                circleIds.clear();
            }
            std::unique_lock<std::mutex> lock(mMutex);
            mCondVar.wait(lock,
            [this,startId]() {
                return mCircIdx.getNElements()==startId;
            });
    ...
    class Proc3 {
      public:
        ...
        void operator()( size_t batchId ) {
            ...
            mCondVar.wait(lock,
            [this,startId]() {
                return mCircIdxWriter.getNElements()==startId;
            });
