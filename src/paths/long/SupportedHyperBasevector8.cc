...
void SupportedHyperBasevector::Hookup( const long_logging& logc )
{
     ...
     {    cout << EdgeObjectCount( ) << " edges and " << NComponents( )
          ...
          Sort(len);
          cout << "N50 edge = " << N50(len) << std::endl;    }
     ...
     for ( int pass = 1; pass <= 2; pass++ )
     {
          ...
          vec< vec< std::pair<int,int> > > users(ne);
          for ( int i = 0; i < joins.isize( ); i++ )
          ...
          {    if ( joins[i].empty( ) || Inv( joins[i][0] ) < 0 ) continue;
               ...
               if ( p < 0 )
               {    if ( verbosity >= 3 )
                    {    cout << "problem with join" << std::endl;
                         cout << "join = " << printSeq(joins[i]) << std::endl;
                         cout << "inv = " << printSeq(x) << std::endl;    }
                    dead[i] = True;    }
               else
               {    vec<int> y = joins[i];
                    Sort(x), Sort(y);
          ...
          {    cout << "\nFINAL JOINS:\n";
               ...
               {    const vec<int>& x = joins[i];
                    ...
                    cout << ") ends = " << printSeq( ends[i] ) << std::endl;    }    }
     
          // Edit graph.

          if ( verbosity >= 1 ) std::cout << "\n" << Date( ) << ": editing graph" << std::endl;
          ...
          if ( verbosity >= 1 ) std::cout << Date( ) << ": transforming paths" << std::endl;
          vec<int> news;
          for ( int i = 0; i < joins.isize( ); i++ )
               news.push_back( ne + i );
          ...
          if ( verbosity >= 1 ) std::cout << Date( ) << ": cleaning up" << std::endl;
          RemoveUnneededVertices( );
          RemoveEdgelessVertices( );
          RemoveDeadEdgeObjects( );
          if ( verbosity >= 1 ) std::cout << Date( ) << ": testing" << std::endl;
          TestValid(logc);
          ...
          if ( verbosity >= 1 ) std::cout << Date( ) << ": done" << std::endl;
