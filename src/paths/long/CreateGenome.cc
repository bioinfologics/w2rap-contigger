...
{    if ( IsRegularFile( genome_head + ".fastamb" ) )
     {    const int ext = 50000;
          ...
          {    std::cout << Date( ) << ": Warning, ";
               if (extend) std::cout << "extension of ";
               std::cout << region << " has " << "seq of ambiguous bases of len "
                    << amblen[0] << "." << std::endl;
               return True;    }    }
     return False;    }
...
{    
     ...
     {    String dir 
          ...
          std::cout << Date( ) << ": genome/region has size " 
               << int64_t(round(true_size))  << std::endl;
          return;    }
     ...
     {    vec<String> chrs;
          ...
          std::map<String,basevector> defs;
     ...
     {    String genome_fastb = genome_head + ".fastb";
          ...
          {    
               ...
               {    vec<String> regions0, regionsx, regions2;
                    vec< vec< std::pair<String,String> > > junctions, breaks, edits;
               ...
               {    int npasses = 1;
                    ...
                    if ( found > 0 )
                    {    std::cout << Date( ) << ": found known assemblies for " << found 
