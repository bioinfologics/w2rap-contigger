...
void SupportedHyperBasevector::PullApart( const double min_weight_split,
        const long_logging& logc ) {
    if (logc.STATUS_LOGGING) std::cout << Date( ) << ": pulling apart" << std::endl;
    while(1) {
        ...
        if ( logc.verb[ "PULL_APART" ] >= 1 )
            std::cout << Date( ) << ": start round of pulling apart" << std::endl;
        for ( int v = 0; v < N( ); v++ ) {
                    ...
                    if ( logc.PULL_APART_DEBUG ) {
                        ...
                        for ( int e = 0; e < EdgeObjectCount( ); e++ ) {
                            ...
                            if ( to_left[e] != to_leftx[e] ) {
                                std::cout << "to_left wrong at edge " << e
                                          << std::endl;
                                PRINT2( to_left[e], to_leftx[e] );
                                TracebackThisProcess( );
                            }
                            if ( to_right[e] != to_rightx[e] ) {
                                std::cout << "to_right wrong at edge " << e
                                          << std::endl;
...
void SupportedHyperBasevector::UnwindAssembly(
    ...
    if (verbosity >= 1)
        std::cout << Date() << ": entering UnwindAssembly" << std::endl;
    if (verbosity >= 3) {
        std::cout << "\nEdges in assembly:\n";
        for (int e = 0; e < EdgeObjectCount(); e++) {
            std::cout << e << " [" << EdgeLengthKmers(e) << " kmers]";
    ...
    vec<int> to_left, to_right;
    ToLeft(to_left), ToRight(to_right);
    vec<std::pair<vec<vec<int> >, vec<int> > > recs;
    if (verbosity >= 1)
        std::cout << Date() << ": start main loop" << std::endl;
    REPORT_TIME( clock1, "used in unwind setup");
    if (logc.STATUS_LOGGING) std::cout << Date( ) << ": begin unwind main" << std::endl;
    ...
    for (int e = 0; e < EdgeObjectCount(); e++) {
        ...
        for (int s = ps[0].isize(); s > 1; s--) {
            ...
            if (InvDef(edge1)) {
                ...
                std::set<int> rdels;
                for (std::set<int>::iterator i = dels.begin(); i != dels.end();
                        i++) {
                    rdels.insert(Inv(*i));
                }
                std::set<int> delsp(dels), rdelsp(rdels);
                delsp.insert(edge1), delsp.insert(edge2);
                rdelsp.insert(redge1), rdelsp.insert(redge2);
                int over = 0;
                for (std::set<int>::iterator i = delsp.begin(); i != delsp.end();
                        i++) {
                    if (Member(rdelsp, *i))
                        over++;
                }
...
//	       vec<int> dels = Betweens(*this, to_right, ps[0]);
            ...
            if (!InvDef(ps[0].front())) {
                ...
                std::pair<vec<vec<int> >, vec<int> > recp = std::pair<vec<vec<int> >,
                                                     vec<int> >(ps, dels);
                std::pair<vec<vec<int> >, vec<int> > recq;
        ...
        if (verbosity >= 2)
            out << "done, time used = " << TimeSince(clock) << std::endl;
    ...
    if (logc.STATUS_LOGGING)
        std::cout << Date( ) << ": intersecting recommendations" << std::endl;
    UniqueSort(recs);
    {
    ...
    if (verbosity >= 1) std::cout << Date() << ": processing recommendations" << std::endl;
    UniqueSort(recs);
            ...
            if (fronts[i1] == fronts[i2] && backs[i1] == backs[i2]
                ...
                if (Meet(s1, s2)) {
                    if (s1.size() > s2.size()
                        ...
                        if (verbosity >= 1) {
                            std::cout << "conflict, deleting recommendation "
                                      << i2 + 1 << "\n";
                        }
                        to_delete[i2] = True;
                    }
    ...
    for (int i1 = 0; i1 < recs.isize(); i1++)
        for (int i2 = 0; i2 < recs.isize(); i2++) {
            ...
            if (fronts[i1] != fronts[i2] || backs[i1] != backs[i2])
                continue;
            std::pair<vec<vec<int> >, vec<int> > recp1 = recs[i1], recp2 =
                        recs[i2];
            ...
            if (InvDef(fronts[i1])) {
                std::pair<vec<vec<int> >, vec<int> > recq1, recq2;
    ...
    if ( verbosity >= 1 ) std::cout << ": defining involution of recs" << std::endl;
    ...
    for (int pass = 1; pass <= 2; pass++) {
        ...
        for (int i = 0; i < recs.isize(); i++) {
            std::pair<vec<vec<int> >, vec<int> >& recp = recs[i];
    ...
    if (verbosity >= 1) {
        ...
        for (int i = 0; i < recs.isize(); i++) {
            ...
            for (int j = 0; j < recs[i].first.isize(); j++) {
                if (j > 0)
                    std::cout << "\nor";
                std::cout << " " << printSeq(recs[i].first[j], " ");
                if (logc.UNWIND_TRUTH) {
                    basevector b;
            ...
            if (logc.UNWIND_TRUTH && !placed) {
                ...
                for (int j = 0; j < splacesf.isize(); j++) {
                    ...
                    for (int l = 0; l < p.x.isize(); l++)
                        std::cout << " " << s[p.x[l]];
                    std::cout << " placed at " << p.g << "." << p.start << "-"
                              << p.stop << ":" << (p.fw ? "fw" : "rc")
                              << ".\n";
                }
            }
    ...
    if (logc.STATUS_LOGGING) {
        std::cout << Date( ) << ": instantiating " << recs.size( )
                  << " recommendations 1" << std::endl;
    }
    ...
    if (logc.STATUS_LOGGING)
        std::cout << Date( ) << ": instantiating recommendations 2" << std::endl;
    for (int i = 0; i < recs.isize(); i++) {
        if ( !ok[i] ) continue;
    ...
    if (logc.STATUS_LOGGING) std::cout << Date( ) << ": unwind cleaning up" << std::endl;
    ...
    if (verbosity >= 1) std::cout << Date() << ": leaving UnwindAssembly" << std::endl;
}
