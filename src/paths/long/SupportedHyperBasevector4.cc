...
{    
     ...
     if (logc.STATUS_LOGGING) std::cout << Date( ) << ": killing weak exits 2" << std::endl;
     ...
     {    Reverse( );
          ...
          {    int e = EdgeObjectIndexByIndexTo( v, i );
               ...
               vec< std::pair<int,int> > see; // (vertex, distance)
               see.push( v, 0 );
               Bool fail = False;
               while(1)
               {    Bool progress = False;
     ...
     if (logc.STATUS_LOGGING)
          std::cout << Date( ) << ": deleting " << dels.size( ) << " edges" << std::endl;
     DeleteEdges(dels);
          ...
          {    Reverse( );
	       ...
	       for ( int j = 0; j < Path(i).isize( ); j++ ) 
               ...
               {    if ( To(v).size( ) != 1 || From(v).size( ) != 2 ) continue;
                    ...
                    if ( logc.verb[ "KILL_WEAK_EXITS" ] >= 1 )
                    {    std::cout << "KillWeakExits2, solo, severing connection from "
                              << "edge " << e << " to edge " << f[1] << std::endl;    }
                    sever_count++;
                    AddVertices(1);
                    GiveEdgeNewFromVx( f[1], v, N( ) - 1 );    }    }
          if (logc.STATUS_LOGGING)
          {    std::cout << Date( ) << ": " << sever_count << " connections severed" 
                    << std::endl;    }    }
...
{    std::cout << Date( ) << ": initial assembly has K = " << shb.K( ) 
          << " and " << shb.EdgeObjectCount( ) << " edges" << std::endl;
     vec<int> nuni, nuni2;
             ...
             vec<int>* snp_edge_list) const
{
     ...
     if (logc.STATUS_LOGGING)
          std::cout << Date( ) << ": found " << snp_bubbles->size( ) << " bubbles" << std::endl;
     ...
     {    int n = KmerId( bases[id], L, 0 );
          ...
          {    vec<int> e = places[i].E( );
               ...
               {    
                    ...
                    break;    }    }    }
     for ( int i = 0; i < trace.isize( ); i++ )
     {    std::cout << "\ncoverage of edge " << trace[i] << std::endl;
          int n = count[i].size( );
          ...
          {    if ( !near_interesting[j] ) continue;
               if ( j > 0 && last_printed < 0 ) std::cout << "\n...\n";
               else if ( j > last_printed + 1 ) std::cout << "...\n\n...\n";
               last_printed = j;
               std::cout << j << " -->";
               int bigs = 0;
               for ( int k = 0; k < 4; k++ )
               {    if ( count[i][j][k] >= 50 ) bigs++;
                    ...
                    std::cout << " " << as_base(k) << "[" << count[i][j][k] << "]";    }
               if ( bigs >= 2 ) std::cout << " *****";
               std::cout << "\n";    }
          if ( last_printed < n - 1 ) std::cout << "...\n";    }
     std::cout << "\n";
