...
vec<std::pair<int,edit0>> AlignToEdits(const align& a, const basevector& aSeq,
const basevector& rSeq, N50Calculator* pN50Calc ) {
    vec<std::pair<int,edit0>> edits;
...
align BestAlign(const basevector& b1, const basevector& b2, std::ostream& out, int iAlignerK) {
    alignment al;
...
//        std::cout << "ForDoubled: less than half-filled, shrinking" << std::endl;
        ...
        int64_t b2_2 = std::max(int64_t(0),b1_2-padding);
        int64_t e2_2 = std::min(max_range,e1_2+padding);
        alignment al;
...
//        std::cout << "ForDoubled: more than half-filled, splitting" << std::endl;
        vec< std::pair<int64_t,size_t> > distance_index;
        distance_index.reserve(coors_hbp.size());
        for(size_t cc=0; cc<coors_hbp.size(); ++cc) {
            distance_index.emplace_back(  std::min( abs(std::get<5>(coors_hbp[cc])-range)
...
//PRINT4(be_1,ee_1,be_2,ee_2);
            ...
            int64_t b2_2 = std::max(int64_t(0),be_2-padding);
            int64_t e2_2 = std::min(max_range,ee_2+padding);
...
void PrintPath(const vec<int>& path, const vec<std::pair<int,Bool>>& hbp_to_hb,
               std::ostream& out) {
    for (size_t i = 0; i < path.size(); i++) {
        if (i!=0) out << " ";
  ...
  public:
    ...
    void UpdateSeq(bool bPopFront, bool bPushBack, int right=std::numeric_limits<int>::min()) {
        if(bPopFront) {
            ...
            for(size_t ii=0; ii+1<seq.size(); ++ii) {
                seq[ii]=seq[ii+1];
            }
...
void MakeHBPDot(std::ostream& out,const HyperBasevector&hbp,const vec<std::pair<int,Bool>>& hbp_to_hb) {
    vec<String> vl(hbp.N());
...
void AssemblyError::PrintSummary(std::ostream& out) {
    auto gaps=GetGaps();
    ...
    ,[](const std::pair<int,err_per_ref> & in ) {
        return in.second.GoodTopology();
    } );
    ...
    for(const auto& bp: GetBestPaths()) {
        std::copy(bp.begin(),bp.end(),std::ostream_iterator<int>(out,","));
        out << std::endl;
    }

    out << "EVAL_ASSEMBLY: ";

    bool bHasLGap = std::any_of(viLGap.begin(),viLGap.end(),[](int in) {
        return in!=0;
    });
    bool bHasRGap = std::any_of(viRGap.begin(),viRGap.end(),[](int in) {
        return in!=0;
    });

    if (gaps.empty() && indels.empty() && subs == 0 && !bHasLGap && !bHasRGap ) {
        out << "perfect" << std::endl;
        if(!bGoodTopology) {
    ...
    auto oss = std::ostream_iterator<int>(out, " ");
    bool prev = false;
...
void UnrollAssemblyLoops(HyperBasevector& out, const HyperBasevector& in, vec<std::pair<int,int>>& out_to_in) {
    ...
    for(int ee=0; ee<out_to_in.isize(); ++ee) {
        out_to_in[ee]=std::make_pair(ee,1);
    }
...
class GraphTransformer {
    ...
    class edge {
      public:
        edge(const edge&L,const edge&R,int K):seq(L.GetSeq()),len(L.GetLen()+R.GetLen()+1-K) {
            seq.append(R.GetSeq());
        };
        edge():seq(),len(0) {};
        edge(const vec<ELEM_T>& v,int64_t l ):seq(v),len(l) {};
        edge(const ELEM_T& e,int64_t l ):seq( {
            e
        }),len(l) {};
        ...
        const int64_t& GetLen()const {
            return len;
        };
        vec<ELEM_T>& GetSeq() {
            return seq;
        };
        const vec<ELEM_T>& GetSeq()const {
            return seq;
        };
      private:
  ...
  public:
    ...
    class elem_t {
      public:
        ...
        const SEG_ENUM& GetSeg()const {
            return seg;
        };
        friend bool operator==(const elem_t& L, const elem_t& R) {
            return L.eid==R.eid && L.seg==R.seg;
        }
      private:
        int eid; //edge id
     ...
     */
    void BreakTwoVerticesLoopBack(int64_t limit,size_t max_step=100) {
        for( size_t ss=0; BreakTwoVerticesLoopBackStep(limit) && ss<max_step; ++ss) {};
    }
    ...
    const graph_t& getNewGraph()const {
        return m_new;
    };
...
//                  << std::endl;
    ...
    vec<std::pair<int,Bool>> new_to_old( const vec<std::pair<int,Bool>>& new_seq) {
        vec<std::pair<int,Bool>> old_seq;
        old_seq.reserve(new_seq.size());
        int last_eid=-1;
        Bool bLastFlag=True;
...
bool GraphTransformer::BreakTwoVerticesLoopBackStep(int64_t limit) {
    ...
    auto HasLoopBack = [&to_left,&to_right,this](int ee) {
        ...
        return vv!=ww && std::any_of(m_new.From(ww).begin(),m_new.From(ww).end(),[&vv](int nv) {
            return nv==vv;
        });
    };
    ...
    for(const auto& candidate:candidates) {
        ...
        ,[&](int vv) {
        return frozen_vertices.find(vv)!=frozen_vertices.end();
        })) {
            continue;
        }
        if(! ( (v_in.size()>0 && w_out.size() >0) || (v_out.size()>0 && w_out.size()>0) )  ) continue;
        for(const auto& nv:v_candidates) {
        ...
        if( v_out.size()>0 && w_out.size() >0 ) {
            ...
            for(const auto eid: v_in) {
                m_new.AddEdge(to_left[eid],vv_new,m_new.EdgeObject(eid));
            }
            for(const auto eid: v_out) {
                m_new.AddEdge(vv_new,to_right[eid],m_new.EdgeObject(eid));
            }
            for(const auto eid: v_loop) {
                m_new.AddEdge(vv_new,vv_new,m_new.EdgeObject(eid));
            }

            for(const auto eid: w_in) {
                m_new.AddEdge(to_left[eid],ww_new,m_new.EdgeObject(eid));
            }
            for(const auto eid: w_out) {
                m_new.AddEdge(ww_new,to_right[eid],m_new.EdgeObject(eid));
            }
            for(const auto eid: w_loop) {
                m_new.AddEdge(ww_new,ww_new,m_new.EdgeObject(eid));
            }

            for(const auto eid: bck) {
                m_new.AddEdge(ww_new,vv_new,m_new.EdgeObject(eid));
            }
...
void GraphTransformer::AbsorbShortEdges(int64_t limit) {
    ...
    auto GetLongestNeighbor = [&to_left,&to_right,this](int ee) {
        ...
        const int vv = to_left[ee];
        const int ww = to_right[ee];
        bool bHasLoop =   std::any_of(m_new.From(ww).begin(),m_new.From(ww).end(),[&](int nv) {
            return nv==ww || nv==vv;
        })
        && std::any_of(m_new.To(vv).begin(),m_new.To(vv).end(),[&](int nv) {
            return nv==ww || nv==vv;
        }) ;
                ...
                if(std::get<0>(info)>0) {
                    ...
                    if(short_eids.size()>0) {
                        if(std::get<1>(info)==1) { // to the right
                            ...
                            ,[&](int nv) {
                            return frozen_vertices.find(nv)!=frozen_vertices.end();
                            }
                                           )) {
                                ...
                                if(bDeleteVertex) {
                                    v_delete.push_back(ww);
                                } else {
                                    e_delete.append(short_eids);
                                }
                                bLoop=true;
                            }
                        } else { //to the left
...
AssemblyError EvalAssemblyCore( const ref_data& ref, const HyperBasevector& hb,
                                const vec<int>& inv, std::ostream& out, int iSWAk, int verbosity,
    ...
    if ( verbosity >= 1 ) out << Date() <<":b4 AlignEdgesToRefExp<L>" << std::endl;
    edge_placements.AlignEdgesToRefExp<L> (verbosity, out);
    if ( verbosity >= 1 ) out << Date() <<":after AlignEdgesToRefExp<L>" << std::endl;
    if ( verbosity >= 1 ) out << Date() <<":b4 RemoveBadPlacements()" << std::endl;
    edge_placements.RemoveBadPlacements();
    if ( verbosity >= 1 ) out << Date() <<":after RemoveBadPlacements()" << std::endl;
    if ( verbosity >= 1 ) out << Date() <<":b4 TwiddleSmart()" << std::endl;
    edge_placements.TwiddleSmart();
    if ( verbosity >= 1 ) out << Date() <<":after TwiddleSmart()" << std::endl;
    vec< vec<int> > spaths;
    ...
    */
    AssemblyError res;
//    {
//        std::ofstream ofs("bayo_ref.fasta");
    ...
    for ( int g = 0; g < (int) edge_placements.G.size( ); g++ ) {
        ...
        if(!linear_ref.IsDoubled(g)) { // if reference is not doubled, due to circular reference
            ...
            for (size_t i = 0; i < path_segs.size(); i++) {
                ...
                if ( verbosity >= 1 ) out << Date() <<":before long-to-long alignment" << std::endl;
                aligns[i] = BestAlign(seq_seg, edge_placements.G[g], out, iSWAk);
                if ( verbosity >= 1 ) out << Date() <<":after long-to-long alignment" << std::endl;
...
//                              << std::std::endl;
                ...
                iFront = std::min(iFront,aligns[i].pos2());
                iBack  = std::max(iBack,aligns[i].Pos2());
            }
...
AssemblyError EvalAssembly( const ref_data& ref, const HyperBasevector& hb,
                            const vec<int>& inv, std::ostream& out, int iSWAk, int verbosity,
                            N50Calculator* pN50Calc ) {
    if ( verbosity >= 1 ) out << Date() << ":EvalAssembly" << std::endl;
    AssemblyError  res;
    ...
    if(has_circular) {
        ...
        for(auto& entry: res) {
            ...
            vec<std::pair<int,Bool>> bp_old;
            bp_old.reserve(bp_new.size());
            for(const auto& entry: bp_new) {
                bp_old.insert(bp_old.end()
                              ,new_to_old[entry.first].second
                              ,std::make_pair(new_to_old[entry.first].first,entry.second));
            }
         ...
         */
        if ( verbosity >= 1 ) out << Date() << ":calling GraphTransformer" << std::endl;
        ...
        if(verbosity>=1) {
            std::ofstream ofs("gt_init.dot");
            engine.MakeDot(ofs);
        }
        if ( verbosity >= 1 ) out << Date() << ":b4 GraphTransformer::ble" << std::endl;
        engine.BreakLongEdges(absorb_limit, break_tgt);
        if ( verbosity >= 1 ) out << Date() << ":after GraphTransformer::ble" << std::endl;
        if(verbosity>=1) {
            std::ofstream ofs("gt_ble.dot");
            engine.MakeDot(ofs);
        }
        if ( verbosity >= 1 ) out << Date() << ":b4 GraphTransformer::2vlb" << std::endl;
        engine.BreakTwoVerticesLoopBack(absorb_limit);
        if ( verbosity >= 1 ) out << Date() << ":after GraphTransformer::2vlb" << std::endl;
        if(verbosity>=1) {
            std::ofstream ofs("gt_2loop.dot");
            engine.MakeDot(ofs);
        }
        if ( verbosity >= 1 ) out << Date() << ":b4 GraphTransformer::ase" << std::endl;
        engine.AbsorbShortEdges(absorb_limit);
        if ( verbosity >= 1 ) out << Date() << ":after GraphTransformer::ase" << std::endl;
        if(verbosity>=1) {
            std::ofstream ofs("gt_abs.dot");
            engine.MakeDot(ofs);
        }
        if ( verbosity >= 1 ) out << Date() << ":b4 GraphTransformer::use" << std::endl;
        engine.UnrollShortLoops(unroll_limit);
        if ( verbosity >= 1 ) out << Date() << ":after GraphTransformer::use" << std::endl;
        if(verbosity>=1) {
            std::ofstream ofs("gt_unroll.dot");
            engine.MakeDot(ofs);
        }
        if ( verbosity >= 1 ) out << Date() << ":getting new edges" << std::endl;
        ...
        for(auto& entry: res) {
            ...
            /*
            const auto& bp_new = err_per_ref.GetBestPath();
            vec<std::pair<int,Bool>> bp_old; bp_old.reserve(bp_new.size());
            for(const auto& entry: bp_new){
                const auto& seq = engine.new_to_old(entry.first);
                if( entry.second ){ //if forward
                    std::transform(seq.begin(),seq.end(),std::back_inserter(bp_old),[&](int i){return std::make_pair(i,entry.second);});
                }
                else{
                    std::transform(seq.rbegin(),seq.rend(),std::back_inserter(bp_old),[&](int i){return std::make_pair(i,entry.second);});
                }
            }
            err_per_ref.SetBestPath(bp_old);
            */
...
//        res = EvalAssemblyCore( ref, hb, inv, out, iSWAk, verbosity );
    ...
    if ( verbosity >= 1 ) out << Date() << ":EvalAssembly done " << std::endl;
    return res;
}
...
//    std::cout << std::count(is_del.begin(),is_del.end(),true) << " bases not covered" << std::endl;
    ...
    for(const auto&entry:indels) {
        ...
        if( entry.second > 0 )   {
            ins_count[entry.first]+=entry.second;
        } else if(entry.second < 0) {
            std::fill(is_del.begin()+entry.first,is_del.begin()+entry.first-entry.second,true);
        }
    }
//    std::cout << std::count(is_del.begin(),is_del.end(),true) << " bases not covered" << std::endl;
...
//                  << std::endl;
    ...
    for(int64_t bb=0; bb<range; ++bb) {
        nError += is_sub[bb] + is_del[bb] + ins_count[bb];
    }
    ...
    if(min_error==std::numeric_limits<int64_t>::max()) {
        ...
        viLGap.push_back( std::min(-1,-int(base_coor[best_front_linear].second)) );

        auto end_element = base_coor[(best_front_linear+range)%max_range];
        viRGap.push_back( std::min(-1,-int( std::get<2>(coors_hbp[end_element.first]) - end_element.second )) );
    ...
    best_seq.erase(std::remove_if(std::begin(best_seq),std::end(best_seq),[](int64_t in) {
        return in < 0;
    }),std::end(best_seq));
