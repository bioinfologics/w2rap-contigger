...
template<int K> void MakeBlocks(
    ...
    vec< std::pair<kmer<K>,int> > pkmers0;
    ...
    vec< std::pair<kmer<K>,int> > qkmers0;
    vec<int> rep_id;
    for ( int j = 0; j < reps.isize( ); j++ ) {
        vec<int> o;
        e.Orbit( reps[j], o );
    ...
    for ( int id = 1; id < N; id++ ) {
        vec< std::pair<kmer<K>,int> > tkmers0;
    ...
    vec< std::pair<basevector,int> > pkmers( pkmers0.size( ) );
...
redelete:
    ...
    if (logc.PRINT_BLOCK_ERROR_RATES) {
        ...
        for ( int id = 0; id < N; id++ ) {
            ...
            out1 << std::setiosflags(std::ios::fixed) << std::setprecision(1) << 100.0 * err[id]
            ...
            for ( int m = 0; m < blocks.isize( ) - 1; m++ ) {
                if ( block_defined[id][m] && block_defined[id][m+1] ) {
                    ...
                    if ( !( block_stop[id][m] <= block_start[id][m+1] ) ) {
                        if ( logc.verb[ "ULTRA" ] >= 2 )
                            out << "working around weird failure" << std::endl;
                        break;
                    }
    ...
    if ( logc.verb[ "ULTRA" ] >= 1 && gread0.isize( ) > 0 ) {
        ...
        for ( int m = 0; m < block_pos.isize( ); m++ ) {
            ...
            std::ostringstream out2, out3, out4, out5;
            if ( gkmers.nonempty( ) ) {
            ...
            if ( block_defined[0][m] ) {
                ...
                out << std::setiosflags(std::ios::fixed) << std::setprecision(1) << 100.0
                    * block_err_rate[0][m] << std::resetiosflags(std::ios::fixed);
            } else {
                out3 << "---";
                out5 << "---";
            }
...
template void MakeBlocks<16>( const int ec, const vecbasevector& reads,
                              const vec< vec< std::pair<int,int> > >& a, threaded_blocks& tb,
                              const basevector& gread0, const vec<basevector>& gkmers, const int gid,
                              const int gstart, std::ostream& out, const Bool FILTER_BAD_BLOCKS,
