...
template<int K> void GetFriendsAndAlignsInitial(
    ...
    vec< vec< std::pair<int,int> > >& a,        // kmer aligns of gang to read 0
    ...
    if ( logc.PRINT_TIME_USED && omp_get_thread_num( ) == 0 )
        out << TimeSince(match_clock) << " used computing kmer matches" << std::endl;
    ...
    for ( int p = 0; p <= gang[0].isize( ) - K; p++ )
        a[0].push( p, p );
    if ( logc.PRINT_TIME_USED && omp_get_thread_num( ) == 0 )
        out << TimeSince(align_clock) << " used aligning" << std::endl;
    ...
    if ( logc.PRINT_TIME_USED && omp_get_thread_num( ) == 0 )
        out << TimeSince(err_clock) << " used filtering based on errors" << std::endl;
    ...
    if ( logc.PRINT_TIME_USED && omp_get_thread_num( ) == 0 )
        out << TimeSince(cclock) << " used in column filtering" << std::endl;
    ...
    vec< std::pair<int,int> > gang_start;
    vec<int> colsp;
    ...
    if (logc.PRINT_GANG_LOCS) {
        ...
        out << "\nfound " << true_friends << " true friends"
            << " and " << false_friends << " false friends" << std::endl;
    }
    ...
    if ( logc.PRINT_TIME_USED && omp_get_thread_num( ) == 0 )
        out << TimeSince(delete_clock) << " used deleting reads" << std::endl;
}
...
template void GetFriendsAndAlignsInitial<16>( const IAndOsVec& F,
        ...
        vec< vec< std::pair<int,int> > >& a, std::ostream& out, const ConsensusScoreModel&,
