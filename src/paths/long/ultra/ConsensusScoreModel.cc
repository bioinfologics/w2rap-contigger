...
void ConsensusScoreModel::PrintScoreMatrix(  const BaseVec& a0, const BaseVec& b0 ) const {
    ...
    std::cout << "len_head= " << len_head << std::endl;
    std::cout << "len_tail= " << len_tail << std::endl;
...
int ConsensusScoreModel::ScoreFast( const BaseVec& a0, const BaseVec& b0, String* pt_align_a, String* pt_align_b ) const 
{
    ...
    //for ( size_t x = 0; x < s.size(); ++x ) {
    //    std::cout << "row " << x << ": ";
    //    copy( s[x].begin(), s[x].end(), ostream_iterator<int>(std::cout," ") );
    //    std::cout << std::endl;
    //}
        ...
        vec< std::pair<int,edit0> >* loc_edits ) const 
{
    ...
    std::map< std::pair<int,edit0>, vec<int> > vote_pool; // record the votes
    for ( size_t i = 0; i < threads.size(); ++i ) {
    ...
    vec <int> counts;
    for( std::map< std::pair<int,edit0>, vec<int> >::iterator it = vote_pool.begin();
            it != vote_pool.end(); it++ ) {
...
void FastScorer::GetLocEdits( vec< std::pair<int, edit0> > *p_loc_edits, vec<unsigned int> * p_counts, unsigned int min_vote ) const
{
    ...
    for ( size_t ithread = 0; ithread < threads_.size(); ++ithread ) {
        if ( !valid_[ithread] ) continue;
        vec< std::pair<int, edit0> > new_loc_edits;
        ...
        for ( size_t i = 0; i < new_loc_edits.size(); ++i ) {
            ...
            if ( p >= read0_.size() && e.etype != INSERTION ) {
                std::cout << "Edit out of range " << std::endl;
                String str = "sub";
...
void FastScorer::AlignToLocEdits( const String & align_a, const String & align_b, vec< std::pair<int,edit0> >* loc_edits )
{
    ...
    while ( pos < align_a.size() ) {
        ...
        if ( align_a[pos] == '-' ) {
            ...
            while ( next_pos < align_a.size() && align_a[next_pos] == '-' ) next_pos++;
            String seq = align_b.substr( pos, next_pos - pos );
            loc_edits->push_back( std::make_pair(actual_pos, edit0( INSERTION, seq)) );
            pos = next_pos;
