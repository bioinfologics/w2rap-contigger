        ...
        const long_logging& logc, const long_heuristics& heur )
{
     ...
     vec< vec< std::pair<int,int> > > subs_index( EdgeObjectCount( ) );
     ...
     {    int e = subs[i].first;
          ...
          {    int id = aux.paths_index[e][j].first;
               ...
               {    int r = BinPosition( aux.paths_index[ q[l] ], std::make_pair(id,l) );
                    if ( r >= 0 ) 
               ...
               q = qnew;
               for ( int l = pos; l < q.isize( ); l++ )
               {    aux.paths_index[ q[l] ].push_back( std::make_pair(id,l) );
          ...
          {    int id = subs_index[e][j].first, pos = subs_index[e][j].second;
               ...
               for ( int l = pos; l < q.isize( ); l++ )
               {    int r = BinPosition( subs_index[ q[l] ], std::make_pair(id,l) );
                    if ( r >= 0 ) 
               ...
               q = qnew;
               for ( int l = pos; l < q.isize( ); l++ )
               {    subs_index[ q[l] ].push_back( std::make_pair(id,l) );
                    Sort( subs_index[ q[l] ] );    }
               subs_index[e].clear( );    }    }
     ...
     vec< vec< std::pair<fix64_6,int64_t> > > new_weights_fw_origin;
     vec< vec< std::pair<fix64_6,int64_t> > > new_weights_rc_origin;
     ...
     */
     ...
     {    const vec<int> &p1 = PairLeft(i1), &q1 = PairRight(i1);
          ...
          if ( i2 < 0 )
          {    PairsMutable( ).push_back( std::make_pair( q2, p2 ) );
        ...
        const long_logging& logc )
{
    ...
    while ( progress )
    {
        ...
        if ( verbosity )
            std::cout << Date() << ": starting iteration "
        ...
        // Process joins.
        std::cout << Date() << ": process joins" << std::endl;
        ...
        {    std::cout << Date() << ": processing " << joins.size() << " potential joins"
                  << std::endl;    }
        if ( verbosity >= 2 )
            std::cout << "\n";
        ...
        for ( int i = 0; i < joins.isize(); i++ )
        {
            ...
            for ( int p = 0; p < proc.isize(); p++ )
            {
                ...
                {
                    if ( verbosity >= 2 )
                        std::cout << "aborting join" << std::endl;
                    continue;
                }
                progress = true;
        ...
        // Clean up.
        std::cout << Date() << ": clean up" << std::endl;
...
void SupportedHyperBasevector::Gulp( const long_logging_control& log_control, 
     const long_logging& logc )
{    double clock1 = WallClockTime( );
     if (logc.STATUS_LOGGING) std::cout << Date( ) << ": gulping edges" << std::endl;
     vec<int> to_left, to_right;
     ToLeft(to_left), ToRight(to_right);
     ...
     while(1)
     {    if ( logc.verb[ "GULP" ] >= 1 ) 
               std::cout << Date( ) << ": starting gulp iteration" << std::endl;
          Bool changed = False;
          for ( int v = 0; v < N( ); v++ )
     ...
     FixWeights(logc);
     if (logc.STATUS_LOGGING)
     {    std::cout << Date( ) << ": after gulping edges there are "
               << EdgeObjectCount( ) << " edges" << std::endl;    }
     TestValid(logc);    }

void SupportedHyperBasevector::Ungulp( const long_logging& logc )
{    double clock = WallClockTime( );
     if (logc.STATUS_LOGGING) std::cout << Date( ) << ": ungulping edges" << std::endl;
     const int max_gulp = 20;
     vec<int> to_left, to_right;
     ToLeft(to_left), ToRight(to_right);
     int nv = N( );
     ...
     RemoveDeadEdgeObjects( );
     if (logc.STATUS_LOGGING) 
     {    std::cout << Date( ) << ": after ungulping there are " << EdgeObjectCount( ) 
               << " edges" << std::endl;    }
     TestValid(logc);    }
     ...
     const long_logging& logc )
{
     ...
     if (logc.STATUS_LOGGING)
     {    std::cout << Date( ) << ": see " << EdgeObjectCount( ) 
               << " edges, removing hanging ends" << std::endl;    }
