    ...
    for ( int pass = 1; pass <= npasses; pass++ ) {
        ...
        if ( heur.PREFAB ) {
            if ( pass == 1 ) {
                ...
                std::cout << Date() <<  ": Selected  " << rid1.size()
                          << " reads for correction" << std::endl;
                ridx = rid1;
            ...
            if ( pass == 2 ) {
                ...
                if ( n_pre_corrected * 10 < reads.size() ) {
                    std::cout << "Warning: than 10% of the reads are pre-corrected"
                              << std::endl;
                }
...
// here's a completely deterministic version
        ...
        */
        ...
        std::cout << Date( ) << ": starting main loop" << std::endl;
        int count = 0;
        ...
        for ( int bi = 0; bi < batches.isize( ); bi++ ) {
            ...
            for ( int j = 0; j < batches[bi].isize( ); j++ ) {
                ...
                std::ostream& xout = ( !logc.DIRECT ? out : std::cout );
                if ( logc.verb[ "ULTRA" ] >= 1 )
                    xout << "\ncorrecting read " << ec << " of " << N << std::endl;
                if (logc.ANNOUNCE) {
                    #pragma omp critical
                    {    std::cout << "begin read " << ec << std::endl;    }
                }

                // Get friends and kmer alignments of the gang to read 0.

                vecbasevector gang;
                vec< vec< std::pair<int,int> > > a;
                vec<int>* p_friends
                ...
                if ( logc.verb[ "ULTRA" ] >= 1 )
                    xout << "found " << gang.size( ) - 1 << " friends" << std::endl;
            ...
            if (logc.BATCH_TRACKING) {
                #pragma omp critical
                {
                    std::cout << "\n" << Date( ) << ": pass = " << pass
                    << ", end batch " << bi
                    << ", time used = " << TimeSince(biclock) << std::endl;
                    for ( int j = 0; j < batches[bi].isize( ); j++ ) {
                        int xi = batches[bi][j];
                        int ec = ridx[xi];
                        PRINT4( j, xi, ec, F[ec].size( ) );
        ...
        std::cout << Date( ) << ": main loop complete, time used = "
                  << TimeSince(mclock) << std::endl;
