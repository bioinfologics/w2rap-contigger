...
int ReadOverlapGraph::RemoveTransitiveFrom(int rid, int alt, vec< vec<bool> >& visited )
{
    ...
    while ( ! que.empty() ) {
        ...
        for ( size_t k = 0; k < froms_[i][j].size(); ++k )
            que.push( std::make_pair( froms_[i][j][k].rid2, froms_[i][j][k].alt2 ) );
    }
    return count;
}
...
void ReadOverlapGraph::CombineAmbReads( const vec< vec<Ambiguity> >& creads_ambs ) 
{
    ...
    for ( size_t i = 0; i < reads_.size(); ++i ) {
        ...
        //std::cout << "check read " << i << " alts= " << reads_[i].size() << std::endl;
        for ( size_t j1 = 0; j1 < reads_[i].size(); ++j1 ) {
            for ( size_t j2 = j1+1; j2 < reads_[i].size(); ++j2 ) {
                ...
                { // check the froms
                    ...
                    Sort(edges2);
                    if ( edges1 != edges2 ) {
                        //std::cout << "froms mismatch from " << j1 << " to " << j2 << std::endl;
                        mismatch = true;
                        break;
                    }
                }
                { // check the tos
                    ...
                    Sort(edges2);
                    if ( edges1 != edges2 ) {
                        //std::cout << "tos  mismatch from " << j1 << " to " << j2 << std::endl;
                        mismatch = true;
                        break;
                    }
                }
    ...
    //std::cout << "Combined " << combined_.CountValue(true) << " amb reads " << std::endl;
}
