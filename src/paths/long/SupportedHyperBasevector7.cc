    ...
    for ( int pi = 0; pi < to_use; pi++ ) {
        ...
        if ( paths_index.nonempty( ) && p != p_old ) {
            for ( int l = 0; l < p_old.isize( ); l++ ) {
                ...
                #pragma omp critical
                {
                    auto low = LowerBound( paths_index[e],
                                           {i,0} );
...
void SupportedHyperBasevector::Bootstrap( const SupportedHyperBasevector& shb0,
    ...
    if (logc.STATUS_LOGGING) std::cout << Date( ) << ": begin bootstrap" << std::endl;
    ...
    vec< vec< std::pair<int,int> > > cindex( hb_orig.EdgeObjectCount( ) );
    ...
    if (logc.STATUS_LOGGING)
        std::cout << Date( ) << ": mapping reads onto assembly" << std::endl;
    ...
    vec< std::pair< vec<int>, vec<int> > >& usuy = shb.PairsMutable( );
    vec< vec<pair_point> >& county = shb.PairDataMutable( );
    usuy.clear( ), county.clear( );
    vec< std::pair< std::pair< vec<int>, vec<int> >, vec<pair_point> > > all_fullsp;
    for ( int i = 0; i < shb0.Pairs( ).isize( ); i++ ) {
        ...
        for ( int j1 = 0; j1 < fulls1.isize( ); j1++ )
            for ( int j2 = 0; j2 < fulls2.isize( ); j2++ ) {
                ...
                all_fullsp.push( std::make_pair( fulls1[j1], fulls2[j2] ), p );
            }
    }
    Sort(all_fullsp);
...
void SupportedHyperBasevector::UnrollLoops( const long_logging& logc ) {
    double clock = WallClockTime( );
    if (logc.STATUS_LOGGING) std::cout << Date( ) << ": unrolling loops" << std::endl;
    vec< std::pair< triple<int,int,int>, vec<int> > > edits;
    vec<int> to_left, to_right;
    ToLeft(to_left), ToRight(to_right);

    // Define proposed edits.

    vec< std::pair< triple<int,int,int>, vec<int> > > plans;
    for ( int v = 0; v < N( ); v++ ) {
        if ( To(v).size( ) != 2 || From(v).size( ) != 2 ) continue;
        int i1 = 0, i2 = 1, j1 = 0, j2 = 1;
        if ( To(v)[i1] != v ) std::swap(i1, i2);
        if ( To(v)[i1] != v ) continue;
        if ( From(v)[j1] != v ) std::swap(j1, j2);
    ...
    for ( int j = 0; j < plans.isize( ); j++ ) {
        ...
        for ( int pass = 1; pass <= npasses; pass++ ) {
            ...
            if ( logc.verb[ "UNROLL_LOOPS" ] >= 1 ) {
                std::cout << "replacing loop " << fx << " by " << printSeq(counts)
                          << " copies of it" << std::endl;
            }
    ...
    if (logc.STATUS_LOGGING) {
        std::cout << Date( ) << ": after unrolling loops there are "
...
void SupportedHyperBasevector::DeleteWeakEdges( const long_logging& logc ) {
    ...
    if ( verbosity >= 1 ) {
        std::cout << Date( ) << ": begin deleting weak edges" << std::endl;
        int64_t npe = 0;
        for ( int i = 0; i < NPaths( ); i++ )
            npe += Path(i).size( );
        std::cout << Date( ) << ": edges = " << EdgeObjectCount( )
                  << "; paths = " <<  NPaths( )
                  << "; segs = " << npe << std::endl;
    }
    ...
    while(1) {
        ...
        if ( verbosity >= 1 ) std::cout << Date( ) << ": defining adjacencies" << std::endl;
        ...
        vec<int64_t> ind( ne + 1, -1 ), indb( ne + 1, -1 );
        {
            ...
            if ( verbosity >= 1 ) std::cout << Date( ) << ": appending" << std::endl;
            ...
            if ( verbosity >= 1 )
                std::cout << Date( ) << ": adding trivial adjacencies" << std::endl;
            vec<int> to_right;
            ToRight(to_right);
            for ( int e = 0; e < ne; e++ ) {
                int w = to_right[e];
            ...
            for ( int64_t i = 0; i < adjb.jsize( ); i++ )
                std::swap( adjb[i].first, adjb[i].second );
            ParallelSort(adjb);
        ...
        RemoveEdgelessVertices( );
        if ( verbosity >= 1 ) std::cout << Date( ) << ": removing unneeded verts" << std::endl;
        RemoveUnneededVertices( );
        if ( verbosity >= 1 ) std::cout << Date( ) << ": tail" << std::endl;
        RemoveDeadEdgeObjects( );
        if ( verbosity >= 1 ) {
            int64_t npe = 0;
            for ( int i = 0; i < NPaths( ); i++ )
