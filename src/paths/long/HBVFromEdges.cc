...
class EdgeEnd {
  public:
    ...
    bvec::SwitchHitterIter begin() const {
        return mItr;
    }
    bvec::SwitchHitterIter end() const {
        return mItr+mK;
    }

    struct Hasher {
        typedef EdgeEnd argument_type;
        size_t operator()( EdgeEnd const& ee ) const {
            return ee.getHash();
        }
    };
...
class Vertex : public EdgeEnd {
  ...
  public:
    ...
    IandO const* begin() const {
        return mEdges;
    }
    IandO const* end() const {
        return mEdges+mNEdges;
    }
...
class VertexDictBuilder {
  public:
    ...
    void map( size_t edgeId, OItr oItr ) {
        ...
        *oItr = EdgeEnd(&edge,false,false,mKLO);
        ++oItr;
        *oItr = EdgeEnd(&edge,false,true,mKLO);
        ++oItr;
        if ( edge.getCanonicalForm() != CanonicalForm::PALINDROME ) {
            *oItr = EdgeEnd(&edge,true,false,mKLO);
            ++oItr;
            *oItr = EdgeEnd(&edge,true,true,mKLO);
            ++oItr;
        }
    }
...
class HBVBuilder {
  ...
  public:
    HBVBuilder( vecbvec const& edges, unsigned K, VertexDict const& dict,
        ...
        mFwd.clear();
        mFwd.resize(nEdges,NULL_EDGEID);
        mRev.clear();
        mRev.resize(nEdges,NULL_EDGEID);
    }
