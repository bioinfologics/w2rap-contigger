...
{    
     ...
     std::cout << Date( ) << ": chunking edges" << std::endl;
     ...
     {    const vec<int>& v = chunks[ci];
          if (verbose) 
          {    std::cout << "processing " << printSeq(v);
               if ( InvDef( v[0] ) )
               {    vec<int> rv;
                    for ( int j = 0; j < v.isize( ); j++ )
                         rv.push_back( Inv(v[j]) );
                    rv.ReverseMe( );
                    std::cout << " (inv = " << printSeq(rv) << ")";    }
               std::cout << std::endl;    }
          ...
          {    int id = paths_index[x1][i].first, pos1 = paths_index[x1][i].second;
               ...
               if (verbose) std::cout << id << ": 1 replacing " << printSeq( Path(id) );
               int pos2;
               ...
               for ( int j = pos1 + 1; j < pos2; j++ )
                    PathMutable(id)[j] = -1;
               if (verbose) std::cout << " by " << printSeq( Path(id) ) << std::endl;
               edited.push_back(id);    }
          ...
          {    int id = paths_index[xn][i].first, pos1 = paths_index[xn][i].second;
               ...
               if (verbose) std::cout << id << ": 2 replacing " << printSeq( Path(id) );
               int pos2;
          ...
          {    int xi = v[r];
               if (verbose) std::cout << "looking at xi = " << xi << std::endl;
               Bool compat = True;
               ...
               {    int id = paths_index[xi][i].first; 
                    ...
                    if ( Path(id,pos) != xi ) continue;
                    if (verbose) 
                         std::cout << id << ": 3 replacing " << printSeq( Path(id) );
          ...
          {    int xi = v[i], xj = v[j];
               ...
               {    int id = paths_index[xi][l].first; 
                    ...
                    if ( !Path(id).Contains( w, pos ) ) continue;
                    if (verbose) 
                         std::cout << id << ": 4 replacing " << printSeq( Path(id) );
                    PathMutable(id,0) = EdgeObjectCount( ) - 1;
                    for ( int j = 1; j < Path(id).isize( ); j++ )
                         PathMutable(id,j) = -1;    
                    if (verbose) std::cout << " by " << printSeq( Path(id) ) << std::endl;
     ...
     UniqueOrderPaths( );
     std::cout << Date( ) << ": after chunking there are "
