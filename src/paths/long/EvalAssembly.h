...
public:
    ...
    public:
        ...
        void SetBestPath(const vec<std::pair<int,Bool>>& in) {best_path=in;};
        const vec<std::pair<int,Bool>>& GetBestPath()const{return best_path;};
        template<typename _OIter>
        void LogBestPath(_OIter itr)const{
            std::transform(best_path.begin(),best_path.end(),itr,[](const std::pair<int,Bool>&in){return in.first;});
        }
