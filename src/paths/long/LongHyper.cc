...
Bool LongHyper(const VecEFasta &correctede, const vec<pairing_info> &cpartner, SupportedHyperBasevector &shb,
    ...
    vec<vec<std::pair<fix64_6, int64_t> > > weights_fw_origin, weights_rc_origin;
    {
        ...
        if (heur.REMAP_READS) {
            ...
            std::cout << Date() << ": remapping reads" << std::endl;
        ...
        if (heur.PERTURB_TRANSLATIONS && Bases.size() > 0) {
            ...
            vec<vec<std::pair<int, int> > > usu_index(hb.EdgeObjectCount());
            ...
            for (int i = 0; i < nus; i++) {
                        ...
                        for (int l1 = 0; l1 < usu_index[u1].isize(); l1++)
                            for (int l2 = 0; l2 < usu_index[u2].isize(); l2++) {
                                ...
                                if (usu_index[u1][l1].second >
                                    usu_index[u2][l2].second) { continue; }
                                vec<int> w;
                                for (int m = usu_index[u1][l1].second;
                                     m <= usu_index[u2][l2].second; m++) { w.push_back(usu[pid][m]); }
                ...
                for (int j = 0; j < where.isize(); j++) {
                    ...
                    if (hb.From(to_right[u1]).nonempty()
                        && p1 > hb.EdgeObject(u1).isize() - K2) { continue; }
                ...
                if (where.size() > 1 || where[0].first != target) {
                    ...
                    for (int j = 0; j < where.isize(); j++)
#ifdef __APPLE__
                            ps.push_back(expl(-Q[j] / 10.0 * logl(10)));
#else
                    ...
                    if (verbose) {
                        //#pragma omp critical
                        {
                            ...
                            PRINT(int(fwplace[i]));
                            std::cout << "translates to " << printSeq(target)
                                      << std::endl;
                            std::cout << "start1:\n";
                            for (unsigned j1 = 0; j1 < locs[n1].size(); j1++) {
                                int u1 = locs[n1][j1].first;
                            ...
                            for (int j = 0; j < where.isize(); j++) {
                                ...
                                for (int l = 1; l < where[j].first.isize();
                                     l++) { std::cout << "," << where[j].first[l]; }
#ifdef __APPLE__
                                long double p = expl(-Q[j] / 10.0 * logl(10)) / psum;
#else
                                long double p = exp10l( -Q[j]/10.0 ) / psum;
#endif
                                fix64_6 weight(p * 1000000, 1000000);
                                std::cout << "." << where[j].third
                                          << " [Q=" << Q[j] << ",w=" << weight
                                          << "]";
