...
{    Ifstream( hin, "/wga/dev/references/Homo_sapiens/NA12878_Fosmid_Pool.regions" );
     ...
     {    getline( hin, line );
          if ( hin.fail( ) )
          {    std::cout << "failed to find Fosmid " << ID << std::endl;
               _exit(1);    }
...
{    vec<String> regions;
     ...
     vec< std::pair<String,ho_interval> > regs;
     for ( auto const fid : AllFosmids() )
     {    int g, rstart, rstop;
          String gid, loc;
          GetRegionInfo(ToString(fid), g, rstart, rstop, gid, loc );
          rstart -= padding; rstart = std::max( rstart, 0 );
          rstop += padding; rstop = std::min( rstop, sizes[g] );
