...
public:
    EdgePlacements(const HyperBasevector& hbp, const vec<std::pair<int,Bool>>& hbp_to_hb,
    ...
    void AlignEdgesToRef( 
         ...
         // logging:
              bool REFTRACE_VARIANTS, const int verbosity, std::ostream& out );

    template <int L>
    void AlignEdgesToRefExp(const int verbosity, std::ostream& out);
...
public:
    ...
    vec< quad< triple<int,int,int>, triple<int,int,int>, 
        std::pair<int,int>, std::pair<int,int> > > vedata;
    vec<align> aligns;
...
public:
    ...
    vec< triple< int, int, std::pair<int,int> > > edges;
    vec< triple<int,int,int> > egd; 
    digraphE<int> Z;
private:
    void BuildGraph(const int verbosity, std::ostream& out);

    void AddGapEdges(const int min_dist, const int max_dist, const int verbosity, 
            std::ostream& out ,const bool bPreserveDisconnectedComponents=false);
          ...
          bool REFTRACE_VARIANTS, const int verbosity, std::ostream& out )
{
     ...
     {    const basevector& e = hbp.EdgeObject(i);
          ...
          vec< triple< int, int, std::pair<int,int>  > > og;
          ...
          {    int k;
               ...
               og.push( k - j, offset[j].first, 
                    std::make_pair( offset[j].second, offset[k-1].second ) );
               j = k - 1;    }
          ReverseSort(og);
          if ( verbosity >= 4 )
          {    
          ...
          {    
               ...
               {    out << "\noffsets for edge " << i << " (hb_edge=" 
                         << hbp_to_hb[i].first << ", nkmers=" << nkmers << ")" << std::endl;
                    for ( int j = 0; j < og.isize( ); j++ )
                    {    out << "[" << j << "] " << og[j].second << "."
          ...
          {    int g = og[j].second;
               ...
               if ( !REFTRACE_VARIANTS )
               {
                   ...
                   if (SMA_method == 1) {
                       ...
                       std::cout << "unrecognized SMA_method" << std::endl;
                   }
               ...
               if ( verbosity >= 3 )
               {
                    ...
                    {    out << "\nalignment " << j << " of edge " << i << " ("
                              ...
                              << ", hb_edge=" << hbp_to_hb[i].first << ")" << std::endl;
                         vec<int> errs 
               ...
               {    vedata.push( make_triple( g, a.pos2( ), xto_left[i] ),
                         ...
                         std::make_pair( i, errors ), std::make_pair( a.pos1( ), a.Pos1( ) ) );
                    aligns.push_back(a);    }    }    }    
...
template<int L> 
void EdgePlacements::AlignEdgesToRefExp(const int verbosity, std::ostream& out)
{
     // Setup for alignment.
     vecbasevector all(G);
     vec< triple<kmer<L>,int,int> > kmers_plus;
     ...
     for(size_t gg=1;gg<G.size();++gg){max_g_len=std::max(max_g_len,G[gg].size());}
     ...
     typedef triple< int, int, std::pair<int,int> >  og_type;   // the og specification from old code
...
//                   errors += hbp.EdgeObject(i).size()-a.Pos1();
                   ...
                   {    vedata.push( make_triple( g, a.pos2( ), xto_left[indices.first] ),
                             ...
                             std::make_pair( indices.first, errors ), std::make_pair( a.pos1( ), a.Pos1( ) ) );
                        aligns.push_back(a);    }
              }    
          }
     }    
