     ...
     vec< triple<int,int,Bool> >& offset_id_rc2, const Bool verbose )
{
     ...
     {
          #pragma omp critical
          {    std::cout << "raising q score for read " << id1 << ", bases ";
               for ( int i = 0; i < raises.isize( ); i++ )
               {    if ( i > 0 ) std::cout << ", ";
                    int j;
                    for ( j = i + 1; j < raises.isize( ); j++ )
                         if ( raises[j] - raises[j-1] != 1 ) break;
                    if ( j - i == 1 ) std::cout << raises[i];
                    else std::cout << raises[i] << "-" << raises[j-1];
                    i = j - 1;    }
               std::cout << std::endl;    }    }
     ...
     {    
          ...
          {    if ( call[0][i] != winner )
               {    if (verbose)
                    {    
                         #pragma omp critical
                         {    std::cout << "trace read " << id1 << ", changing base "
                                   << i << " from " << as_base( call[0][i] ) 
                                   << " to " << as_base(winner) << std::endl;    }    }
               ...
               {    int j = ids[jm];
                    ...
                    {    if ( pass == 2 && ms == 0 ) break;
                         ...
                         {    char m;  
                              ...
                              if ( m == '*' ) ms++;    
                              if ( pass == 2 ) std::cout << m;    }
                         if ( pass == 2 ) 
                         {    if ( tt < 4 ) std::cout << tag[tt++];
                              std::cout << "\n";    }    }
                    for ( int c = c1; c < c2; c++ )
                    {    if ( call[j][c] == ' ' ) std::cout << ' '; // ?????????????
                         else std::cout << as_base( call[j][c] );    }
                    if ( tt < 4 ) std::cout << tag[tt++];
                    std::cout << "\n";
                    for ( int c = c1; c < c2; c++ )
                         std::cout << ( call[j][c] == ' ' ? ' ' : '-' );
          ...
          {    vec<char> base;
               ...
               {    if ( j > 0 ) std::cout << " ";
                    ...
                    for ( k = j + 1; k < base.isize( ); k++ )
                         if ( qb[k] != qb[j] ) break;
                    if ( qb[j].first >= 0 )
                    {    std::cout << as_base(qb[j].second) << "[" << qb[j].first << "]";
                         if ( k - j > 1 ) std::cout << "*" << k-j;    }
                    j = k - 1;    }
               ReverseSortSync( total, id );
               std::cout << "  --  ";
               for ( int j = 0; j < 4; j++ )
               {    if ( total[j] == 0 ) break;
                    std::cout << as_base( id[j] ) << "[" << total[j] << "]";    }
               std::cout << "\n";
               
               // Assess Q30 competitors.
          
               std::cout << "winning base = " << as_base( id[0] ) << std::endl;
               vec<int> competitors;
               for ( int j = 0; j < base.isize( ); j++ )
               {    if ( qb[j].first >= 20 && qb[j].second != id[0] )
                    {    std::cout << "competitor = " << ids[j] << ", base = " 
