...
void KmerAlign(
    ...
    if ( VERBOSITY >= 2 ) {
        ...
        for ( size_t i = 0; i < offset.size(); ++i )
            std::cout << offset[i].first << "," << offset[i].second << " ";
        std::cout << std::endl;
    }
    ...
    if ( VERBOSITY >= 1 ) {
        ...
        std::cout << "    " << equiv.OrbitSize( reps[0] ) << "(" << equiv.OrbitSize( reps[0] ) * 100 / offset.size() << "%)"
                  << " in first group, total number of groups= " << equiv.OrbitCount() << ", status "
                  << status << std::endl;
        for ( size_t i = 0; i < reps.size() && equiv.OrbitSize( reps[i] ) > 10  ; ++i )
            std::cout << "        Group " << i << ": " << reps[i] << ", size " << equiv.OrbitSize( reps[i] )
                      << " shift~ " << offset[ reps[i] ].first << " pos1~ " << offset[ reps[i] ].second << std::endl;
        std::cout << std::endl;
    }

    // Starting from the largest group, deletete inconsitent groups.
    vec< vec< std::pair<int,int> > > good_groups; // good roups of alignments
    for ( size_t i = 0; i < reps.size() && equiv.OrbitSize( reps[i] ) >= Min_Group_Size; ++i ) {
        vec< std::pair<int,int> > the_group;
        vec<int> orbit;
    ...
    vec< std::pair<int,int> > good_aligns;
    for ( size_t i = 0; i < good_groups.size(); ++i )
        good_aligns.append( good_groups[i] );
    Sort(good_aligns);
    if ( VERBOSITY >= 1 ) std::cout << "good aligns= " << good_aligns.size() << std::endl;

    // remove nonunique kmers
    std::map<int,int> kmer1, kmer2;
    ...
    if ( VERBOSITY >= 1 ) std::cout << "uniq aligns= " << good_aligns.size() << std::endl;
...
void GroupAndSelect(
    ...
    // Heuristics
    if ( VERBOSITY >= 1 )
        std::cout << "offset_in.size()= " << offset_in.size() << std::endl;
    if ( VERBOSITY >= 2 ) {
        std::cout << "offset_in= ";
        for ( size_t i = 0; i < offset_in.size(); ++i )
            std::cout << offset_in[i].first << "," << offset_in[i].second << " ";
        std::cout << std::endl;
    }
    // Group the kmer matching, so that within the group the offsets are less
    // than 20% and pos1 differences are less than 200 bases
    ...
    if ( VERBOSITY >= 1 ) {
        String status = ( reps.solo() || equiv.OrbitSize( reps[0] ) > equiv.OrbitSize( reps[1] ) * 2 ) ? "Good":"Bad";
        std::cout << "  " << equiv.OrbitSize( reps[0] ) << "(" << equiv.OrbitSize( reps[0] ) * 100 / offset_in.size() << "%)"
                  << " in first group, total number of groups= " << equiv.OrbitCount() << ", status "
                  << status << std::endl;
        for ( size_t i = 0; i < reps.size() && equiv.OrbitSize( reps[i] ) > 10  ; ++i )
            std::cout << "        Group " << i << ": " << reps[i] << ", size " << equiv.OrbitSize( reps[i] )
                      << " shift~ " << offset_in[ reps[i] ].first << " pos1~ " << offset_in[ reps[i] ].second << std::endl;
        std::cout << std::endl;
    }
    ...
    for ( size_t i = 0; i < reps.size(); ++i ) {
        ...
        if ( VERBOSITY >=2 ) {
            if ( valid )
                std::cout << "    take " << i << " " << mean_offset<< std::endl;
            else
                std::cout << "    skip " << i << " " << mean_offset << std::endl;
        }
    }
}
...
void KmerAlign3(
    ...
    vec< std::pair<int,int> >  offset1, offset2, offset3;
    if ( VERBOSITY >= 1 )
    ...
    vec< std::pair<int,int> > aligns_in;
...
void KmerAlignCompare( const vec< std::pair<int,int> >& aligns1, const vec< std::pair<int,int> >& aligns2 ) {
    std::cout << "aligns.size()= " << aligns1.size() << " " << aligns2.size() << std::endl;
    std::cout << "align1 ";
    for ( size_t i = 0; i < aligns1.size(); ++i )
        std::cout <<  aligns1[i].first << "," << aligns1[i].second << " ";
    std::cout << std::endl;
    std::cout << "align2 ";
    for ( size_t i = 0; i < aligns2.size(); ++i )
        std::cout << aligns2[i].first << "," << aligns2[i].second << " ";
    std::cout << std::endl;
}
