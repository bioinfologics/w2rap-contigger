...
void CorrectPairs1( String const& tmpDir, const int K, const int max_freq, vecbasevector& bases,
    ...
    /*
    {    std::cout << Date( ) << ": begin raise" << std::endl;
         ...
         quals = quals2;
         std::cout << timer1 << " seconds spent constructing" << std::endl;
         std::cout << timer2 << " seconds spent raising" << std::endl;
        ...
        */
        ...
        if (trace) {
            #pragma omp critical
            {
                std::cout << "\ntracing reads id1 = " << id1
                << ", id1p = " << id1p << std::endl;
                std::cout << "final offsets = " << printSeq(final_offsets)
                << std::endl;
            ...
            */
            ...
            if (trace) {
                #pragma omp critical
                {
                    ...
                    << id1p << "\noffset = " << offsets[oj] << std::endl;
                    PRINT4( minq, minq_floor, min_glue, min_glue_floor );
                    if ( minq < minq_floor ) {
                        std::cout << "low-quality positions:";
                        int count = 0;
                        for ( int j = 0; j < (int) conq.size( ); j++ ) {
                            if ( conq[j] == minq ) {
                                if ( ++count <= 10 ) std::cout << " " << j;
                                else std::cout << " ...";
                            }
                        }
                        std::cout << "\n";
                    ...
                    for ( int j = 0; j < aligns.isize( ); j++ ) {
                        ...
                        aligns[j].PrintReadableBrief( std::cout,
                                                      "con", ToString(g) );
                        aligns[j].PrintVisual( std::cout, con, conq,
                                               (*(log_control.G))[g], False );
                    }
                    /*
                    vec<basevector> cs = stack.Consensuses2( 30, 2 );
                    std::cout << "found " << cs.size( ) << " consensuses\n";
                    const int max_cs_print = 100;
                    for ( int l = 0;
                         l < Min( max_cs_print, cs.isize( ) ); l++ )
                    {    std::cout << "alignment of consensus " << l << std::endl;
                         ...
                         {    int g = aligns[j].target_id;
                              aligns[j].PrintReadableBrief( std::cout,
                                   "cs[l]", ToString(g) );
                              aligns[j].PrintVisual( std::cout, cs[l],
                                   (*(log_control.G))[g], False );    }
                              }
                    */
            ...
            if ( logc.verb[ "CORRECT1" ] >= 1 ) {
                ...
                if ( ( left.size( ) > 0 && p1.empty( ) )
                    ...
                    #pragma omp critical
                    {
                        ...
                        for ( int i = 0; i < closures.isize( ); i++ ) {
                            std::cout << "\nclosure " << i;
                            std::cout << "\noffset = " << closureso[i] << std::endl;
                            vec<placementy> p = log_control.
                            FindGenomicPlacements(closures[i]);
                            if ( p.nonempty( ) ) std::cout << "perfect!" << std::endl;
                            else std::cout << "imperfect" << std::endl;
                            ...
                            std::cout << "mean column qsum = "
                                      << double(totalq) / double( s.Cols( ) )
                                      << std::endl;
                        }
                        std::cout << "perfect ids: " << printSeq(perfs) << std::endl;
                        std::cout << "imperfect closure for " << id1 << ":" << std::endl;
                        static int count(0);
                        if ( count < 100 ) {
                            vec<look_align> aligns;
                            ...
                            for ( int j = 0; j < aligns.isize( ); j++ ) {
                                ...
                                aligns[j].PrintVisual( std::cout, left,
                                                       (*(log_control.G))[g] );
                            }
                            ...
                            for ( int j = 0; j < aligns.isize( ); j++ ) {
                                ...
                                aligns[j].PrintVisual( std::cout, right,
                                                       (*(log_control.G))[g] );
                            }
                            std::cout << "\nstack 1:\n";
                            stack1.Print(std::cout);
