...
void SupportedHyperBasevector::WordifyAlt2( const long_logging_control& log_control,
    ...
    vec< vec< std::pair<int,int> > > windex( wmax + 1 );
    for ( int i = 0; i < w.isize( ); i++ )
        for ( int j = 0; j < w[i].isize( ); j++ )
    ...
    if (logc.STATUS_LOGGING) std::cout << Date( ) << ": wordify finding overlaps" << std::endl;
    ...
    if (logc.STATUS_LOGGING)
        cout << Date( ) << ": wordify finding overlap segments" << std::endl;
    vec< vec<int> > segs;
    ...
    if (logc.STATUS_LOGGING)
        cout << Date( ) << ": wordify sorting overlap segments" << std::endl;
    ParallelUniqueSort(segs);
    if (logc.STATUS_LOGGING)
        cout << Date( ) << ": found " << segs.size( ) << " segments" << std::endl;

    // Find the paths containing each segment.

    if (logc.STATUS_LOGGING) std::cout << Date( ) << ": wordify finding paths" << std::endl;
    vec< vec< std::pair<int,int> > > slocs( segs.size( ) );
    ...
    if (logc.STATUS_LOGGING) std::cout << Date( ) << ": begin wordify main loop" << std::endl;
    #pragma omp parallel for schedule( dynamic, 1 )
    for ( int s = 0; s < segs.isize( ); s++ ) {
        int nkmers = 0;
        int w = segs[s].size( );
            ...
            for ( int e2 = 0; e2 < right_ext.isize( ); e2++ ) {
                ...
                if ( logc.verb[ "WORDIFY" ] >= 2 ) {
                    out << "\nseg = " << printSeq(segs[s]) << ", left_ext = "
                        << left_ext[e1]
                        << ", right_ext = " << right_ext[e2] << std::endl;
                }
                vec< vec<int> > MT(M);
                int mrows = M.size( );
                vec<Bool> left_full( mrows, False ), right_full( mrows, False );
                vec< std::pair<Bool,fix64_6> > WT(mrows);
                ...
                for ( int i = 0; i < nrows; i++ ) {
                    ...
                    for ( int j = 0; j < mrows; j++ ) {
                        ...
                        if ( OK && logc.verb[ "WORDIFY" ] >= 3 )
                            out << "should delete " << printSeq(M[j]) << std::endl;
        ...
        if ( logc.verb[ "WORDIFY" ] >= 1 )
        {    out << "\nlefts = " << printSeq(lefts) << std::endl;
             out << "rights = " << printSeq(rights) << std::endl;    }
        vec<fix64_6> counts;
    ...
    {    vec< std::pair<int,int> > con;
         // std::cout << "\nassessing " << printSeq(w[i]) << std::endl; // XXXXXXXXXXXXXXXXXX
         for ( int u = 0; u < windex[ w[i][0] ].isize( ); u++ )
         {    int j = windex[ w[i][0] ][u].first, o = windex[ w[i][0] ][u].second;
              if ( j == i ) continue;
              // std::cout << "contained in " << printSeq(w[j]) << std::endl; // XXXXXXXXXXXX
         ...
         {    int j = con[r].first, o = con[r].second;
              ...
              {    int l = over[j][n].first, q = over[j][n].second;
                   ...
                   {    join2.resize( Max( m2, -q + m1 ) );
                        ...
                        for ( int l = 0; l < m2; l++ )
                             join2[l] = v2[l];    }
                   // std::cout << "join2 is " << printSeq(join2) << std::endl; // XXXXXXXXXX
         ...
         {    if ( over_del[i][m] ) continue;
              ...
              // std::cout << "looking at overlap with " << printSeq(w[k]) << std::endl; //XXX
              ...
              {    join.resize( Max( n2, -p + n1 ) );
                   ...
                   for ( int l = 0; l < n2; l++ )
                        join[l] = w2[l];    }
              // std::cout << "join is " << printSeq(join) << std::endl; // XXXXXXXXXXXXXXXXX
              Bool found = False;
         ...
         if (valid)
         {    delp[i] = True;
              // std::cout << "killing " << printSeq(w[i]) << std::endl; // XXXXXXXXXXXXXXXXX
              if ( Inv( w[i][0] ) >= 0 )
              {    vec<int> wr;
                   ...
                   if ( g >= 0 ) delp[g] = True;    }    }    }
    if (logc.STATUS_LOGGING)
         cout << TimeSince(zclock) << " used in word containment" << std::endl;
    */
    ...
    //      cout << "[" << i << "] " << printSeq(w[i]) << std::endl; // XXXXXXXXXXXXXXXX

    // Define overx.

    vec< vec< std::pair<int,int> > > overx( over.size( ) );
    for ( int i = 0; i < w.isize( ); i++ ) {
    ...
    */
    ...
    if ( logc.verb[ "WORDIFY" ] >= 1 )
        cout << "\ntranslating " << NPaths( ) << " paths" << std::endl;
    #pragma omp parallel for
    for ( int z = 0; z < NPaths( ); z++ ) {
        ...
        if ( logc.verb[ "WORDIFY" ] >= 3 ) {
            ...
            {
                cout << "\n" << Date( ) << ": z = " << z << ", Path(z) = "
                << printSeq(w) << std::endl << paths.size( ) << " paths found"
                << std::endl;
            }
        }
        #pragma omp critical
    ...
    vec< std::pair< vec<int>, vec<int> > > pairsx;
