...
void EvalCorrected(
    ...
    cout << Date( ) << ": start looking for reference kmers" << std::endl;
    ...
    cout << "\n" << PERCENT_RATIO( 4, Sum(found), (int64_t) kmers.size( ) )
         << " sensitivity"
         << " (fraction of genomic 100-mers present in corrected reads)" << std::endl;
    cout << PERCENT_RATIO( 3, unaligned, ids.isize( ) )
         << " of corrected reads are unaligned" << std::endl;
    cout << std::setiosflags(ios::fixed) << setprecision(2) << 10000.0 * Mean(err_rate)
         << "%% error rate for aligned reads" << std::endl;
    cout << setprecision(2) << 10000.0 * amb_rate << std::resetiosflags(ios::fixed)
         << "%% ambig rate for aligned reads" << std::endl;
