        ...
        std::map<Variant, vec<std::pair<int,int>>> *p_var_friending)
{
    ...
    for (auto it = vars.begin(); it != vars.end(); ++it) {
        ...
        for (const std::pair<int,Caller>& x: it->second) {
...
void EdgesOnRef::UnrollAll(const int verbosity, const int iLoBound, const int iHiBound, const bool bCorrection)
{
    ...
    vec<vec<std::pair<double,int>>> backbone_extension_alts(backbone.size());

    if (verbosity >= 1) std::cout << Date() << ": Extending the path tail" << std::endl;
    ...
    if (verbosity >= 1) std::cout << Date() << ": Extending the path head" << std::endl;
    int head_edge = 0;
    while (true) {
    ...
    if ( verbosity >= 1 ) std::cout << Date() << ": looking for inversions" << std::endl;
    ...
    for ( auto el : length_to_edgeid ) {
        auto list = el.second;
        vector< std::pair<int, int> > rc_pairs;
    ...
    if (verbosity >= 2) {
        std::cout << "Delete additional inversion edges that belongs to the same components: edge_id = ";
        to_del2.Println(std::cout);
        std::cout << "Delete additional inversion edges that belongs to the reverted backbone: edge_id = ";
        to_del3.Println(std::cout);
    }
    ...
    //if (verbosity >= 2) {
    //    std::cout << "Delete additional inversion edges that belongs to extended reverted backbone: edge_id = ";
    //    to_del4.Println(std::cout);
    //}
    ...
    if ( verbosity >= 2 ) {
        std::cout << Date() << ": inversion edges to delete:" << std::endl;
        for ( auto edge : to_delete ) std::cout << edge << " ";
        std::cout << std::endl;
    ...
    if (verbosity >= 2) {
        ...
        std::cout << "The corresponding hbp edges are: ";
        vec<int> backbone0 = ConvertToAssemblyPathFromUnrolled(backbone);
        for (size_t i = 0; i < backbone0.size(); i++) {
            if (i!=0) std::cout << " ";
            std::cout << backbone0[i];
            if (Member(backbone0, to_inv[backbone0[i]]) )
                std::cout << "("<< to_inv[backbone0[i]] <<")";
        }
        std::cout << std::endl;
    }
        ...
        int verbosity) 
{
    ...
    for (size_t i = 0; i < hb_.From(node).size(); i++) {
        ...
        const vec<std::pair<int,int>>& starts = known_starts[edge_new];
        ...
        vec<align>* p_edge_aligns, int verbosity) 
{
    ...
    vec<std::pair<bool,bool>> free_ends;
    ...
    for (size_t i = 0; i < anchoring_edges.size(); i++) {
        ...
        align a;
        int bandwidth = std::max(1, (int)edge_seq.size() /2 );
    ...
    if (verbosity >=1) std::cout << Date() << ": Ended calling variants" << std::endl;
};
...
void EdgesOnRef::MakeBubbleGraph(int verbosity) 
{
    ...
    if (verbosity >=2)
        std::cout << "build the graph backbone" << std::endl;
    bubble_graph_.Clear();
    ...
    if (verbosity >=2)
        std::cout << "build the graph bubbles" << std::endl;
        ...
        int verbosity) 
{
    ...
    //   ...  // }
    vec<vec<std::pair<int,int>>> homes_index(bubble_graph_.EdgeObjectCount());
//    FindReadHomesBest(bases, quals, bubble_graph_, &homes_index, vec<int>(), NULL, verbosity);
    ...
    for (int n1 = 0; n1 < bubble_graph_.N(); n1++) {
        ...
        for (size_t j = 0; j < branches.size(); j++) {
            int e = branches[j];
            if (verbosity>=2) {
                std::cout << "branches " << j << " edge= " << e << std::endl;
        ...
        vec<align>& single_edge_aligns, int verbosity) 
{
    ...
    if (todel.CountValue(True) > 0) {
        if (verbosity >= 2) {
            std::cout << "Erasing short anchoring edges:  " << std::endl;
            for (size_t i = 0; i < todel.size(); i++) {
                if (todel[i]) {
                    std::cout << " i= " << i << " path= ";
                    single_edges[i].Println(std::cout);
                }
            }
        }
...
void EdgesOnRef::RemoveInversion(vec<vec<int>>& paths, vec<basevector>& edges) const 
{
    ...
    if (todel.CountValue(True) > 0) {
        std::cout << "Delete " << todel.CountValue(True) << " inversion edges "  << std::endl;
        for (size_t i = 0; i < paths.size(); i++) {
            std::cout << "path= ";
            paths[i].Println(std::cout);
            std::cout << "path0= ";
            ConvertToAssemblyPathFromUnrolled(paths[i]).Println(std::cout);
        }
    } else {
        std::cout << "No inversion detected." << std::endl;
    }
    EraseIf(paths, todel);
    EraseIf(edges, todel);
}
