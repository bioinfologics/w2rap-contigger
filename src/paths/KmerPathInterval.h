...
class KmerPathInterval {
 ...
 public:
  ...
  Bool Overlaps( const KmerPathInterval& other ) const
    { return( std::max(Start(),other.Start()) <= std::min(Stop(),other.Stop()) ); }
  ...
  friend std::ostream& operator<<(std::ostream& out, const KmerPathInterval& rpi) {
    if( rpi.isSeq() )
      out << "[" << std::setw(12) << rpi.Start() << "-" << std::setw(12) << rpi.Stop() << "]";
    else
      out << "      (gap " 
          << std::setw(3) << rpi.Minimum() 
          << " - " 
          << std::left << std::setw(3) << rpi.Maximum() 
          << std::right << ")      ";
    return out;
  }
  ...
  struct Blank {
    friend std::ostream& operator<<(std::ostream& out, const Blank& s ) {
      return out << "                           ";
    }
  };
  ...
  struct OrderByStart : public std::binary_function<KmerPathInterval,KmerPathInterval,bool> {
