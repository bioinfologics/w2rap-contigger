...
class KmerBaseBrokerTemplate {
  ...
  private:
    ...
    vecbasevector bases;
    mutable std::map<kmer_id_t, basevector> bases_cache;
    ...
    mutable std::map<cache_key,KmerPath> between_cache;

    mutable std::map<cache_key,int> min_offset_cache;
    mutable std::map<cache_key,int> max_offset_cache;
