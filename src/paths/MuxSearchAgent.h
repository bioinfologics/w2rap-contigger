        ...
        if( ! final_mux_subsumes_something &&
            ...
            if( m_verbosity > 5 )
                cout << "Not exploring " << final_mux.GetPathId()
                     << ", would make path too long" << std::endl;
            return true;
        } else
            return false;
    }
...
/////////////////////////////
///
///  This search agent prunes any std::pair<okpid, dist> it has already seen
///
...
class MuxSearchAgentSimple : public MuxSearchAgentBase {
  ...
  public:
    ...
    longlong BasicHashOfPosition() {
        ...
        if( m_verbosity > 6 )
            cout << "state hash " << hex << here << dec << std::endl;
        return here;
    }
...
class MuxSearchAgentGoodReads : public MuxSearchAgentBase {
  ...
  private:
    ...
    set< std::pair<longlong,longlong> > seen;
