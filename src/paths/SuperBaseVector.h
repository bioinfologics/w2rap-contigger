...
public:
  ...
  std::pair<int,int> Gap( int i ) const { return std::make_pair(mingaps[i],maxgaps[i]); }
...
// 	      std::mem_fun_ref_t<void,basevector>( &basevector::ReverseComplement) );
	      std::mem_fun_ref( (basevector & (basevector::*)()) 
  ...
  void PushGap( std::pair<int,int> gap ) {
