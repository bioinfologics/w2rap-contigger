...
void PrintBases( std::ostream& out, int blanks, const BASEVEC& rd, int from, int to )
...
void PrintBases( std::ostream& out, int blanks, const basevector& rd, int from, int to )
{    PrintBlanks(out, blanks);
     for ( int i = from; i < to; i++ )
          out << as_base(rd[i]);
     out << "\n";    }

void PrintBases( std::ostream& out, int blanks, const fastavector& rd, int from, int to )
{    PrintBlanks(out, blanks);
     for ( int i = from; i < to; i++ )
          out << rd[i];
     out << "\n";    }

void PrintBases( std::ostream& out, int blanks, const vec<char>& rd, int from, int to )
{    PrintBlanks(out, blanks);
     for ( int i = from; i < to; i++ )
          out << rd[i];
     out << "\n";    }
...
{    const avector<int>& gaps = a.Gaps( );
     ...
     {    if ( gaps(j) > 0 ) enuf += gaps(j);
          if ( gaps(j) < 0 ) enuf -= gaps(j);
          enuf += lengths(j);    }
     std::vector<char> v1(enuf), v2(enuf), e(enuf), s1(enuf), s2(enuf), t1(enuf),
          t2(enuf);
...
void PrintAlignment(std::ostream& out, const basevector& rd1, const basevector& rd2,
     const alignment& a)
