...
template <class T> inline void ParallelUniqueSort(vec<T> & v) {
    ParallelUniqueSort(v,std::less<T>(),std::equal_to<T>());
}
...
inline  void ParallelUniqueSortAndCount(vec<T> & v, vec<unsigned int> & counts) {
    ParallelUniqueSortAndCount(v,std::less<T>(),std::equal_to<T>(),counts);
}

template <class T, class FilterPredicate>
inline  void ParallelUniqueSortAndCount(vec<T> & v, vec<unsigned int> & counts,
                                        FilterPredicate pass) {
    ParallelUniqueSortAndCount(v, std::less<T>(), std::equal_to<T>(), counts, pass);
}
...
void ParallelWhatPermutation( const V & v, vec<IDX>& permutation,
                              bool inv = true ) {
    ParallelWhatPermutation(v,permutation,std::less<typename V::value_type>(),inv);
}
...
void ParallelUniqueSortSync( V1& v, V2& w ) {
    ParallelSortSync(v, w, std::less<typename V1::value_type>());
...
void ParallelReverseSortSync( V1& v, V2& w, V3& x ) {
    ParallelSortSync(v,w,x,std::greater<typename V1::value_type>());
}
...
void ParallelReverseSortSync( V1& v, V2& w, V3& x, V4& y ) {
    ParallelSortSync(v,w,x,y,std::greater<typename V1::value_type>());
}
