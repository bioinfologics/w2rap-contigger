...
void writeLibraryStats( std::ostream & out, const vec<PM_LibraryStats> & stats);
...
public:  
  ...
  void printLibraryStats( std::ostream & out, const String bases_filename = "" ) const;
  
  // Write nicely formatted information about each read to an output stream.
  void printPairInfo( std::ostream & out ) const;
  
  void print( std::ostream & out ) const { printLibraryStats( out ); out << "\n\n"; printPairInfo( out ); out << "\n\n"; }
  ...
  { AssertLe(2*_n_pairs,nReads); _n_reads = nReads; }
  ...
  std::pair<longlong,longlong> operator[]( const longlong & pair_ID ) const {
    return std::make_pair( _ID1[pair_ID], _ID2[pair_ID] );
  }
...
private:
  ...
  void clearCache( ) const { Destroy(_pairs_index); Destroy(_partners_index); }
