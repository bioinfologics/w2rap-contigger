...
const std::map< int, arachne_contig >* annotated_contig::arachne_contigs_ = NULL;
...
void arachne_contig_placement::Print( std::ostream &o ) const {
  o << "ID: " << ID() << "; [from,to]: ["
    << start_ << ","
    << stop_ << "]";
  if ( IsGap() )
...
void annotated_contig::ComputeSemiannotations() {
  ...
  typedef std::set< semiannotation, semiannotation_loccomp > set_loccomp;
  typedef set_loccomp::iterator                         set_loccomp_itr;
  typedef std::map< int, set_loccomp >::iterator             map_set_loccomp_itr;

  std::map< int, set_loccomp > stdIDs_and_locs; /* The 'key' of this map is the standard id,
					      and the std::set< contig_actualloc, ctg_actloc_loccomp >
  ...
  for ( ;
    ...
    // std::cout << "\t\tArachne Contig  ";
    // arach_ctg.Print( std::cout );
    // std::cout << std::endl;
	...
	if ( curr_stdID_set_loccomp_itr != stdIDs_and_locs.end() ) {
	  ...
	  if ( find_this_semi != curr_stdID_set_loccomp_itr->second.end() ) {
	    ...
	    for ( ;
		  ...
		  ++currsemi_print ) {
	      // std::cout << "\t\t\t\t\t";
	      // currsemi_print->Print( std::cout );
	      // std::cout << std::endl;
	    }
	  }
	  else {
	    curr_stdID_set_loccomp_itr->second.insert( this_semi );
	    // std::cout << "\t\t\t\t Inserted this semi" << std::endl;
	  }
	}
	else {
	  stdIDs_and_locs.insert( std::pair< int, set_loccomp > ( curractloc->StdID(),
							     set_loccomp() ) );
	  ...
	  curr_stdID_set_loccomp_itr->second.insert( this_semi );
	  // std::cout << "\t\t\t\t Inserted this semi, after inserting STANDARD" << std::endl;
	}

	// std::cout << "\t\t\t\t\t Semi count: " << curr_stdID_set_loccomp_itr->second.size() << std::endl;
      }
    }
  }
  ...
  for ( ;
	...
	continue;
      ...
      /*
      std::cout << "\t\t\t\t tot_actloc: " << total_actlocs
	   << "; tot_shift: " << total_shift
	   << ", " << total_shift/ total_actlocs + minstart
	   << "; minstart,maxstop: " << minstart << ", " << maxstop << std::endl;
      */
      for ( ++currsemi_itr;
	    currsemi_itr != end_semi_itr &&
	...
	std::cout << "\t\t\t\t tot_actloc: " << total_actlocs
      ...
      /*
      std::cout << "\t\t\t\t\t\t Exited loop because ";
      if ( currsemi_itr == end_semi_itr ) std::cout << " reached end of semiannotations " << std::endl;
      else if ( currsemi_itr->RC() != prevsemi_itr->RC() ) std::cout << " conflicting RC bits" << std::endl;
      else std::cout << " too little fudge " << std::endl;
      ...
      */
      ...
      longlong implied_length      = std::min( (longlong)Length(), maxstop - minstart + 1 );
      longlong implied_minstart    = ( prevsemi_itr->RC() ?
					    std::max( (longlong)0, Length() - maxstop ):
					    minstart );
      ...
      /*
      std::cout << "\t\t\t\t Implied left_on_std, length, minstart, maxstop: " << implied_left_on_std << ", " << implied_length << ", ( "
	   << implied_minstart << " , " << implied_minstart + implied_length - 1 << " )" << std::endl;
      */
...
void semiannotation::Print( std::ostream &o ) const {
...
void annotated_contig::Print( std::ostream &o ) const {
  o << "Contig " << id_ << ": length = " << quals_.size() << ".";
}

void annotated_supercontig::Print( std::ostream &o ) const {
  o << "Supercontig of length " << Length() << ":" << std::endl;
...
std::ostream& operator<<( std::ostream &o, const annotated_contig &a ) {
  ...
  o << a.semiannotations_.size() << std::endl;
  for ( int i = 0; i < (int)a.semiannotations_.size(); ++i )
    o << a.semiannotations_[ i ] << std::endl;

  o << a.annotations_.size() << std::endl;
  for ( int i = 0; i < (int)a.annotations_.size(); ++i )
    o << a.annotations_[ i ] << std::endl;

  o << a.arachne_ctg_placements_.size() << std::endl;
  for ( int i = 0; i < (int)a.arachne_ctg_placements_.size(); ++i )
    o << a.arachne_ctg_placements_[ i ] << std::endl;
...
std::ostream& operator<<( std::ostream &o, const annotated_supercontig &a ) {
  ...
  o << a.contigs_.size() << std::endl;
  for ( int i = 0; i < (int)a.contigs_.size(); ++i )
    o << a.contigs_[ i ] << std::endl;

  o << a.gaps_.size() << std::endl;
...
std::ostream& operator<<( std::ostream &o, const annotated_final_answer &a ) {
  ...
  o << a.supers_.size() << std::endl;

  for ( int i = 0; i < (int)a.supers_.size(); ++i )
    o << a.supers_[ i ] << std::endl;
...
std::istream& operator>>( std::istream &in, annotated_contig &a ) {
  int id;
  in >> id;
  a.SetID( id );
...
std::istream& operator>>( std::istream &in, annotated_supercontig &a ) {
  int len;
...
std::istream& operator>>( std::istream &in, annotated_final_answer &a ) {
