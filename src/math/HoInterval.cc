...
{    vec< std::pair<int,int> > start_stop;
     for ( int i = 0; i < (int) in.size( ); i++ )
     {    start_stop.push_back( std::make_pair( in[i].Start( ), +1 ) );
          start_stop.push_back( std::make_pair( in[i].Stop( ), -1 ) );    }
     sort( start_stop.begin( ), start_stop.end( ) );
     out.clear( );
     if ( start_stop.size( ) == 0 )
     {    out.push_back( std::make_pair( ho_interval( 0, n ), 0 ) );
          return;    }
     if ( start_stop[0].first != 0 )
          out.push_back( std::make_pair( ho_interval( 0, start_stop[0].first ), 0 ) );
     int coverage = 0;
     ...
     if ( start_stop.back( ).first < n )
          out.push_back( std::make_pair( ho_interval( start_stop.back( ).first, n ),
               coverage ) );    }
