  ...
  while (center!=last) {
    ...
    *out = *smallest;
    ...
    if (w>nbhd) {
      --w;
      if (smallest==first++) {
	smallest = std::min_element(first, winLast, op);
      }
    }
  }
}
...
//[GONZA] added to compile
#ifdef __APPLE__
inline size_t BigSum( const vec<long unsigned int>& v )
{    size_t answer = 0;
  for ( unsigned int i = 0; i < v.size( ); i++ )
    answer += v[i];
  return answer;    }
...
{ return os << X.mu_ << "\t" << X.sigma_ << std::endl; }

inline std::istream &operator>>( std::istream &in, NormalDistribution &nd ) {
  in >> nd.mu_ >> nd.sigma_;
  return in;
}
...
struct QuadraticFunction { 
  ...
  /// aren't any.
  std::pair<double,double> solutions() const;
};
...
float Cosine( ForwardIter1 begin1, ForwardIter1 end1,
  ...
  int size = static_cast<int>(std::min(d1.size(), d2.size()));
  ...
  ret = inner_product(d1.begin(), d1.begin() + size, d2.begin(), 0.0)
    / (norm1*norm2);
  if (ret > 1.0001) {
    std::cerr << "cosine too high: " << ret << "\n";
    for (int i=0; i != size; ++i) {
      std::cerr << *(begin1+i) << " " << (*begin2+i) << "\n";
    }
  }
...
std::pair<double, double> BinomialConfidenceInterval(int passes, int trials, double error=0.05);
