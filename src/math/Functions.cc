...
double OptimalCutoff(const NormalDistribution & d1,
    ...
    if (1 == ratio) {//simplest case
        ...
        std::pair<double,double> sol = quad.solutions();
...
std::pair<double,double> QuadraticFunction::solutions() const {
    if (0 == a) { //not quadratic!
        ...
        return std::make_pair(sol,sol);
    }

    double discrim = b*b -4*a*c;
    if (discrim < 0) return std::make_pair(std::numeric_limits<double>::quiet_NaN(),
                                               std::numeric_limits<double>::quiet_NaN());
    else return std::make_pair((-b - sqrt(discrim))/(2*a),
                                   (-b + sqrt(discrim))/(2*a));
}
...
std::pair<double, double> BinomialConfidenceInterval(int passes, int trials, double error) {
    ...
    return std::make_pair(center-halfwidth, center+halfwidth);
}
