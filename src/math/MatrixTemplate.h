...
{    
     ...
     int n = Nrows( );
     static std::vector<int> pivotindices;
     ...
     {    
          ...
          if ( pivotindex != k ) std::swap( P[k], P[pivotindex] );
...
#if 0
    L.PrettyPrint(cout); std::cout << std::endl;
    U.PrettyPrint(cout); std::cout << std::endl;
    P.PrettyPrint(cout); std::cout << std::endl;
#endif
...
template<class T> void matrix<T>::PrettyPrint( std::ostream& o ) const
{    vec< vec<String> > rows;
