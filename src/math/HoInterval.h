...
class ho_interval {
  ...
  public:
    ...
    void Lengthen( int lengthen, int maxStop ) {
        if (0!=lengthen) {
            start_ = std::min(std::max(0, start_ - lengthen), stop_);
            stop_ = std::max(std::min(stop_ + lengthen, maxStop), start_);
        }
    }
    ...
    friend std::ostream&operator<<( std::ostream& out, const ho_interval& i ) {
...
void CondenseIntervals( int n, const vec<ho_interval>& in,
                        vec< std::pair<ho_interval, int> >& out );
...
class HoIntervalWithId: public ho_interval {
  public:
    ...
    friend std::istream & operator>>(std::istream & is, HoIntervalWithId & i) {
...
inline
std::ostream & operator<< (std::ostream & os, const HoIntervalWithId & i) {
...
struct less< vec<HoIntervalWithId> > :
    ...
    bool operator() (const vec<HoIntervalWithId> & a,  const vec<HoIntervalWithId> & b) const {
        ...
        unsigned int min_size = std::min(a.size(),b.size());
...
inline bool MemberSorted(const vec<HoIntervalWithId> & intervals, int id,
  ...
  std::pair<viter,viter> contigRange =
