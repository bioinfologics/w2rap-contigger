...
///   cout << "Total time for task1(): " << std::endl;
///   cout << t1 << std::endl;
///   cout << "Total time for task2(): " << std::endl;
///   cout << t2 << std::endl;
///
...
class TaskTimer {
  ...
  public:
    ...
    void Start() {
        if ( is_running_ ) {
            cerr << "This timer is already running." << std::endl;
            return;
        }
        is_running_ = true;
        ++tasks_timed_;
    ...
    void Stop() {
        ...
        if ( ! is_running_ ) {
            cerr << "This timer is not running." << std::endl;
            return;
        }
    ...
    void Reset() {
        if ( is_running_ ) {
            cerr << "This timer is running and cannot be reset." << std::endl;
            return;
        }
    ...
    void Write(String filename, bool update = true) {
        ...
        if (outfile)
            outfile << (oldelapsed + newelapsed) << std::endl;
        outfile.close();
    }
