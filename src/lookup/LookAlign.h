...
class look_align {
  ...
  public:
    ...
    void PrintParseable( std::ostream& out, const basevector& query,
    ...
    void PrintReadableBrief( std::ostream& out, const basevector& query,
                             const qualvector& query_qual, const basevector& target,
                             int start_on_target, const vec<String>& seq_names,
                             const vec<String>& target_names ) const;

    ///Print a human-readable readable summary on one line.
    void PrintReadableBrief( std::ostream& out, const String & query_name,
    ...
    void PrintRmr( std::ostream& out, const basevector& query,
                   const qualvector& query_qual, const basevector& target,
                   int start_on_target ) const;

    void PrintMM( std::ostream& out, const basevector& query,
                  const qualvector& query_qual, const basevector& target,
                  int start_on_target ) const;

    void PrintRmrByBlock( std::ostream& out, const basevector& query,
                          const qualvector& query_qual, const basevector& target,
                          int start_on_target, const vec<String>& seq_names,
                          const vec<String>& target_names ) const;
    ...
    void PrintNqs( std::ostream& out, const basevector& query,
                   const qualvector& query_qual, const basevector& target,
                   int start_on_target, const vec<String>& seq_names,
                   const vec<String>& target_names ) const;
    ...
    void PrintQualScore( std::ostream& out, const basevector& query,
                         const qualvector& query_qual, const basevector& target,
                         int start_on_target, const qualvector& qual_g ) const;
    ...
    void PrintVisual( std::ostream& out, const basevector& query,
                      const qualvector& query_qual, const basevector& target,
                      int start_on_target, Bool abbr = True ) const;
    void PrintVisual( std::ostream& out, const basevector& query,
                      const qualvector& query_qual, const fastavector& target,
                      int start_on_target, Bool abbr = True ) const;
    void PrintVisual( std::ostream& out, const fastavector& query,
                      const qualvector& query_qual, const basevector& target,
                      int start_on_target, Bool abbr = True ) const;
    void PrintVisual( std::ostream& out, const fastavector& query,
                      const qualvector& query_qual, const fastavector& target,
...
inline bool IsAmbiguous(const look_align &best, const look_align &secondBest,
    ...
    if (COMPARE_BY_RATE)
        return (secondBest.ErrorRate()
                <= ERR_RATE_MULT * std::max(best.ErrorRate(), float(1.0/best.query_length)));
...
class look_align_plus : public look_align {
  ...
  public:
    ...
    void WriteParseable( std::ostream& out ) const;
};
...
class genome_pos {
  ...
  public:
    ...
    friend std::istream& operator>>( std::istream& in, genome_pos& x ) {
        return in >> x.pos1 >> x.Pos1 >> x.t >> x.pos2 >> x.Pos2 >> x.rc1
               >> x.mismatch_percent;
    }

    friend std::ostream& operator<<( std::ostream& out, const genome_pos& x ) {
...
class genome_pos_plus {
  ...
  public:
    ...
    friend std::istream& operator>>( std::istream& in, genome_pos_plus& x ) {
        return in >> x.pos1 >> x.Pos1 >> x.t >> x.pos2 >> x.Pos2
               >> x.length2 >> x.rc1 >> x.mismatch_percent;
    }

    friend std::ostream& operator<<( std::ostream& out, const genome_pos_plus& x ) {
        return out << x.pos1 << " " << x.Pos1 << " " << x.t << " "
...
struct order_lookalign_Begin
    : public std::binary_function<const look_align&, const look_align&, bool> {
  public:
...
struct order_lookalign_TargetLookAlignOffset
    : public std::binary_function<const look_align&,
...
struct order_lookalign_QueryIdFullTargetMutations : public std::binary_function<const look_align &, const look_align &, bool> {
...
struct equal_lookalign_QueryIdFullTargetMutations : public std::binary_function<const look_align &, const look_align &, bool> {
    bool operator()(const look_align & a, const look_align & b) const {
        if ( a.QueryId() != b.QueryId() ) return false;
        if ( a.TargetId() != b.TargetId() ) return false;
        if ( a.mutations != b.mutations ) return false;
...
struct gt_lookalign_Errors :
    public std::unary_function<const look_align&, bool> {
  private:
    int errors;
  public:
...
std::pair<int,int> hangingEnd( const look_align &align );
...
std::ofstream & operator << (std::ofstream & os, const look_align & la);

void SetWritePrettyLookAligns(std::ofstream & os);
