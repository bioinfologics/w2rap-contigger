...
void look_align::PrintParseable( std::ostream& out, const basevector& query,
...
void look_align::PrintParseable( std::ostream& out,
...
void look_align::ReadParseable( std::istream & ins )
{
  String field, target_id_s;
  ins >> field;
  ForceAssert( field == QUERY );
...
void look_align::PrintParseableBrief( std::ostream& out, const basevector& query,
                                      const qualvector& query_qual, const basevector& target,
                                      int start_on_target, const vec<String>& seq_names,
                                      const vec<String>& target_names ) const
{
...
void look_align::PrintReadableBrief( std::ostream& out, const String & query_name,
...
void look_align::PrintReadableBrief( std::ostream& out, const basevector& query,
...
void look_align::PrintReadableBrief( std::ostream& out) const
{
...
void look_align::PrintNqs( std::ostream& out, const basevector& query,
                           const qualvector& query_qual, const basevector& target,
                           int start_on_target, const vec<String>& seq_names,
                           const vec<String>& target_names ) const
{
  ...
  out << "see " << see << " NQS(30,25) differences, " << "rate = "
      << std::setprecision(4) << 100.0 * float(see) / float( a.Pos1( ) - a.pos1( ) )
      << "%\n";
}
...
void look_align::PrintQualScore( std::ostream& out, const basevector& query,
                                 const qualvector& query_qual, const basevector& target,
                                 int start_on_target, const qualvector& qual_g ) const
{
...
void look_align::PrintRmr( std::ostream& out, const basevector& query,
                           ...
                           int start_on_target ) const
{
  out << "rmr = " << std::setprecision(2)
      << RmrPercent( query, query_qual, target, start_on_target ) << "%\n";
}

void look_align::PrintMM( std::ostream& out, const basevector& query,
                          const qualvector& query_qual, const basevector& target,
                          int start_on_target ) const
{
...
void look_align::PrintRmrByBlock( std::ostream& out, const basevector& query,
                                  const qualvector& query_qual, const basevector& target,
                                  int start_on_target, const vec<String>& seq_names,
                                  const vec<String>& target_names ) const
{
...
void look_align::PrintVisual( std::ostream& out, const basevector & query,
...
void look_align::PrintVisual( std::ostream& out, const fastavector & query,
                              const basevector& target, const Bool abbr ) const
{    if (rc1)
     {    fastavector b = query;
          b.ReverseComplement( );
...
void look_align_plus::WriteParseable( std::ostream& out ) const
{
...
std::pair<int,int> hangingEnd( const look_align &align )
{
  ...
  {
    if ( IsLeft( align ) )
      return ( std::make_pair(align.a.Pos1(), align.query_length) );
    else if ( IsRight( align ) )
      return ( std::make_pair(0, align.a.pos1()) );
    else return ( std::make_pair( -1, -1 ) );
  }
  else
  {
    if ( IsLeft( align ) )
      return ( std::make_pair( align.query_length-align.a.pos1(), align.query_length) );
    else if ( IsRight( align ) )
      return ( std::make_pair(0, align.query_length-align.a.Pos1()) );
    else return ( std::make_pair( -1, -1 ) );
  }
}
...
void RemoveOverlap(look_align & la1, look_align & la2, bool verbose) {
  ...
  if (verbose) {
    la1.PrintParseable(std::cout);
    la2.PrintParseable(std::cout);
  }
  ...
  if (startOn1 == startOn2 ) { //remove the shorter of the two alignments.
    if (la1.a.extent1() < la2.a.extent1()) std::swap(la1,la2);
    la2.a = align();
    ...
    std::swap(la1, la2);
    std::swap(startOn1, startOn2);
  }
  if (verbose) PRINT2(startOn1, startOn2);
  if (startOn1 >= startOn2) {
    la1.PrintReadableBrief(std::cout);
    la2.PrintReadableBrief(std::cout);
