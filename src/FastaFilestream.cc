...
FastaFilestream<vecT,seqT,convT,verifT>::estimatedSize() {
    if ( ! preview_ptr_ ) {
        std::istream* istream_ptr = getIstream_();
...
FastaFilestream<vecT,seqT,convT,verifT>::estimatedData() {
    ...
    vec<std::streampos> seqSizes = preview_ptr_->getSequenceSizes();
...
FastaFilestream<vecT,seqT,convT,verifT>::verify() {
    ...
    if ( problem )
        std::cout << "There was an illegal character on line " << line_number
                  << " of the file: " << std::endl << filename_ << std::endl;
...
FastaFilestream<vecT,seqT,convT,verifT>::parse_( vecString &names,
    ...
    if ( verbose )
        std::cout << "Reading in data from " << basename << " (one . per 100K reads) " << std::flush;

    std::streampos curr_position = 0;
    ...
    for (unsigned int i = 0; i < seq_indices.size(); ++i ) {
        ...
        std::streampos offset_to_seq = sequence_positions[ seq_index ] - curr_position;
        ...
        while ( offset_to_seq > 0 && istream_ptr->good() ) {
            std::streampos bytes_to_be_read = min( (std::streampos) chunk_size, offset_to_seq );
        ...
        if ( ! istream_ptr->good() ) {
            std::cout << "Read failed on " << filename_ << "." << std::endl;
...
template <typename vecT, typename seqT, typename convT, typename verifT>
void
FastaFilestream<vecT,seqT,convT,verifT>::preview_( std::istream* fasta_istream ) {
    String basename = getBasename_( filename_ );
    // std::cout << "Scanning file " << basename << "... " << std::flush;

    preview_ptr_ = new FastaFilestreamPreview( *fasta_istream );

    // std::cout << "found " << preview_ptr_->getSequenceSizes().size() << " reads." << std::endl;
}
...
FastaFilestream<vecT,seqT,convT,verifT>::getIstream_() const {
    ...
    if ( needs_pipe_ ) {
        ...
        std::string command = "gzip -dc " + filename_;

        procbuf* zcat_pipe = new procbuf( command.c_str(), std::ios::in );
        istream_ptr = new std::istream( zcat_pipe );
    } else {
        istream_ptr = new std::ifstream( filename_.c_str() );
    }
...
template <typename vecT, typename seqT, typename convT, typename verifT>
void
FastaFilestream<vecT,seqT,convT,verifT>::resetIstream_( std::istream*& fasta_istream ) const {
    if ( needs_pipe_ ) {
...
template <typename vecT, typename seqT, typename convT, typename verifT>
void
FastaFilestream<vecT,seqT,convT,verifT>::closeIstream_( std::istream* fasta_istream ) const {
    if ( needs_pipe_ ) {
