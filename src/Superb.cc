...
void WriteSuperbs( const String& fn, const vec<superb>& s ) {
    ...
    for ( int i = 0; i < n; i++ ) {
        int ntigs = s[i].Ntigs( );
        if ( ntigs == 0 ) {
            std::cout << "In superb::Write: attempt to write super " << i
...
void superb::Print( std::ostream& out, const String& super_name ) const {
    int n = Ntigs( );
    int total_super = 0;
    for ( int j = 0; j < n; j++ ) {
        total_super += Len(j);
...
void superb::Print( const Boolvector& s_rc, std::ostream& out,
...
void superb::PrintRange( std::ostream& out, const String& super_name,
...
//[GONZA] TODO: fix this, operator<< weird definition, i commented this because it's never used
