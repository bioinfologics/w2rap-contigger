...
inline  void UniqueSortAndCount(vec<T> & v, vec<unsigned int> & counts) {
  UniqueSortAndCount(v,std::less<T>(),std::equal_to<T>(),counts);
}
...
inline  void UniqueSortAndCount(vec<T> & v, vec<unsigned int> & counts, FilterPredicate pass) {
  UniqueSortAndCount(v, std::less<T>(), std::equal_to<T>(), counts, pass);
}
...
{
    WhatPermutation(v, permutation, std::less<typename V::value_type>(), inv);
}
...
{
    SortSync(v, w, std::less<typename V1::value_type>());
...
{
    SortSync(v,w,x,std::greater<typename V1::value_type>());
}
...
{
    SortSync(v,w,x,y,std::greater<typename V1::value_type>());
}

template<class V1, class V2, class V3, class V4, class V5>
void ReverseSortSync( V1& v, V2& w, V3& x, V4& y, V5& z )
{
    SortSync(v,w,x,y,z,std::greater<typename V1::value_type>());
}

template<class V1, class V2, class V3, class V4, class V5, class V6>
void ReverseSortSync( V1& v, V2& w, V3& x, V4& y, V5& z, V6& u ) {
  SortSync(v,w,x,y,z,u,std::greater<typename V1::value_type>());
}
...
Intersection( const vec<T>& v1, const vec<T>& v2, vec<std::pair<T,T> > & result )
{    
  result.clear();
  ...
  while ( v1iter != v1.end() && v2iter != v2.end() )
  {
    if ( *v1iter < *v2iter )
      ...
      result.push_back( std::make_pair(*v1iter++,*v2iter) );
  }
}
