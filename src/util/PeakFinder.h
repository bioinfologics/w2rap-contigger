...
public:
	...
	for (auto i : simple_peaks) {
		...
		if (debug) {
		    std::cout << "CANDIDATES :" << i << "  " << center_x << "  " << center_y << "  " << right_index - i - 1 << "  " << i - left_index << std::endl;
		}
	    }
	...
	for (size_t i = 0; i < peak_count; i++) {
		...
		if (debug)
		    std::cout << "SHALLOW: " << i << "  " << x_val[this_peak] << " " << y_val[this_peak] << " " << 1.0 * y_val[this_peak]/left_min << " " << 1.0 * y_val[this_peak]/right_min << std::endl;
	    }
	}
	...
	for (size_t i = 0; i < peak_count; i++) {
	    ...
	    if (debug)
		std::cout << "CENTRED:" << i << "  " << candidates[i] << "  " << x_val[candidates[i]] << " " << y_val[candidates[i]] << std::endl;
	}
