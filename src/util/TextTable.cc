...
void TextTable::Print( std::ostream& out, int padding, const std::string& alignments, const bool ragged_right ) {
    // determine the actual width for each column
    size_t ncols = 0;
    for ( size_t i = 0; i < lines.size(); ++i ) 
        if (lines[i].second != '\0')
            ncols = std::max( ncols, lines[i].first.size() );
    vec<size_t> widths( ncols );
    for ( size_t i = 0; i < lines.size(); ++i ) 
        if (lines[i].second != '\0')
            for ( size_t j = 0; j < lines[i].first.size(); ++j ) 
                widths[j] = std::max( widths[j], lines[i].first[j].size() );
    ...
    for ( size_t i = 0; i < lines.size(); ++i ) {
        if ( lines[i].second == '\0' ) {
            ...
            out << std::string( total_width, lines[i].second ) << std::endl;
        }
        else {
            for ( size_t j = 0; j < lines[i].first.size(); ++j ) {
                char align = 'l';
                ...
                if ( align == 'l' ) {
                	...
                	out << std::string( widths[j] - lines[i].first[j].size(), ' ' );
                }
                else if ( align == 'r' ) {
                    out << std::string( widths[j] - lines[i].first[j].size(), ' ' ) << lines[i].first[j] ;
                }
