...
struct GZipHeader {
    ...
    bool hasFName() const {
        return mFlags&BGZF_FNAME;
    }
    bool hasComment() const {
        return mFlags&BGZF_FCMNT;
    }
    bool hasHdrCRC() const {
        return mFlags&BGZF_FHCRC;
    }
...
class BAMbuf : public std::streambuf {
  public:
    BAMbuf( String const& bamFile ) : mFR(bamFile), mBlockNo(0) {
        char* end = &mInfBuf.back();
        setg(&mInfBuf.front(),end,end);
    }
...
class BAMbuf : public std::streambuf {
  public:
    BAMbuf( String const& bamFile )
        ...
        char* end = &mInfBuf[0].back();
        setg(&mInfBuf[0].front(),end,end);
    }
...
void BAMReader::readBAM( String const& bamFile,
    ...
    //[GONZA] remove ToStringAddCommas
    std::cout << Date( ) << ": there are " << nReads << " reads";
    if ( reads_b.size( ) > 0 ) {
