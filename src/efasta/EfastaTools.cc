...
void efasta::Canonicalize( efasta& canonical )
{
  ...
  for ( int i = 1; i < blocks.isize()-1; i++ ){
    ...
    if ( se > -1 ){
      //std::cout << "found room for shifting" << std::endl;
      //PRINT( sshift );
      blocks[i-1][0] += sshift;
      for ( int j = 0; j < blocks[i].isize(); j++ )
  ...
  // if ( *this != canonical ){
//     std::cout << "original efasta and canonical version different:" << std::endl;
//     PRINT( *this );
//     PRINT( canonical );
//   }
}
...
void efasta::PrintAmbiguities( std::ostream& out, const String& id ) const
{
  int eventCount = -1;
  vec<char> seps;
  seps.push_back(',');
